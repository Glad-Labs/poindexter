...
generator-not-found fallback to image_gen, metadata re-labelling.
"""
...
class TestAIGenerationProviderFetch:
    ...
    async def test_delegates_to_image_gen_by_default(self):
        image_gen = _mk_provider("image_gen", [
            ImageResult(url="file:///x.png", source="image_gen", metadata={}),
        ])
        ...
        assert results[0].metadata["downstream_generator"] == "image_gen"
        assert results[0].metadata["topic"] == "Docker changed everything"
        image_gen.fetch.assert_awaited_once_with("a cinematic prompt", {})

    async def test_config_forwarded_minus_prompt_model_and_generator(self):
        image_gen = _mk_provider("image_gen", [
            ImageResult(url="file:///x.png", source="image_gen"),
        ])
        ...
        passed = image_gen.fetch.await_args.args[1]
    ...
    async def test_unknown_generator_falls_back_to_image_gen(self):
        image_gen = _mk_provider("image_gen", [
            ImageResult(url="file:///x.png", source="image_gen"),
        ])
        ...
        assert len(results) == 1
        # The downstream tracking says image_gen — the fallback happened.
        assert results[0].metadata["downstream_generator"] == "image_gen"
...
class TestBuildImageGenPrompt:
    async def test_ollama_success_returns_generated(self):
        ...
        with patch("httpx.AsyncClient", return_value=ctx):
            result = await _build_image_gen_prompt("A post title", "llama3:latest")
    ...
    async def test_ollama_failure_returns_fallback(self):
        with patch("httpx.AsyncClient", side_effect=RuntimeError("boom")):
            result = await _build_image_gen_prompt("X", "llama3:latest")
    ...
    async def test_short_response_falls_back(self):
        ...
        with patch("httpx.AsyncClient", return_value=ctx):
            result = await _build_image_gen_prompt("My blog post", "llama3:latest")
    ...
    async def test_no_model_no_pool_returns_fallback_prompt(self):
        ...
        with patch("httpx.AsyncClient", side_effect=AssertionError("must not use httpx")):
            result = await _build_image_gen_prompt("AI chip design", None, site_config=None)
        assert "stylistic or abstract scene related to AI chip design" in result

    async def test_no_model_resolves_via_image_prompt_model_pin(self):
        """model=None + pool → reads the image_prompt_model pin, then dispatch."""
        site_config = MagicMock()
        pool = MagicMock()
        site_config._pool = pool
        site_config.get = MagicMock(side_effect=lambda k, d=None: {
            "image_prompt_model": "ollama/gemma3:27b",
        }.get(k, d))
    ...
    async def test_no_model_no_pin_returns_fallback(self):
        """model=None + image_prompt_model unset → fallback prompt, no crash."""
        site_config = MagicMock()
        site_config._pool = MagicMock()
        site_config.get = MagicMock(return_value="")  # image_prompt_model unset
