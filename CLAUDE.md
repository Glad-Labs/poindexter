...
- 375 Python files under `src/cofounder_agent/services/` (~291 substantive after `__init__.py` stubs; down from ~455 after the 2026-05-08+09+16 cleanup passes). The "load-bearing services" table below covers the services on a critical execution path — `flows/content_generation.py`, `worker_service.py`, and `auto_publish_gate.py` were missing from prior versions of the table.
...
- **Migration files** — `0000_baseline.py` (the squashed history) plus the post-baseline migrations under `services/migrations/`. The baseline is the **Phase E squash (2026-06-06, #1194)** — it supersedes the Phase D baseline (2026-05-29, which absorbed 235 migrations through `20260529_*`) and the original 2026-05-08 squash, folding in every timestamped migration through `20260606_233518_*`. So the files in tree are `0000_baseline.py` plus the 65 migrations from `20260607_*` onward (66 `.py` files total); latest as of 2026-06-20: `20260620_054135_retire_orphaned_ops_triage_system_prompt_app_setting.py` (retires an orphaned ops-triage system-prompt `app_settings` key). Notable post-baseline batches: the `20260607_*` settings seeds (Stable Audio Open music/SFX, podcast Kokoro TTS, and the `qa.self_consistency` rail; #621); `create_skill_catalog` + `create_bench_run_results` + `graduate_eval_rails_to_required` (2026-06-07); the `20260608_*` media-pipeline `graph_def` batch (`seed_media_pipeline_graph_def` plus render/caption/qa/persist/audio-qa reseeds) and `drop_dead_qa_guardrails_node`; the retention-summary tables + sensor-downsample policy (`20260610_*`/`20260611_*`); `reseed_canonical_blog_graph_def_v5_seo_collapsed` (the #734 single-call SEO metadata) and `seed_citation_reconciliation` (#765); the SEO-refresh `graph_def` + `seo_opportunities` table batch (`20260612_*`); `fix_poindexter_set_in_app_settings_descriptions` + `repair_rubric_pipeline_version_titles` (2026-06-13/14); and the topic-discovery V2 + media-approval grandfathering batch (`20260615_*`): `grandfather_video_media_approvals_for_already_live_videos`, `create_topic_pool_table_and_external_taps_niche_id`, `stamp_dispatched_at_on_grandfather_media_approvals_to_defuse_re_dispatch`, and `promote_niche_sources_to_niche_bound_topic_taps` (migrates `niche_sources` rows → niche-bound `external_taps` taps targeting the new `topic_pool` table); `app_settings_updated_at_trigger` (adds `BEFORE UPDATE OF value` trigger to stamp `updated_at` on every value write — fixes forensics gap surfaced by #1594); `reseed_media_pipeline_graph_def_with_render_narration_node` (inserts `render_narration` node between `load_scripts` and `transcribe_narration` to fix silent-video bug in Stage-2, #689); `add_check_constraint_on_pipeline_tasks_status_and_not_null_on_app_settings_value` (adds 15-value NOT VALID + VALIDATE CHECK on `pipeline_tasks.status` and sets `app_settings.value NOT NULL`, #700); and `drop_dead_indexes_embeddingswriter_and_pipeline_tasksstage` (drops two zero-scan indexes, #701); and the `20260617_*`/`20260618_*` batch: `add_media_pipeline_redispatch_count` (adds `media_pipeline_redispatch_count` to `pipeline_tasks`, #689); `remove_bluesky_publishing_adapter_and_atproto_dependency` (drops atproto dependency — Trivy HIGH CVE); `create_agent_permissions_and_approval_queue_tables` (creates `agent_permissions` + `approval_queue` tables, fixes MCP `set_setting` UndefinedTableError); `reseed_canonical_blog_graph_def_qa_rescue_cycle` (adds `qa.rewrite` node + conditional branch/loop edges for the bounded QA rescue cycle); and `add_settings_lifecycle_metadata_columns_owner_value_type_deprecated_superseded_by` (adds `owner`, `value_type`, `deprecated`, `superseded_by` lifecycle-metadata columns to `app_settings`, #756); and the `20260619_*`/`20260620_*` batch: `reseed_canonical_blog_graph_def_v6_director_review` (inserts the `review_video_shot_list` director self-critique node, taking `canonical_blog` to 38 nodes), `align_pipeline_gate_history_post_id_to_uuid`, and `retire_orphaned_ops_triage_system_prompt_app_setting`. New schema changes still go in fresh `YYYYMMDD_HHMMSS_<slug>.py` files; the runner sorts `0000_baseline.py` first because `0` < `2` lexically.
