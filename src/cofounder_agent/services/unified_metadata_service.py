...
class UnifiedMetadataService:
    ...
    def __init__(self, model: str = "auto"):
        ...
        if not self.llm_available:
            logger.warning("⚠️  No LLM available (Anthropic or OpenAI). Using fallback strategies only.")
    ...
    async def generate_all_metadata(
        ...
        metadata = UnifiedMetadata(
            author_id=author_id or "14c9cad6-57ca-474a-8a6d-fab897388ea8"
        )
        ...
        social = self.generate_social_metadata(
            metadata.title,
            metadata.excerpt,
            featured_image_url
        )
        ...
        logger.info(f"✅ Metadata generation complete: title={metadata.title[:50]}, "
...
Generate ONLY the title, nothing else. No quotes, no explanation."""
    ...
    async def generate_excerpt(
        self,
        content: str,
        stored_excerpt: Optional[str] = None,
        max_length: int = 200
    ) -> str:
        """
...
Generate ONLY the excerpt, nothing else. No quotes, no explanation."""
    ...
    async def generate_seo_metadata(
        ...
        title: str,
        content: str,
        stored_seo: Optional[Dict[str, str]] = None
    ) -> Dict[str, Any]:
        """
...
Generate ONLY comma-separated keywords, nothing else. Example: keyword1, keyword2, keyword3"""
    ...
    def _extract_keywords_fallback(self, title: str) -> List[str]:
        ...
        common_words = {'a', 'an', 'the', 'and', 'or', 'but', 'is', 'are', 'to', 'of', 'in', 'on', 'for'}
    ...
    async def _llm_match_category(
        ...
        categories_text = "\n".join([
            f"- {cat['name']}: {cat.get('description', 'N/A')}"
            for cat in available_categories
        ])
