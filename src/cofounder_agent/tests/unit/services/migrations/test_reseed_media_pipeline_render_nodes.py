"""Contract test for the media_pipeline render-node re-seed migration (Plan 4).

The re-seed must INSERT…ON CONFLICT (slug) DO UPDATE the ``media_pipeline`` row
in ``pipeline_templates`` so an already-seeded spine row is upgraded in place to
the Plan-4 graph_def (now carrying the render nodes). It must NOT delete on
down() — the row is intentionally retained (re-seed, not create). The graph_def
source module must stay pure data so migrations-smoke can import it without a
full app boot.
"""
...
_MIGRATION = (
    ...
    / "services"
    / "migrations"
    / "20260608_130000_reseed_media_pipeline_render_nodes.py"
)


def _load_migration():
    spec = importlib.util.spec_from_file_location("reseed_media_pipeline_mig", _MIGRATION)
...
@pytest.mark.asyncio
async def test_up_upserts_media_pipeline_with_render_nodes():
    mod = _load_migration()
    ...
    # carries the two render node ids — the substance of this re-seed.
...
def test_graph_def_carries_render_nodes():
    from services.media_pipeline_spec import MEDIA_PIPELINE_GRAPH_DEF

    node_ids = {n["id"] for n in MEDIA_PIPELINE_GRAPH_DEF["nodes"]}
    assert {"load_scripts", "render_long_video", "render_short_video"} <= node_ids
    node_atoms = {n["atom"] for n in MEDIA_PIPELINE_GRAPH_DEF["nodes"]}
    assert "media.render_long_video" in node_atoms
    assert "media.render_short_video" in node_atoms
