...
class MCPCapabilityRegistry:
    ...
    async def discover_by_capability(self, capability: str) -> List[MCPServer]:
        ...
        available.sort(
            key=lambda s: (s.quality_score / max(s.cost_per_call, 0.001)),
            reverse=True
        )
    ...
    async def call_server(
        ...
        try:
            ...
            if method.upper() == "GET":
                ...
                response = await self.client.post(url, json=json_body, params=params, headers=headers)
            else:
...
class Poindexter_MCPIntegration:
    ...
    async def call_mcp_server_tool(
        ...
        # Call the server
        success, result, error = await self.client.call_server(
            server, method, endpoint, params
        )
...
if __name__ == "__main__":
    async def main():
        ...
        for server in servers:
            print(f"  - {server['name']}: ${server['cost_per_call']}/call, quality={server['quality_score']}")
