...
- ~295 Python files under `src/cofounder_agent/services/` (down from ~455 after the 2026-05-08 cleanup pass — pipeline_flow.py, taps/gitea_issues.py, voice_agent_webrtc.py, phases/example_workflows.py, and the model_router/usage_tracker/model_constants trio all removed). 18 services are load-bearing (table below). The `agents/content_agent/` tree (~22 files + ~15 tests) was _flagged_ for deletion but is still wired through `main.py` → `custom_workflows_service` → `workflow_executor` → `agents.content_agent.agents.{research,creative,qa}_agent`. Removal blocked on the canonical_blog template (#356); 0% production traffic but load-bearing for the LangGraph migration sequence. The **2026-05-08 services audit** at `.shared-context/audits/2026-05-08-services-folder-audit.md` flagged the original ~5,000 LOC deletable; about 1,800 LOC of that landed; Phase 3 (workflow_executor chain → template_runner) is the largest remaining chunk and waits on a canonical_blog template.
