...
- **`template_runner.py`** — LangGraph-backed dynamic-pipeline orchestrator. **Primary pipeline path** (`default_template_slug=canonical_blog` on prod). `run()` prefers the DB-stored `graph_def` (compiled by `pipeline_architect.build_graph_from_spec`) when `pipeline_use_graph_def=true` — the prod default since #355 — else the legacy Python `TEMPLATES` factory. Drives `canonical_blog` (18-node graph_def) + `dev_diary` (4-node in-tree factory; the hand-coded `canonical_blog` factory was deleted). Postgres checkpointer enabled via `template_runner_use_postgres_checkpointer=true`.
...
| 3.5–3.7 | `atoms/qa_*.py`             | Multi-model QA as composable rail atoms (`qa.critic` / `qa.deepeval` / `qa.guardrails` / `qa.ragas` → `qa.aggregate`) delegating to `multi_model_qa.py`. `qa.aggregate` halts on reject — no rewrite loop. Replaced the deleted `cross_model_qa.py` (#355). |
...
See [`architecture/services/template_runner.md`](../architecture/services/template_runner) for
