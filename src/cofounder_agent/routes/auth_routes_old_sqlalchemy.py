...
async def get_current_user(
    ...
    # Development: Create mock user if using mock token and user doesn't exist
    if not user and token.startswith("mock_jwt_token_"):
        # For development, return mock user dict
        user = {
            "id": user_id,
...
def get_database_service() -> DatabaseService:
    ...
    # TODO: Inject from main.py instead
    global _db_service
    if not hasattr(get_database_service, '_instance'):
        from main import database_service
        if database_service:
            get_database_service._instance = database_service
        else:
            get_database_service._instance = DatabaseService()
