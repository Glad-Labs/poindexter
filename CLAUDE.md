...
- 409 Python files under `src/cofounder_agent/services/` (~291 substantive after `__init__.py` stubs)
...
- ~11,440 test functions across 994 test files under `tests/unit` (+ 16 integration + 17 integration_db). Last full-green nightly 2026-06-09 (10,352 passed, 84 skipped, 1 xfailed, 0 collection errors); the suite has grown ~1,200 test functions since, so re-run to refresh the tally. A handful skip in-container due to the `Path(__file__).parents[5]` path-depth quirk and run on host.
