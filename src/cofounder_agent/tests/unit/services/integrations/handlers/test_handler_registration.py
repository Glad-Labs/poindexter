...
def test_discord_module_deleted():
    with pytest.raises(ModuleNotFoundError):
        importlib.import_module(
            "poindexter.services.integrations.handlers.outbound_discord"
        )


def test_telegram_streaming_helpers_retained():
    mod = importlib.import_module(
        "poindexter.services.integrations.handlers.outbound_telegram"
    )
...
def test_outbound_discord_not_in_http_client_wiring():
    from poindexter.services.http_client import WIRED_HTTP_CLIENT_MODULES
