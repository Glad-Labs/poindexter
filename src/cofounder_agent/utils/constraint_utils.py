...
class ContentConstraints:
    ...
    writing_style: str = "educational"  # Style: technical, narrative, listicle, educational, thought-leadership
...
def validate_constraints(
    ...
    percentage_diff = ((actual_words - target_words) / target_words * 100) if target_words > 0 else 0
...
def calculate_phase_targets(
    ...
    if constraints.per_phase_overrides:
        targets = {phase: constraints.per_phase_overrides.get(phase, total_word_count // num_phases) 
...
def apply_strict_mode(compliance: ConstraintCompliance) -> Tuple[bool, str]:
    ...
    if not compliance.word_count_within_tolerance:
        return False, compliance.violation_message or "Word count constraint violated in strict mode"
...
def auto_trim_content(
    ...
    logger.info(f"Auto-trimmed content from {current_words} to {count_words_in_content(trimmed_content)} words")
...
def calculate_cost_impact(
    ...
    return {
        ...
        "efficiency_ratio": round(constraint_word_count / original_word_count, 2) if original_word_count > 0 else 1.0
    }
