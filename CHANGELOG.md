...
- **Minor release 1.2.0.** Moves five more environment variables (`SERPER_API_KEY`, `LOCAL_LLM_API_URL`, `LOCAL_LLM_MODEL_NAME`, `MAX_LOG_SIZE_MB`, `MAX_LOG_BACKUP_COUNT`) into the database-backed settings store (see Updates below). Pin to [`v1.2.0`](https://github.com/Glad-Labs/poindexter/releases/tag/v1.2.0) for the latest stable upgrade target. ([#301](https://github.com/Glad-Labs/poindexter/pull/301))
...
- **Five more settings move from env vars to `app_settings`.** `SERPER_API_KEY`, `LOCAL_LLM_API_URL`, `LOCAL_LLM_MODEL_NAME`, `MAX_LOG_SIZE_MB`, and `MAX_LOG_BACKUP_COUNT` now live in the database-backed settings store, so you can change them at runtime without redeploying. `SERPER_API_KEY` is stored as a secret. Existing env-var values keep working as a fallback, so nothing breaks on upgrade. ([#300](https://github.com/Glad-Labs/poindexter/pull/300))
