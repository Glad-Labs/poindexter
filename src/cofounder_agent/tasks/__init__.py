"""Compatibility stub -- `tasks` now lives at `poindexter.tasks` (Glad-Labs/poindexter#1046, step 2).

Importing this installs the flat-import alias finder and replaces this module in
``sys.modules`` with the canonical package, so ``import tasks.x`` yields the SAME
object as ``import poindexter.tasks.x`` -- one module, two names (see
...
# `cofounder_agent.tasks` (the umbrella spelling the entry points use).
