    ...
    def register_all_middleware(self, app: FastAPI, *, site_config: Any = None) -> None:
        ...
        Args:
            ...
            site_config: SiteConfig instance (DI — Phase H, GH#95). Used
                by ``_setup_cors`` to read ``allowed_origins``. Optional
                for back-compat — when None, CORS falls back to the
                uppercased env var (``ALLOWED_ORIGINS``) or the built-in
                localhost defaults.
        ...
        Example:
            ...
            middleware_config.register_all_middleware(app, site_config=sc)
        """
    ...
    def _setup_cors(self, app: FastAPI, *, site_config: Any = None) -> None:
        ...
        Args:
            ...
            site_config: SiteConfig instance (DI — Phase H, GH#95). When
                None, ``ALLOWED_ORIGINS`` env var is consulted via
                ``os.getenv`` for back-compat with the test harness.
        ...
        # Get allowed origins from site_config (with env-var fallback for
        # tests that don't wire up a SiteConfig instance).
