...
const requiredVars = {
  // Database
  DATABASE_CLIENT: { value: undefined, type: 'string', desc: 'Must be "postgres"' },
  DATABASE_URL: { value: undefined, type: 'string', desc: 'PostgreSQL connection string' },
  
  // Server
  URL: { value: undefined, type: 'string', desc: 'Production domain with https://' },
  ...
  ADMIN_JWT_SECRET: { value: undefined, type: 'secret', desc: 'JWT secret for admin' },
  APP_KEYS: { value: undefined, type: 'secret', desc: 'Session encryption keys' },
  API_TOKEN_SALT: { value: undefined, type: 'secret', desc: 'API token salt' },
  TRANSFER_TOKEN_SALT: { value: undefined, type: 'secret', desc: 'Transfer token salt' },
};
...
Object.entries(requiredVars).forEach(([key, meta]) => {
  ...
  if (!hasValue) {
    ...
    let display = value;
    if (meta.type === 'secret') {
      display = value.substring(0, 20) + '...' + (value.length > 20 ? ` (${value.length} chars)` : '');
    }
