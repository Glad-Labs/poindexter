...
class ModularPipelineExecutor:
    ...
    async def execute(self, request: WorkflowRequest) -> WorkflowResponse:
        ...
        try:
            ...
            for task_index, task_name in enumerate(pipeline):
                try:
                    ...
                    if task_result.status == TaskStatus.FAILED:
                        ...
                        self.logger.error(error_msg, extra={"workflow_id": context.workflow_id})
                    ...
                    self.logger.error(
                        error_msg, extra={"workflow_id": context.workflow_id}, exc_info=True
                    )
            ...
            self.logger.error(
                error_msg, extra={"workflow_id": request.workflow_id or "unknown"}, exc_info=True
            )
    ...
    async def resume_workflow(
        ...
        if approval_action == "reject":
            self.logger.info(
                f"Workflow {checkpoint.workflow_id} rejected", extra={"user_id": user_id}
            )
            return WorkflowResponse(
        ...
        self.logger.info(
            f"Resuming workflow {checkpoint.workflow_id} after approval", extra={"user_id": user_id}
        )
    ...
    async def _handle_task_error(
        ...
        if max_retries > 0:
            self.logger.info(
                f"Retrying task {task_result.task_name} " f"(attempt 1 of {max_retries})"
            )
            return True
