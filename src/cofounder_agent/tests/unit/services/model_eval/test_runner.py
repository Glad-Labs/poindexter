...
async def _run(table: dict[str, float], *, challengers, margin=0.02, harness=None):
    return await run_slot_eval(
        slot=_SLOT,
        champion="champ",
...
@pytest.mark.asyncio
async def test_challenger_beating_margin_wins() -> None:
    rep = await _run({"champ": 0.80, "chall": 0.86}, challengers=["chall"])
...
@pytest.mark.asyncio
async def test_challenger_within_margin_keeps_champion() -> None:
    rep = await _run({"champ": 0.80, "chall": 0.805}, challengers=["chall"])  # +0.6% < 2%
    assert rep.winner == "champ"
    assert rep.beats_margin is False


@pytest.mark.asyncio
async def test_best_of_several_challengers_is_chosen() -> None:
    rep = await _run({"champ": 0.80, "a": 0.83, "b": 0.88}, challengers=["a", "b"])
    assert rep.best_challenger == "b"
    assert rep.winner == "b"


@pytest.mark.asyncio
async def test_no_challengers_keeps_champion() -> None:
    rep = await _run({"champ": 0.80}, challengers=[])
    assert rep.winner == "champ"
...
@pytest.mark.asyncio
async def test_results_recorded_to_harness() -> None:
    h = InMemoryEvalHarness()
    await _run({"champ": 0.80, "chall": 0.86}, challengers=["chall"], harness=h)
    latest = await h.latest_by_model(_SLOT, "ndcg@10")
    assert set(latest) == {"champ", "chall"}


@pytest.mark.asyncio
async def test_zero_champion_score_does_not_divide_by_zero() -> None:
    rep = await _run({"champ": 0.0, "chall": 0.5}, challengers=["chall"])
    assert rep.beats_margin is True
    assert rep.winner == "chall"
