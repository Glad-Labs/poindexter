...
class UnifiedOrchestrator:
    ...
    async def _parse_request(
        ...
        if any(kw in input_lower for kw in ["create content", "write about", "new post", "blog post"]):
            return Request(
                request_id=request_id,
        ...
        elif any(kw in input_lower for kw in ["financial", "budget", "spending", "revenue", "balance"]):
            return Request(
                request_id=request_id,
                original_text=user_input,
        ...
        elif any(kw in input_lower for kw in ["should i ", "should we ", "what should ", "recommend", "suggest"]):
            return Request(
                request_id=request_id,
                original_text=user_input,
        ...
        elif any(kw in input_lower for kw in ["help", "status", "health", "commands", "what can you do"]):
            return Request(
                request_id=request_id,
                original_text=user_input,
        ...
        elif any(kw in input_lower for kw in ["stop", "cancel", "intervene", "emergency", "override"]):
            return Request(
                request_id=request_id,
                original_text=user_input,
    ...
    async def _handle_financial_analysis(self, request: Request) -> ExecutionResult:
        ...
        try:
            ...
            result = await agent.analyze() if asyncio.iscoroutinefunction(agent.analyze) else agent.analyze()
            return ExecutionResult(
                request_id=request.request_id,
                request_type=request.request_type,
                status=ExecutionStatus.COMPLETED,
    ...
    async def _handle_compliance_check(self, request: Request) -> ExecutionResult:
        ...
        try:
            result = await agent.audit() if asyncio.iscoroutinefunction(agent.audit) else agent.audit()
            return ExecutionResult(
                request_id=request.request_id,
                request_type=request.request_type,
                status=ExecutionStatus.COMPLETED,
