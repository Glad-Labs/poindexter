...
**Task 9 is deferred to a focused follow-up PR** (see the REVISED note on Task 9 below). During execution the plan's uniform-migration assumption proved wrong — the ~17 effects are not all clean `fetch → setState`, and ~11 already surface errors via a toast (not the silent `catch(){}` the spec targets). The follow-up migrates only the clean single-state effects and documents the rest as the spec-sanctioned "explicit, listed exceptions." `<Freshness>` + `agoLabel` are shipped-but-unrendered until then (staged helpers).
