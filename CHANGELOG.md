...
- **Voice agent service — hands-free meetings with your local LLM.** New `voice_agent` service runs a real-time voice loop entirely on your machine: mic → VAD → faster-whisper STT → Ollama → Kokoro TTS → speakers. No cloud, no API keys. Six `app_settings` knobs (`voice_agent_llm_model`, `voice_agent_ollama_url`, `voice_agent_tts_voice`, `voice_agent_tts_speed`, `voice_agent_whisper_model`, etc.) tune it at runtime — flip voice, model, or speed without restarting. Seeded by migration `0104`. ([#39c8a22](https://github.com/Glad-Labs/poindexter/commit/39c8a22288d1c2da1a54973baf88093b763f2a1e))
- **`poindexter migrate` CLI.** Three new subcommands let you manage schema migrations without bouncing the worker: `poindexter migrate status` lists applied vs pending, `poindexter migrate up [N]` applies the next N (or all) pending migrations, and `poindexter migrate down [N]` rolls back the last N. Previously the only way to apply a migration was to restart the worker container or run a Python one-liner. ([#226](https://github.com/Glad-Labs/poindexter/issues/226))
...
- **Relicensed to Apache-2.0.** Poindexter is now Apache-2.0 (was AGPL-3.0-or-later). The doc-paywall posture is dropped — all docs that previously gated behind paid tiers are now in the public repo. Existing forks and downstream users get a more permissive license with no copyleft obligations. ([#62459e2](https://github.com/Glad-Labs/poindexter/commit/62459e204793739d077a2fcf7c47c68a7b5edbba))
- **`brand_keywords` is now configurable per niche.** The topic-discovery brand-keyword set used to be hardcoded to a tech/AI/ML/gaming voice. It's now an `app_settings` value (`topic_discovery_brand_keywords`) so a real-estate, dental, or indie-musician install can override the keyword list without forking the codebase. Empty default preserves the built-in tech behavior. ([#216](https://github.com/Glad-Labs/poindexter/issues/216))
- **Public repo is now auto-synced from `main`.** Every push to the upstream `main` branch now mirrors a filtered subset (private operator files stripped) into the public [`Glad-Labs/poindexter`](https://github.com/Glad-Labs/poindexter) repo via GitHub Actions. Public-repo users no longer wait on manual sync runs to pick up new releases.
...
- **Fresh-DB installs no longer fail on broken migrations.** A sweep across the migration set repaired nine migrations that wouldn't apply against a clean database (a number-collision on migration `0093` and eight downstream issues). Fresh installs now boot cleanly. ([#86](https://github.com/Glad-Labs/poindexter/issues/86), [#87](https://github.com/Glad-Labs/poindexter/issues/87))
- **Migration `0114` now handles production schema where `content_tasks` is a VIEW.** The migration assumed `content_tasks` was a table and crashed on installs that had migrated it to a VIEW. It now detects and handles both shapes. ([#185c9a3](https://github.com/Glad-Labs/poindexter/commit/185c9a3))
- **CLI no longer references a non-existent `api_auth_token` row on a fresh install.** A leftover orphan reference made `poindexter` fail with a confusing error on fresh DBs; the row is now cleaned up and the error message is corrected. ([#231](https://github.com/Glad-Labs/poindexter/issues/231))
