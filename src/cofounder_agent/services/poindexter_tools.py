...
class PoindexterTools:
        ...
        try:
            ...
            if constraints.get("enable_critique", True) and max_iterations > 1:
                for iteration in range(max_iterations - 1):
                    ...
                    logger.info(f"[GENERATE] Step {iteration + 2}: Quality critique (score: {current_quality:.2f})")
    ...
    async def critique_content_tool(
        ...
        try:
            ...
            logger.info(f"[CRITIQUE] Evaluating {len(content)} chars against {len(criteria)} criteria")
