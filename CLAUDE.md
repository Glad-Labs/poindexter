...
- **Module v1 (Glad-Labs/poindexter#490):** [`docs/architecture/module-v1.md`](docs/architecture/module-v1.md) — Phase 1 (Module Protocol + `get_modules()` registry + manifest validation) shipped 2026-05-13. The 20th entry-point group (`poindexter.modules`) joins the existing 19 in `plugins/registry.py`. Each Module bundles lower-level plugin contributions (stages / reviewers / probes / jobs / taps / adapters / providers / packs) plus the things the existing plugin registry doesn't yet track (DB migrations, Grafana panels, HTTP routes, CLI subcommands). Phases 2-5 (per-module migrations, ContentModule package, route/dashboard auto-discovery, visibility flag) get their own plans.
