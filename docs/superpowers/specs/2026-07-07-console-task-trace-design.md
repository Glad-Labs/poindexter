...
| Langfuse (run + step)                                              | existing `GET /api/traces?task_id=` (Langfuse `sessionId`) + `plugins/tracing.py` `{public_url}/trace/{trace_id}`                  | reuse (session id stamped at dispatch — see note) |

> **2026-07-10 — the `sessionId = task_id` linkage is now actually enforced.** This row presumed Langfuse traces carried `sessionId = task_id`, but nothing stamped it: an audit found **0 of 3,236** recent traces had a session, so every deeplink here resolved to nothing. `services/llm_providers/dispatcher.py::dispatch_complete` now sets the reserved Langfuse OTEL attribute **`session.id = task_id`** (constant `LangfuseOtelSpanAttributes.TRACE_SESSION_ID`, verified against the installed langfuse 4.x SDK) plus **`langfuse.observation.metadata.phase`** on every task-attributed LLM span, so a task's calls group into one session and the deeplink resolves. The same span now also stamps the reserved **`langfuse.observation.input` / `.output`** attributes (the serialized prompt + completion), so every direct dispatch caller — the QA rails, citation repair, media scripts — carries its I/O in the trace, not just token counts (previously only the `ollama_chat_text`-wrapped writer path did; the raw span held usage but 0/4073 input). A `_MAX_SPAN_IO_CHARS` cap truncates pathological payloads (the base64 image the vision rail routes through `dispatch_complete`). `dispatch_embed` is unchanged (no `task_id` in scope).
