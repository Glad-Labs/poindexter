...
### Key Numbers (repo-derived stats auto-synced daily by CI; DB-derived counts last refreshed 2026-08-15)
...
- 183 live posts on gladlabs.io (357 posts total; 2,036 pipeline_tasks across all generation runs)
- 434 Python files under `src/cofounder_agent/services/` (~291 substantive after `__init__.py` stubs)
...
- **Migration files** — the Phase G squash (2026-07-11) folded the Phase F baseline and its 42 post-baseline migrations into a fresh `0000_baseline.py` (+ `.schema.sql` + `.seeds.sql`). 36 ordinary migrations have accumulated since (`20260712_084907_rename_opening_to_content_originality.py` … `20260816_041519_self_claim_rail_gate_row_and_canonical_reseed_v10.py` — spell the newest one out in full, `.py` and all: `claude_md_sync`'s `migration_drift_note` is a literal substring test for that filename, so a `_*` glob leaves the nightly Discord note firing no matter how current the count is). Baseline seeds: non-secret `app_settings` 692 (+2 empty secret placeholders), `pipeline_templates` 6, `qa_gates` 20, `retention_policies` 32. Mechanics in the Database-migrations section.
...
- 69,173 embeddings across posts / issues / audit / memory / brain / claude_sessions (retention prunes claude_sessions/brain vectors, so this drifts)
