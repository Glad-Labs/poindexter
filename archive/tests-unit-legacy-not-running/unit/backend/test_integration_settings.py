...
class TestSettingsWorkflow:
    ...
    def test_create_read_update_delete_workflow(self, client, mock_user):
        ...
        with patch(
            ...
            # CREATE
            create_data = {"theme": "dark", "language": "en", "notifications_enabled": True}
            response_create = client.post("/api/settings", json=create_data, headers=auth_headers)
            ...
            response_update = client.put("/api/settings", json=update_data, headers=auth_headers)
...
class TestSettingsWithAuthentication:
    ...
    def test_settings_requires_valid_token(self, client):
        ...
        # Invalid token
        response = client.get("/api/settings", headers={"Authorization": "Bearer invalid"})
        assert response.status_code == 401
    ...
    def test_settings_with_multiple_users(self, client):
        ...
        with patch("cofounder_agent.routes.settings_routes.get_current_user", return_value=user1):
            ...
            response1 = client.post(
                "/api/settings", json={"theme": "dark"}, headers={"Authorization": "Bearer token1"}
            )
        ...
        with patch("cofounder_agent.routes.settings_routes.get_current_user", return_value=user2):
            ...
            response2 = client.post(
                "/api/settings", json={"theme": "light"}, headers={"Authorization": "Bearer token2"}
            )
...
class TestSettingsBatchOperations:
    ...
    def test_bulk_update_settings(self, client, mock_user):
        ...
        with patch(
            ...
            response = client.put(
                "/api/settings", json=bulk_data, headers={"Authorization": "Bearer fake-token"}
            )
            assert response.status_code in [200, 201, 404]

    def test_partial_bulk_update(self, client, mock_user):
        """Test partial bulk update of settings"""
        partial_data = {"theme": "dark", "notifications_enabled": False}

        with patch(
            "cofounder_agent.routes.settings_routes.get_current_user", return_value=mock_user
        ):
            response = client.put(
                "/api/settings", json=partial_data, headers={"Authorization": "Bearer fake-token"}
            )
            assert response.status_code in [200, 201, 404]
...
class TestSettingsErrorHandling:
    ...
    def test_settings_with_extra_fields(self, client, mock_user):
        ...
        with patch(
            ...
            response = client.post(
                "/api/settings",
                json={"theme": "dark", "unexpected_field": "value", "another_field": 123},
                headers={"Authorization": "Bearer fake-token"},
            )
...
class TestSettingsConcurrency:
    ...
    def test_concurrent_reads(self, client, mock_user):
        ...
        with patch(
            ...
            for _ in range(5):
                response = client.get(
                    "/api/settings", headers={"Authorization": "Bearer fake-token"}
                )
    ...
    def test_all_settings_changes_logged(self, client, mock_user):
        ...
        with patch(
            ...
            with patch("cofounder_agent.middleware.audit_logging.log_audit") as mock_audit:
                ...
                # Delete
                client.delete("/api/settings", headers={"Authorization": "Bearer fake-token"})
