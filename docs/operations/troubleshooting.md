...
**Root cause.** Not a wedged autovacuum, and not a config override — `SELECT source FROM pg_settings WHERE name LIKE 'autovacuum%'` reads `default` for every parameter on this stack. The actual cause is that `poindexter-postgres-local` restarts non-gracefully far more often than expected. Postgres's own log has the tell: `database system was interrupted; last known up at ...` — the crash-recovery message, as opposed to the clean `received fast shutdown request` → `shutting down` → `database system is shut down` sequence. Across a representative 2-week log window, 20+ restarts were crash-recovered vs. only 3 clean shutdowns. The likely trigger is the known WSL2/Docker Desktop backend wedge (`project_vram_oversub_docker_crashes` memory) — its documented recovery path (`wsl --shutdown`, run manually or by `scripts/docker-watchdog.ps1`) force-kills the whole VM, giving Postgres no chance to checkpoint.

PostgreSQL discards cumulative `pg_stat_user_tables` counters on crash recovery by design — a crash-recovered stats snapshot can't be trusted to match the WAL-replayed state — so `n_live_tup`/`n_dead_tup`/`autovacuum_count`/`last_autovacuum` reset to zero/null on _every_ crash-restart, not just the first one. Because `n_dead_tup` is also what autovacuum's own trigger logic reads, a table's real dead-tuple bloat becomes invisible to that trigger the moment the counters reset: the dead tuples are still physically on disk, but Postgres now believes the table is pristine and won't fire again until _new_ churn re-accumulates past the threshold (`autovacuum_vacuum_threshold` + `autovacuum_vacuum_scale_factor` × live rows) from that false zero baseline. Tables with heavy churn (`job_run_state`, `capability_registry`) cross the threshold fast enough to fire before the next crash; slower tables (`sensor_samples`, `checkpoints`, `checkpoint_writes`, `gpu_metrics`, `atom_runs`, `brain_decisions`, `alert_events`, `pipeline_versions`, `gpu_task_sessions`, `post_performance`, and others) don't, so their bloat silently persists across restart cycles.

**Fix.** Don't chase the WSL2 wedge from here — it's a separate, already-tracked infrastructure issue with its own watchdog (`project_vram_oversub_docker_crashes` memory, `scripts/docker-watchdog.ps1`). The actionable fix for this symptom is a scheduled `VACUUM (ANALYZE)` that doesn't depend on autovacuum's stats-driven trigger at all: `services/jobs/postgres_vacuum.py`'s `PostgresVacuumJob` already existed (every 6 hours, via the `plugin.job.postgres_vacuum` app_setting) but only covered 7 tables. Extended `DEFAULT_TABLES` to add the ten tables identified in this investigation (`sensor_samples`, `checkpoints`, `checkpoint_writes`, `gpu_metrics`, `atom_runs`, `brain_decisions`, `alert_events`, `pipeline_versions`, `gpu_task_sessions`, `post_performance`) so they get vacuumed on a guaranteed floor regardless of how recently the container crash-restarted. No `settings_defaults.py`/migration change was needed — the `plugin.job.postgres_vacuum` app_settings row has no `config.tables` override on prod, so the Python-level default applies on both prod and fresh installs.

For an immediate one-off fix on a table that's already badly bloated: `VACUUM (ANALYZE) <table>;` by hand (or `VACUUM FULL ANALYZE <table>;` if you need the disk space reclaimed to the OS — it takes an `ACCESS EXCLUSIVE` lock, so only do this outside the table's hot path).

**Verify.** `SELECT relname, n_live_tup, n_dead_tup, last_autovacuum, autovacuum_count FROM pg_stat_user_tables WHERE relname = '<table>';` alongside a real `SELECT count(*) FROM <table>;` to confirm the stats caught up. `docker logs poindexter-postgres-local 2>&1 | grep -c "was interrupted"` gives a rough count of how crash-prone the container has been recently.

**Related.** `services/jobs/postgres_vacuum.py`, `tests/unit/services/jobs/test_postgres_vacuum_job.py`, `project_vram_oversub_docker_crashes` memory (the WSL2/Docker wedge + `wsl --shutdown` recovery mechanism), `docs/operations/self-healing.md` § "Docker port-forward recovery" (a related but distinct failure mode of the same underlying wedge).
