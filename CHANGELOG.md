...
- **Fresh-database installs apply every migration cleanly.** New installs against an empty Postgres used to leave 10 of 95 migrations unapplied — production was unaffected because it was bootstrapped via `init.sql` first, but spinning up a brand-new database from scratch silently skipped pieces of the schema. Two duplicate migrations sharing the same number have been renumbered, and nine migrations that assumed a pre-bootstrapped database now apply (or skip cleanly) on a fresh container. Existing deployments are unaffected — every change is idempotent. ([#86](https://github.com/Glad-Labs/poindexter/pull/86), [#87](https://github.com/Glad-Labs/poindexter/pull/87))
