...
def financial_agent():
    ...
    return FinancialAgent(
        cofounder_api_url="http://localhost:8000", pubsub_client=None, enable_cost_tracking=False
    )
...
def financial_agent_with_tracking():
    ...
    return FinancialAgent(
        cofounder_api_url="http://localhost:8000", pubsub_client=None, enable_cost_tracking=True
    )
...
class TestFinancialAgentInitialization:
    ...
    def test_with_cost_tracking_enabled(self):
        ...
        agent = FinancialAgent(cofounder_api_url="http://localhost:8000", enable_cost_tracking=True)
    ...
    def test_with_pubsub_client(self):
        ...
        mock_pubsub = Mock()
        agent = FinancialAgent(pubsub_client=mock_pubsub, enable_cost_tracking=True)
...
class TestAnalyzeCosts:
    ...
    async def test_analyze_costs_with_alert(self, financial_agent_with_tracking):
        ...
        mock_analysis_with_alert = {
            ...
            "projections": {"projected_monthly_total": 160.0, "projected_overage": 60.0},
        }
...
class TestGetMonthlySummary:
    ...
    def test_get_monthly_summary_with_alerts(self, financial_agent_with_tracking):
        ...
        mock_summary_with_alerts = {
            ...
            "projections": {"projected_monthly_total": 170.0, "projected_overage": 70.0},
        }
    ...
    def test_get_monthly_summary_projections(
        self, financial_agent_with_tracking, mock_monthly_summary
    ):
    ...
    async def test_alert_propagation(self, financial_agent_with_tracking):
        ...
        mock_analysis_with_alert = {
            ...
            "alert": {
                ...
                "recommendations": ["Prioritize critical tasks only", "Increase cache hit rate"],
            },
            "recommendations": [],
        }
...
class TestErrorHandling:
    ...
    async def test_analyze_costs_api_failure(self, financial_agent_with_tracking):
        ...
        with patch.object(
            ...
            "analyze_costs",
            new=AsyncMock(
                return_value={"status": "error", "message": "Failed to fetch cost metrics"}
            ),
        ):
            result = await financial_agent_with_tracking.analyze_costs()
