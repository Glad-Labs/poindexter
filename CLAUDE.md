...
- ~327 Python files under `src/cofounder_agent/services/` (~285 substantive after migrations + `__init__.py` stubs; down from ~455 after the 2026-05-08+09+16 cleanup passes). The "load-bearing services" table below covers the 19 services on a critical execution path — `flows/content_generation.py`, `worker_service.py`, and `auto_publish_gate.py` were missing from prior versions of the table; `content_router_service.py` and `task_executor.py` are scheduled for deletion in the current sweep.
- **In-flight cleanup sweep (2026-05-16)**: 10 PRs ranked by ROI — Prefect Stage 4 (delete `task_executor.py`), LangGraph cutover finalization (delete `content_router_service.py` legacy chunked path), llm_text → dispatcher consolidation, shared httpx client, Module v1 Phase 3.5 (ContentModule physical move + FinanceModule wiring), settings discipline. **Cutover gates are all already `true` on prod** — Prefect is the dispatcher, canonical_blog is the pipeline, LiteLLM is the LLM router, LlamaIndex+Ragas+DeepEval+Guardrails are all on. The legacy code is dead-but-loaded; the sweep removes the dead weight.
...
| `template_runner.py`                      | LangGraph-backed dynamic-pipeline orchestrator (TemplateRunner). **PRIMARY PIPELINE PATH as of 2026-05-16** (`canonical_blog` template is the prod default). Drives `canonical_blog` + `dev_diary` + future architect-composed pipelines. Postgres checkpointer enabled via `template_runner_use_postgres_checkpointer=true`. Templates registered in `services/pipeline_templates/__init__.py`.                                                                                                                                                                                                                                                                                                                                                                             |
...
- **Prefect cutover (#410):** [`docs/architecture/prefect-cutover.md`](docs/architecture/prefect-cutover.md) — **LIVE on prod**: `app_settings.use_prefect_orchestration=true`. `services/flows/content_generation.py` is the dispatch path; the Prefect server at port 4200 owns task scheduling. The legacy `task_executor.py` 5-second polling daemon is scheduled for deletion in the current sweep (Stage 4 of the cutover). Operator UI is Prefect's at http://localhost:4200.
