...
| File:line                                                     | Key                      | Containing function                      | Why deferred                                                                                                                                                                                                                                                                                                                                                                               | Follow-up shape                                                                                                                                                                                                                                                                                                                                                                | Status                                                                                                                                                                                                  |
| ------------------------------------------------------------- | ------------------------ | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/cofounder_agent/services/webhook_delivery_service.py:35` | `openclaw_webhook_token` | `WebhookDeliveryService.__init__` (sync) | Constructor is called from sync wiring (`main.py` lifespan, DI container). Cannot `await` here.                                                                                                                                                                                                                                                                                            | Stop reading the token in `__init__`; either lazy-load it inside `_deliver_event` (already async), or read it via `await site_config.get_secret(...)` once during `start()` (already async) before entering the delivery loop.                                                                                                                                                 | **Resolved 2026-04-28** (Glad-Labs/poindexter#156) — `__init__` retains a SiteConfig reference and `start()` awaits `get_secret("openclaw_webhook_token")` once before spinning up the delivery loop.   |
| `src/cofounder_agent/services/telegram_config.py:34`          | `telegram_bot_token`     | `get_telegram_bot_token` (sync)          | Helper is plain `def`. Used to be called from `social_poster._notify` — this PR replaced that callsite with an inline `await site_config.get_secret("telegram_bot_token")` and dropped the import, so the helper no longer has any production callers. The helper is still publicly exported and would re-introduce the bug if any future code grabs it.                                   | Convert `get_telegram_bot_token` and `telegram_configured` to `async def` (they now both touch a secret), update the test suite (`test_telegram_config.py`) to await them. Or delete `get_telegram_bot_token` outright since its only consumer now reads the token directly.                                                                                                   | **Resolved 2026-04-28** (Glad-Labs/poindexter#156) — both helpers converted to `async def`, the dead imports in `task_executor.py` were dropped, and `test_telegram_config.py` now awaits both helpers. |
| `src/cofounder_agent/services/newsletter_service.py:34`       | `resend_api_key`         | `_cfg` (sync)                            | The newsletter config aggregator builds a flat `dict` of ~13 keys. Caller (`send_newsletter_for_post`) is async, so converting `_cfg` to `async def` is mechanical, but the same `_cfg` dict shape is consumed in three deeper sync helpers (`_send_via_resend`, `_send_via_smtp`, `_build_html`) — the change is well-contained but still touches more than the audit was scoped for.     | Convert `_cfg` to `async def`, replace `cfg["resend_api_key"]` reads with awaited fetches, and bump the test suite (`test_newsletter_service.py`) to `await _cfg(...)`.                                                                                                                                                                                                        | **Resolved 2026-04-28** (Glad-Labs/poindexter#156) — `_cfg` is now `async def`, only the encrypted `resend_api_key` is awaited via `get_secret`, and `send_post_newsletter` awaits `_cfg(site_config)`. |
| `src/cofounder_agent/services/newsletter_service.py:38`       | `smtp_password`          | `_cfg` (sync)                            | Same as above. **Note**: `smtp_password` is _not currently_ `is_secret=true` in `app_settings` — there's no such row at all in the live DB as of 2026-04-24. The site is still in the same shape because anyone who _adds_ the row encrypted would hit the same bug. The follow-up should both (a) flip the encrypted flag when the row gets seeded and (b) move the read to `get_secret`. | **Still open** as of 2026-04-28 — `smtp_password` row remains absent / not flagged `is_secret=true` in the live DB. When it's seeded encrypted, the `_cfg` line for `smtp_password` must move from `site_config.get(...)` to `await site_config.get_secret(...)`. The current code path in `_cfg` is documented inline so future operators see the constraint at the callsite. |

**Total deferred: 4 callsites in 3 files. 3 of 4 resolved 2026-04-28 (#156); `smtp_password` remains gated on a schema change.**

## Phase 5 webhook handler (Glad-Labs/poindexter#155)

In addition to the four sync-container deferrals above, two webhook-
verifier helpers in `routes/external_webhooks.py` were reading
`is_secret=true` rows via sync `site_config.get(...)` and feeding the
`enc:v1:<ciphertext>` blob straight into `hmac.new(...)`. Same failure
pattern: HMAC comparison with ciphertext-as-key never matches a real
provider-signed payload, so every legitimate webhook 401s.

| File:line                                             | Key                            | Containing function                             | Resolution                                                                                                                                                                  |
| ----------------------------------------------------- | ------------------------------ | ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/cofounder_agent/routes/external_webhooks.py:61`  | `lemon_squeezy_webhook_secret` | `_verify_lemon_squeezy_signature` (now `async`) | **Resolved 2026-04-28** (#155) — verifier converted to `async def`, awaits `site_config.get_secret("lemon_squeezy_webhook_secret", "")`. Route handler awaits the verifier. |
| `src/cofounder_agent/routes/external_webhooks.py:177` | `resend_webhook_secret`        | `_verify_resend_signature` (now `async`)        | **Resolved 2026-04-28** (#155) — same shape: verifier is `async def`, awaits `site_config.get_secret("resend_webhook_secret", "")`. Route handler awaits the verifier.      |

Both keys were not flagged `is_secret=true` at the time of the
original 2026-04-24 audit (they appear in the "Out-of-audit
observations" section below), but the live DB has since been updated —
both are encrypted as of 2026-04-27, which is why #155 was filed.
...
- `lemon_squeezy_webhook_secret` — `routes/external_webhooks.py:61` _(now encrypted in live DB; resolved 2026-04-28 in Glad-Labs/poindexter#155 — see Phase 5 webhook handler section above)_
- `resend_webhook_secret` — `routes/external_webhooks.py:177` _(now encrypted in live DB; resolved 2026-04-28 in Glad-Labs/poindexter#155 — see above)_
- `smtp_password` — `services/newsletter_service.py:38` _(still not encrypted as of 2026-04-28 — pending schema change before migration)_
- `eia_api_key` — `services/jobs/update_utility_rates.py:208` _(still not encrypted as of 2026-04-28)_
