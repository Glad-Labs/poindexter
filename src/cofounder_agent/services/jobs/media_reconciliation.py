...
class MediaReconciliationJob:
    ...
    async def run(self, pool: Any, config: dict[str, Any]) -> JobResult:
        ...
        #
        # Stage-2 demotion (Plan 8 / #689): if a post already has a
        # ``media_assets`` row for video (``video``, ``video_long``, or
        # ``video_short``), its video was generated by the Stage-2
        # ``media_pipeline`` graph and lives at a task-keyed path
        # (``{task_id}.mp4`` under ``~/.poindexter/video``), NOT at the
        # R2 URL the HEAD check probes (``{r2_base}/video/{post_id}.mp4``).
        # The R2 HEAD therefore returns 404 even though the asset exists.
        # Triggering ``_regen_video`` for such a post would re-generate
        # via the legacy ``video_service`` path, bypassing all Stage-2
        # quality gates — and produce a second competing asset on R2.
        # We demote the regen pass for those posts: skip ``_regen_video``
        # entirely; the advisory finding still surfaces the R2 gap so the
        # operator can decide whether a manual R2 upload is needed.
        def _in_regen_window(r: dict[str, Any]) -> bool:
            pub = r.get("published_at")
            return pub is not None and pub >= regen_cutoff

        def _has_stage2_video(r: dict[str, Any]) -> bool:
            """True if the post already has a media_assets row for any
            video type — meaning Stage-2 already generated it.  The
            reconciliation regen pass must not override Stage-2 assets."""
            pid = r["id"]
            return any(
                (pid, t) in existing_pairs
                for t in ("video", "video_long", "video_short")
            )

        regen_podcast = [r for r in missing_podcast if _in_regen_window(r)]
        # Demoted: skip video regen for posts that Stage-2 already generated.
        regen_video = [
            r for r in missing_video
            if _in_regen_window(r) and not _has_stage2_video(r)
        ]
        demoted_video = [
            r for r in missing_video
            if _in_regen_window(r) and _has_stage2_video(r)
        ]
        if demoted_video:
            logger.info(
                "media_reconciliation: skipped regen for %d Stage-2 video "
                "asset(s) — media_assets row exists, R2 HEAD is advisory only; "
                "post ids: %s",
                len(demoted_video),
                [r["id"] for r in demoted_video],
            )
