...
The repo is over 10,500 commits, about 700,000 lines of Python across 2,311 files, 18,000+ unit tests, 220 tagged releases. Nearly all of it was written by Claude Code sessions that I directed — specifying what to build, reading what came back, fixing what wasn't right, and deciding what came next. I've been embarrassed about this for most of the year. I'd say "I didn't actually code it" the way you'd confess to something. I'm writing this post partly because I've stopped believing that framing.
...
What I do with the result is the part I had wrong for a long time. I don't really reject it. The output is too large to review the way you'd review a PR — by the time you've read it closely enough to reject it honestly, you could have refined it twice. So I take what comes back and fix it — except "fix" is the wrong word for what I actually do. I don't write the fix. I find the gap and describe it to the agent precisely enough that it can close the gap itself. The seeing and the saying are the work; the typing hasn't been mine for a long time. I'm not an expert engineer and I'm not claiming to be one, and at this point it would be pretentious to think I write better code than a frontier model. I don't. What I've stopped doing is reading every line of every change.

Which is the actual answer to what I spend my judgment on. It is almost never code quality. It's architecture, planning and execution — the gap between what I asked for and what the system actually needed, the case nobody thought about, the decision that was locally reasonable and globally wrong. I correct misses in design far more often than errors in code. That distinction is also why the test gate below is non-negotiable and still not sufficient: tests catch the class of mistake I've stopped worrying about, and they cannot see the one I'm actually there for. Nothing in CI tells you that the thing you carefully built shouldn't exist.

So the skills that turned out to matter were not coding skills, exactly. They were: knowing what to build next, writing specifications that survive contact with a literal-minded reader, smelling when something is off before the tests catch it, and being willing to throw away work — the agent's and mine — without sunk-cost flinching.
...
I went looking for the single one that still stings and couldn't find it, which turns out to be the more uncomfortable answer. The failures that cost me aren't events. They're the steady accumulation of things that quietly stopped being true: a tap that ingests nothing, a config that drifted from what I believed was running, a number I'd been quoting for months that described a version of the system that no longer existed. Every one of them I found sideways, while looking for something else. Nothing crashes. Nothing pages. The system keeps reporting success, because the thing that broke isn't the thing being checked.

The other standing pain is GPU contention, which is the same shape in different clothes — two cards, the pipeline, a vision judge and a handful of sidecars all wanting VRAM in the same second. The process that loses doesn't crash either. It just fails to do the useful thing, and I find out later.

## What this cost

Running it costs about $69 a month, measured over the last 30 days: roughly $59 of electricity (54 kWh) and $10 of API spend. On top of that, about $240 a month in subscriptions: roughly $40 for Vercel and GitHub, and $200 for the Claude Max plan that does the actual building. That one I may drop to the $100 tier, for the obvious reason that this doesn't make money yet. Call the whole thing somewhere near $3,700 a year to run, hardware excluded. The hardware is one machine, running Pop!_OS in my house: a Ryzen 9 9950X3D, an RTX 5090 and a 3090, 64 GB of DDR5, two NVMe drives, a 1500 W supply and a custom water loop because the thing runs hot for hours at a time. About $10k of parts in May 2025, before RAM prices went stupid — the same build is worse value today, which is worth saying if you're pricing one.
