...
Poindexter titles posts on two independent paths. Both were producing
recognisably formulaic titles; this document covers the mechanism that steers
them apart, what it is measured against, and the one gap it does **not** close.
...
`qa_title_similarity_threshold` (0.6) reads like an internal-diversity gate.
It is not. `check_title_originality` runs `SequenceMatcher` against
**DuckDuckGo search results only** — it is an external-plagiarism check.
Nothing in the pipeline compares our own titles to each other with a
threshold.

Evidence from the live corpus: _"The Shift to Native Telemetry"_ and _"The
Shift to a Native UI"_ score **0.75** against each other, well over the 0.60
threshold, and both shipped. The avoidance block is soft prompt guidance, so
it reduces the odds of that pair without ever gating it.

Closing it means comparing the candidate against the recent window and routing
a real collision back through the regeneration loop — the loop already exists
(`content_generate_title.run`), it is just wired to the web rather than to us.
