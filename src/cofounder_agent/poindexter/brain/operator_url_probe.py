...
def _default_dashboards_dir() -> Path:
    """``infrastructure/grafana/dashboards`` on a host checkout, found by walking
    up from this file (poindexter/brain/ -> ... -> repo root). A fixed
    ``parents[N]`` is wrong somewhere by construction: the container image holds
    only ``/app/poindexter/brain``, so ``parents[4]`` raises IndexError there
    and took the daemon down at import (caught by the PR-B image smoke,
    2026-09-10). In the container the directory does not exist and the
    ``operator_url_probe_dashboards_dir`` setting is what the probe reads."""
    here = Path(__file__).resolve()
    for parent in here.parents:
        candidate = parent / "infrastructure" / "grafana" / "dashboards"
        if candidate.is_dir():
            return candidate
    return here.parents[-1] / "infrastructure" / "grafana" / "dashboards"


DEFAULT_DASHBOARDS_DIR = _default_dashboards_dir()
