...
### Key Numbers (code-derived stats auto-synced daily by CI; DB-derived counts — posts, embeddings, app_settings totals — hand-refreshed 2026-07-26 from the live DB)

> **Known gap:** the daily `claude-md-sync` session computes fresh DB counts and
> logs "opened CLAUDE.md sync PR", but no such PR exists — the DB-derived
> numbers below sat frozen at their 2026-06-10 values for six weeks until this
> manual refresh. The repo-derived stats (file/dashboard counts) sync fine via
> `.github/workflows/sync-claude-md.yml`; it's only the local DB half that is
> silently dropping its PR.

- 159 live posts on gladlabs.io (333 posts total; 1,974 pipeline_tasks across all generation runs)
- 379 Python files under `src/cofounder_agent/services/` (~291 substantive after `__init__.py` stubs; down from ~455 after the 2026-05-08+09+16 cleanup passes). The "load-bearing services" table below covers the services on a critical execution path — `flows/content_generation.py`, `worker_service.py`, and `auto_publish_gate.py` were missing from prior versions of the table.
- **Big deletions that explain missing files** (May 2026 cleanup wave; full detail in git): the entire workflow_executor chain — `workflow_executor.py`, `custom_workflows_service.py`, `template_execution_service.py`, `workflow_validator.py`, `phase_mapper.py`, `phase_registry.py`, `workflow_progress_service.py`, `phases/`, `schemas/custom_workflow_schemas.py`, `agents/` (~3,800 LOC, 2026-05-09); `services/task_executor.py` (~1,500 LOC, 2026-05-16 — replaced by `services/flows/content_generation.py` on Prefect, its `_notify_discord`/`_notify_alert` helpers moving to `services/integrations/operator_notify.py` and its auto-publish helpers to `modules/content/auto_publish.py`); and `plugins/stage_runner.py` + the legacy chunked `content_router_service` path.
- **Cutover gates are all `true` on prod** — Prefect is the dispatcher, `canonical_blog` is the pipeline, LiteLLM is the LLM router, LlamaIndex + Ragas + DeepEval + Guardrails are all on.
- **Atom-cutover (#355, 2026-06-02)**: `canonical_blog` cut over from the hand-coded LangGraph factory to a DB-stored `graph_def` (21 nodes at cutover — 13 `stage.*` + 5 `qa.*` + 3 `seo.*`; since grown to 44, see the Content-pipeline-stages section), compiled by `services/pipeline_architect.py::build_graph_from_spec`; `pipeline_use_graph_def=true` is the prod default. The `cross_model_qa` stage was **deleted** (`services/stages/cross_model_qa.py`, 733 LOC; ~1,220 with its tests) and replaced by composable QA atoms (5 at cutover, now 13) → `qa.aggregate`, in `modules/content/atoms/` (moved there in the 2026-06-04 content-module migration) that delegate to the retained `multi_model_qa.py` rail library. New `atom_runs` table + `services/atom_runs.py` capture per-atom run + outcome (gated by `atom_runs_capture_enabled`).
- **Migration files** — the **Phase G squash (2026-07-11)** folded the Phase F baseline (2026-06-22) plus its 42 post-baseline migrations into a fresh `0000_baseline.py` (+ `.schema.sql` + `.seeds.sql`), leaving a true baseline-only tree with no surviving convergence migration. Ordinary migrations have accumulated on top since — 15 as of 2026-07-26 (`20260712_084907_*` … `20260726_054959_*`). Baseline seed contents: non-secret `app_settings` 692 (+2 empty secret placeholders), `pipeline_templates` 6, `qa_gates` 19, `retention_policies` 31. New schema changes go in fresh `YYYYMMDD_HHMMSS_<slug>.py` files; the runner sorts `0000_baseline.py` first because `0` < `2` lexically, and orphan `schema_migrations` rows for squashed-away files are harmless (it skips by filename, never reconciles in reverse). Squash mechanics + verification detail live in the Database-migrations section below.
- 12 Grafana dashboards (named + described in the Monitoring section below; Revenue parked 2026-07-01 in `infrastructure/grafana/dashboards-parked/` until `revenue_events` has real writes; each board carries a one-line CHARTER in its description). Alerting is two-sourced: **19 static** rules across 4 active `infrastructure/prometheus/alerts/*.yml` files (deadmans-switch / infrastructure / observability-sidecars / prometheus-alertmanager — `postgres-connections.yml` is a 5th, comment-only placeholder since its rules went DB-rendered for app_settings-tunable thresholds), plus **20 DB-sourced defaults** across `poindexter-business` / `-content` / `-infra` / `-infrastructure` / `-postgres-connections`, rendered into `rules/*.yml` by `RenderPrometheusRulesJob` every 5 min. So the live count is 19 + whatever the DB currently seeds. Worker-gauge rules use a long `for:` with `last_over_time`/`max_over_time` to bridge deploy-restart scrape holes — see the restart-gap policy comment atop `DEFAULT_RULES` in `services/prometheus_rule_builder.py`. Pyroscope app-profiles ship from worker/brain/voice under `service_name` tags (poindexter#406).
- ~11,440 test functions across 934 test files under `tests/unit` (+ 16 integration + 17 integration_db) — last recorded full-green nightly 2026-06-09 (10,352 passed, 84 skipped, 1 xfailed, 0 failures, **0 collection errors**); the suite has grown ~1,200 test functions since, so re-run to refresh the pass tally. A handful still skip in-container due to the `Path(__file__).parents[5]` path-depth quirk and run on host; the prior "~21 collection errors" line described a since-fixed problem.
- 1,279 app_settings keys live on prod (68 secret; count drifts as keys are added) — the baseline seeds the non-secret defaults; secrets get configured per-operator via `poindexter setup` + bootstrap.toml. Three sources seed the table (`settings_defaults.py` 769, `0000_baseline.seeds.sql` 694 = 692 non-secret + 2 empty secret placeholders, `brain/seed_app_settings.json` 80 free-tier); the overlapping keys must agree — enforced by `scripts/ci/settings_seed_value_drift_lint.py`. (The 4 `cost_tier.{free,budget,standard,premium}.model` mappings were removed in PR #1907 — model selection is now per-step `*_model` pins. Cost-guard key rename 2026-05-27 closed a silent fallthrough on `daily_spend_limit_usd` / `monthly_spend_limit_usd` — see #598.)
...
- 5 declarative-data-plane tables (`external_taps` / `retention_policies` / `webhook_endpoints` / `publishing_adapters` / `qa_gates`) feeding the integrations handler registry's 14 handlers across 4 handler surfaces (`tap` ×4 / `retention` ×6 / `outbound` ×2 / `publishing` ×2). Note `webhook_endpoints` is a declarative *table* but has no handler surface of its own — its rows are consumed by `integrations/outbound_dispatcher.py` + `operator_notify.py`. A 6th `services.declarative_config_service`-managed table, `alert_rules` (`poindexter alerts`), joined 2026-07-13 (poindexter#848) — CRUD'd through the same generic service but synced by the bespoke `brain/alert_sync.py` loop rather than the integrations handler registry.
- 54,712 embeddings across posts / issues / audit / memory / brain / claude_sessions (retention prunes claude_sessions/brain vectors, so this count drifts)
...
| `multi_model_qa.py`                       | Adversarial review (different LLMs check each other) + DeepEval rails + native guardrails rails + Ragas rail (all advisory). DeepEval (#329 sub-issue 1): `deepeval_brand_fabrication`, `deepeval_g_eval`, `deepeval_faithfulness`. guardrails (#329 sub-issue 3; reimplemented **native / dep-free** in `services/guardrails_rails.py` by #996 (2026-06-03) after `guardrails-ai` was dropped for CVE-2026-45758): `guardrails_brand` (runs `content_validator` patterns), `guardrails_competitor` (regex over the `guardrails_competitor_list` CSV). Ragas (#329 sub-issue 2): `ragas_eval` averaging faithfulness + answer-relevancy + context-precision into one score. Six Lane-D QA rails total (DeepEval ×3, guardrails ×2 — now native, Ragas). All on in prod (`deepeval_enabled` / `guardrails_enabled` / `ragas_enabled` = `true`). **As of atom-cutover #355** the rails are invoked by 13 `qa.*` atoms (`modules/content/atoms/qa_*.py` → `qa.aggregate`) on the graph_def path — `multi_model_qa.py` is the rail library they delegate to (the monolithic `cross_model_qa` stage is deleted). Advisory-vs-hard-gate is DB-driven per rail via `qa_gates.<rail>.required_to_pass` (`_mark_advisory_if_configured`). |
| `rag_engine.py`                           | LlamaIndex `BaseRetriever` over the existing `embeddings` pgvector table (#329 sub-issue 4 — closes Lane D). `rag_engine_enabled=true` on prod (2026-05-16). Full three-mode stack: vector-only → `HybridRRFRetriever` (BM25 tsvector + pgvector + RRF fusion, `rag_hybrid_enabled=true` — pure SQL, on by default since it needs no extra dependency, glad-labs-stack#2133) → `CrossEncoderRerankRetriever` (sentence-transformers cross-encoder, `rag_rerank_enabled=true` on Matt's prod, but code-defaults `false` — it needs the optional `rerank` poetry extra, so a fresh install must opt in deliberately rather than get warn-spam from a missing dep). The `text_search` column on `embeddings` is `GENERATED ALWAYS AS (to_tsvector('simple', text_preview))` — auto-populated, 100% coverage, GIN-indexed (`idx_embeddings_text_search`). All three modes live as of 2026-06-15 on Matt's operator stack. See [`docs/architecture/rag-retrieval-stack.md`](docs/architecture/rag-retrieval-stack.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `prompt_manager.py`                       | UnifiedPromptManager — **SKILL.md packs authoritative** (poindexter#825, 2026-07-04): prompt edits land in `skills/*/*/SKILL.md` via PR; Langfuse is a read-only mirror kept current by `SyncPromptCatalogToLangfuseJob` (6-hourly). Legacy Langfuse-first override lookup sits behind `langfuse_prompt_overrides_enabled` (default `false`) — enabling it re-introduces the masking trap, so deliberate live experiments only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `cost_lookup.py`                          | LiteLLM-backed cost lookup (wraps `litellm.model_cost`). The `model_router.py` / `usage_tracker.py` / `model_constants.py` trio is **deleted** (Phase 2 cleanup, 2026-05-08). Model selection is now per-step `*_model` pins read at each call site (the `cost_tier.*` tier→model API + `resolve_tier_model` were removed in PR #1907); `cost_lookup` itself just does per-token pricing. See [`docs/architecture/cost-tier-routing.md`](docs/architecture/cost-tier-routing.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `llm_text.py`                             | Plain-text Ollama chat helper for atoms + writer_rag_modes. **2026-05-16**: rewritten to route through `dispatch_complete` when a pool is available — propagates provider-swappability to 5+ writer paths (`narrate_bundle`, `deterministic_compositor`, `pipeline_architect`, `two_pass._revise_node`, `story_spine`). Direct-httpx fallback retained for tests/bootstrap. `maybe_unwrap_json` defense at result boundary catches models that emit JSON envelopes unprompted.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `auto_publish_gate.py`                    | Auto-publish decision logic — imported by `stages/finalize_task.py`. **Two gates stacked**: (1) the global `auto_publish_threshold` (default `0` = disabled) in `modules/content/auto_publish.py::get_auto_publish_threshold`; (2) the per-niche edit-distance gate (`{niche}_auto_publish_threshold` / `_dry_run` / `_min_clean_runs` / `_max_edit_distance`) in this file. Gate 2 fires only when the niche has explicit opt-in keys AND `quality_score ≥ threshold` AND the trailing-N approves had `char_diff < max_edit_distance`. **2026-05-27 fix**: gate 2 now reads `{niche_slug}_auto_publish_*` (was hardcoded `dev_diary_*`, which leaked dev_diary's opt-in to glad-labs — caused the unauthorised auto-publish of "Claude Is Not Your Architect. Stop." on 2026-05-26). **2026-07-11 fix**: that niche-scoping starved the gate for six weeks — `niche_slug` was never a declared `PipelineState` channel, so LangGraph dropped it at ainvoke and every evaluation returned `disabled: "niche_slug missing"` (dev_diary never auto-published; Matt hand-approved daily). Fixed three ways: `niche_slug` declared in `PipelineState`, `evaluate()` resolves the task's own niche from `pipeline_tasks` when the caller passes None (fail-closed), and the trailing-history query is niche-ONLY (the `OR category` clause let glad-labs' technology rows pollute dev_diary's clean-run window once category defaulted to "technology" post-Phase-F). Helpers live in `modules/content/auto_publish.py` (ported from the deleted `task_executor.py` in the 2026-05-16 Stage 4 deletion).                                                                                                                                                                                                                                                                                                                       |
...
The graph*def is **44 nodes** — 11 `stage.*` + 14 `content.*` + 14 `qa.*` + 1 `qa.rewrite` + 1 `seo.*` + 2 `atoms.approval_gate` + 1 `social.generate_drafts` — run as a (now mostly-linear) chain with one bounded QA rescue cycle. (It was 21 at the #355 cutover; #362 decomposed coarse stages into fine-grained atoms, #363 added `draft_gate`; #730 removed dead `qa.guardrails`; #734 collapsed 3 `seo.*` atoms into `seo.generate_all_metadata` (saves ~2 min/post); #765 added `content.reconcile_citations` + advisory `qa.unlinked_attribution`; the v6 reseed (2026-06-19, #1712) added `stage.review_video_shot_list`, a director self-critique of the video shot list; the #765 follow-up added `content.llm_reconcile_citations` (grounded-LLM citation tail after the deterministic pass); the v7 reseed (2026-07-24) added `qa.title_coherence` (advisory title↔body honesty rail — part of the draft-grounded-titling fix), and this line was resynced to 44 to also fold in `content.inject_affiliate_links` that earlier revisions of it omitted.) **QA rescue cycle (deferred from #1668):** a `qa.rewrite` node + a conditional `qa_aggregate → qa_rewrite` branch edge (`"branch": true`) and a `qa_rewrite → qa_programmatic` loop edge (`"loop": true`, exempt from the compiler's DAG validation). A \_rescuable* reject — a soft critic veto, or a below-threshold score with no hard veto, never a fabrication/gate/`missing_required` veto (`_qa_rail_common.is_rescuable_reject`) — gets one revision pass before hard-reject. Bounded by the durable `qa_rewrite_attempts` counter against `app_settings.qa_rewrite_max_attempts` (default 1). Authoritative order is `services/canonical_blog_spec.py::CANONICAL_BLOG_GRAPH_DEF`:

1. `verify_task` → **writer block** 2. `content.generate_draft` → 3. `content.generate_title` (draft-grounded 2026-07-24: prompt reads an article digest — `title_content_excerpt_chars` excerpt + section-heading skeleton — with the topic demoted to assignment-label context; primary keyword = tags[0] → content-derived → topic) → 4. `content.check_title_originality` → 5. `content.normalize_draft` → 6. `draft_gate` (`atoms.approval_gate`, seeded disabled) → 7. `writer_self_review` → 8. `resolve_internal_link_placeholders` → 9. `content.reconcile_citations` (deterministic citation repair — re-links named sources the writer dropped, #765) → 10. `content.llm_reconcile_citations` (grounded-LLM citation tail — links residual attributions the regex grammar missed and flags ungrounded named sources → advisory `citation_grounding` review + `unlinked_named_sources` finding; LLM proposes, code verifies url∈corpus + text-verbatim before applying, #765 follow-up) → 11. `content.inject_affiliate_links` (curated affiliate links, keyword-matched, dark-launched behind `affiliate_injection_enabled`) → 12. `quality_evaluation` (pattern-based scorer) → 13. `url_validation` → **image block** 14. `content.plan_image_markers` (extracts the writer's `[HERO-IMAGE:]` subject + numbers the writer's inline `[IMAGE:]` markers up to `writer_max_inline_images`; when the writer placed none, falls back to the body-fed Image Decision Agent — the node-2/5 leaked-prompt strippers now preserve these intentional marker forms and only strip `[FIGURE:]`/italic-scene prose; image-direction-grounding 2026-07-10) → 15. `content.generate_images` → 16. `content.inject_images` → 17. `source_featured_image` (featured image grounded on the writer's hero subject via `_resolve_featured_subject`, else the decision-agent hero pick, else topic; art style rotates least-recently-used across published posts via `_select_style_lru`; local prompt-builder pinned to `gemma-4-31B-it-qat`) → 18. `caption_images` (vision re-caption) → **QA-rail block (19–32)** `qa.programmatic` → `qa.critic` → `qa.deepeval` → `qa.ragas` → `qa.vision` → `qa.topic_delivery` → `qa.citations` → `qa.unlinked_attribution` (advisory — flags residual unlinked sources post-repair, #765) → `qa.consistency` → `qa.self_consistency` (advisory, #1447) → `qa.content_originality` (advisory RAG self-echo net — flags a whole-post near-duplicate, chunked max-over-chunks; renamed from `qa.opening_originality` + broadened 2026-07-12) → `qa.title_coherence` (advisory title↔body honesty check — LLM verdict that the display title represents the article; catches wrong-domain titles, directive leaks, generic mush; local-model judge, never the cloud writer pin; 2026-07-24) → `qa.web_factcheck` → `qa.aggregate` → **SEO block (33)** `seo.generate_all_metadata` (single structured call, #734; draft-grounded + topic-echo guard 2026-07-24 — seo_title must never be the raw topic/directive) → 34. `generate_media_scripts` → 35. `generate_video_shot_list` → 36. `review_video_shot_list` (director self-critique — revises the shot list before Gate 1 on the writer-grade `video_director_model`; video-quality Piece 1, #1712) → 37. `capture_training_data` → **finalize block (38–43)** `content.compile_meta` → `content.persist_task` → `social.generate_drafts` (Postiz draft rows, one per platform per post) → `content.record_pipeline_version` → `preview_gate` (`atoms.approval_gate`, seeded disabled) → `content.evaluate_auto_publish` (awaiting_approval or auto-publish if score ≥ threshold). (`qa.rewrite` — the bounded rescue-cycle node — sits off this linear chain on the `qa_aggregate → qa_rewrite → qa_programmatic` branch/loop; see above.)

**The `cross_model_qa` stage is deleted** (`services/stages/cross_model_qa.py`, 733 LOC; ~1,220 with its two test files). QA now runs as 13 composable `qa.*` atoms in `modules/content/atoms/` (`qa.programmatic` / `qa.critic` / `qa.deepeval` / `qa.ragas` / `qa.vision` / `qa.topic_delivery` / `qa.citations` / `qa.unlinked_attribution` / `qa.consistency` / `qa.self_consistency` / `qa.content_originality` / `qa.title_coherence` / `qa.web_factcheck`) that delegate to the existing `MultiModelQA` rail methods and append to the `qa_rail_reviews` PipelineState channel; `qa.aggregate` combines them into the gate decision — on reject it does the DB writes the old stage did (via `modules/content/atoms/_qa_persist.py`) and halts the graph, on approve it promotes `quality_score` and populates `qa_reviews`. `multi_model_qa.py` stays — it's the rail library the atoms delegate to. Rail advisory is DB-driven: each rail reads `qa_gates.<rail>.required_to_pass` (via `MultiModelQA._mark_advisory_if_configured`), so graduating a rail (poindexter#454) is still `qa_gates.<rail>.required_to_pass=true`, now effective on the graph_def path. (The legacy `cross_model_qa` rewrite loop was **not** ported — `qa.aggregate` halts on reject rather than auto-rewriting.)
...
- **Grafana (self-hosted):** http://localhost:3000 (or http://100.81.93.12:3000 from the tailnet) — 12 dashboards (Revenue parked 2026-07-01). Grafana Cloud was retired 2026-05-03; the local Docker container (poindexter-grafana) is the only Grafana now. Local Prometheus scrapes node_exporter (host, `:9100`) + the nvidia-smi gpu-exporter container directly (post-Linux-migration; windows_exporter retired with the Windows host); Alloy was the Cloud shipper and is no longer used. **The operator console (`/console/`) serves the deep-telemetry surfaces natively under a Telemetry tab** — Loki logs (worker proxy `GET /api/logs`), Langfuse LLM traces (`GET /api/traces`, waterfall deeplinks out to Langfuse), and native HISTORY / GPU & POWER / DATABASE chart sections (browser-direct Prometheus `query_range` against `:9091`, CORS-allowed) — so day-to-day operation no longer requires opening Grafana directly. The former Grafana `/d-solo` iframe embeds were removed 2026-07 ("Killing the Iframe"); `GF_SECURITY_ALLOW_EMBEDDING=true` remains in the compose `grafana` service but the console no longer depends on it.
- **Dashboards:**
...
accumulated normally since (15 as of 2026-07-26) — "baseline-only" describes
