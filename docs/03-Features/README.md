# 03 Features

Use this section for user-facing and platform-facing feature documentation with concrete examples.

## Planned Pages

- `Content-Generation-Pipeline.md`
- `Workflows-System.md`
- `Capability-Based-Tasks.md`
- `Analytics.md`
- `Model-Selection.md`
- `WebSocket-Real-Time.md`
- `Custom-Workflows.md`
- `OAuth-Integration.md`
- `Service-Registry.md`
- `Components/`

## Current Source Documents

- `docs/03-Features/Capability-Based-Tasks.md`
- `docs/03-Features/Analytics.md`
- `docs/03-Features/Workflows-System.md`
- `docs/03-Features/WebSocket-Real-Time.md`
- `docs/03-Features/Custom-Workflows.md`
- `docs/03-Features/OAuth-Integration.md`
- `docs/03-Features/Service-Registry.md`
- `docs/03-Features/Model-Selection.md`
- `docs/02-Architecture/Multi-Agent-Pipeline.md`
- `docs/components/`
