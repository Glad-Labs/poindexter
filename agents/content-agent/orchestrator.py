...
class Orchestrator:
    ...
    def run_single_job(self, sheet_row_index: int):
        """
        Processes a single content task based on its row index in the Google Sheet.
        This is designed to be triggered by the Pub/Sub listener.
        """
        logging.info(f"Orchestrator: Received request to process job for row {sheet_row_index}.")
    ...
    def run_job(self):
        """
        The main loop that fetches all 'Ready' tasks and processes them.
        """
    ...
    def _process_post(self, post: BlogPost, published_posts_map: dict):
        """The core logic for processing a single blog post."""
