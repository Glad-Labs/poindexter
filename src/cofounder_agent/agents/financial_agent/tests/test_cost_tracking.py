...
def cost_tracking_service():
    ...
    return CostTrackingService(
        cofounder_api_url="http://localhost:8000", pubsub_client=None, enable_notifications=False
    )
...
def mock_cost_metrics():
    ...
    return {
        ...
        "summary": {"total_estimated_savings_usd": 28.01, "optimization_enabled": True},
    }
...
class TestCostTrackingInitialization:
    ...
    def test_factory_initialization(self):
        ...
        service = initialize_cost_tracking(
            cofounder_api_url="http://localhost:8000", enable_notifications=False
        )
...
class TestBudgetThresholds:
    ...
    def test_no_alert_under_75_percent(self, cost_tracking_service):
        ...
        alert = cost_tracking_service._check_budget_thresholds(
            spent=50.0, budget=100.0, percentage=50.0
        )
    ...
    def test_critical_alert_over_budget(self, cost_tracking_service):
        ...
        alert = cost_tracking_service._check_budget_thresholds(
            spent=110.0, budget=100.0, percentage=110.0
        )
    ...
    def test_no_duplicate_alert_same_level(self, cost_tracking_service):
        ...
        alert2 = cost_tracking_service._check_budget_thresholds(
            spent=80.0, budget=100.0, percentage=80.0
        )
    ...
    def test_alert_history_tracking(self, cost_tracking_service):
        ...
        cost_tracking_service._check_budget_thresholds(spent=75.0, budget=100.0, percentage=75.0)
...
class TestProjections:
    ...
    def test_projection_midmonth(self, cost_tracking_service):
        ...
        with patch("agents.financial_agent.cost_tracking.datetime") as mock_datetime:
            ...
            projections = cost_tracking_service._calculate_projections(current_spent=50.0)
    ...
    def test_projection_early_month(self, cost_tracking_service):
        ...
        with patch("agents.financial_agent.cost_tracking.datetime") as mock_datetime:
            ...
            projections = cost_tracking_service._calculate_projections(current_spent=10.0)
    ...
    def test_projection_first_day(self, cost_tracking_service):
        ...
        with patch("agents.financial_agent.cost_tracking.datetime") as mock_datetime:
            ...
            projections = cost_tracking_service._calculate_projections(current_spent=5.0)
...
class TestRecommendations:
    ...
    def test_recommendations_info_level(self, cost_tracking_service, mock_cost_metrics):
        ...
        recommendations = cost_tracking_service._generate_recommendations(
            metrics=mock_cost_metrics, budget_percentage=50.0, alert_level=BudgetAlertLevel.INFO
        )
        ...
        assert any(
            "optimization" in rec.lower() or "cache" in rec.lower() for rec in recommendations
        )
    ...
    def test_recommendations_warning_level(self, cost_tracking_service, mock_cost_metrics):
        ...
        recommendations = cost_tracking_service._generate_recommendations(
            metrics=mock_cost_metrics, budget_percentage=75.0, alert_level=BudgetAlertLevel.WARNING
        )
    ...
    def test_recommendations_urgent_level(self, cost_tracking_service, mock_cost_metrics):
        ...
        recommendations = cost_tracking_service._generate_recommendations(
            metrics=mock_cost_metrics, budget_percentage=90.0, alert_level=BudgetAlertLevel.URGENT
        )

        # Should include priority/critical task guidance
        assert any(
            "prioritize" in rec.lower() or "critical" in rec.lower() for rec in recommendations
        )
    ...
    def test_recommendations_critical_level(self, cost_tracking_service, mock_cost_metrics):
        ...
        assert any(
            "disable" in rec.lower() or "immediate" in rec.lower() for rec in recommendations
        )
    ...
    def test_recommendations_with_projections(self, cost_tracking_service, mock_cost_metrics):
        ...
        with patch("agents.financial_agent.cost_tracking.datetime") as mock_datetime:
            ...
            recommendations = cost_tracking_service._generate_recommendations(
                metrics=mock_cost_metrics, budget_percentage=60.0, alert_level=BudgetAlertLevel.INFO
            )

            # Should include projection warning (50/10*30 = $150 projected)
            assert any(
                "projected" in rec.lower() or "month end" in rec.lower() for rec in recommendations
            )
...
class TestPubSubAlerts:
    ...
    async def test_publish_alert_success(self, mock_pubsub_client):
        ...
        service = CostTrackingService(pubsub_client=mock_pubsub_client, enable_notifications=True)
    ...
    async def test_publish_alert_disabled(self, mock_pubsub_client):
        ...
        service = CostTrackingService(pubsub_client=mock_pubsub_client, enable_notifications=False)
    ...
    async def test_publish_alert_no_client(self):
        ...
        service = CostTrackingService(pubsub_client=None, enable_notifications=True)
