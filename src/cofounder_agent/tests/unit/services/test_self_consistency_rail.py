...
from poindexter.services.self_consistency_rail import evaluate, is_enabled
...
class TestEvaluateSampleFailures:
    ...
    async def test_zero_samples_returned_skips(self):
        with patch(
            "poindexter.services.self_consistency_rail._sample_summaries",
            return_value=[],
        ):
    ...
    async def test_embedding_failure_skips(self):
        with patch(
            ...
            "poindexter.services.self_consistency_rail._pairwise_mean_cosine",
            return_value=-1.0,
        ):
            passed, score, reason = await evaluate(
                content="a body", topic="t",
