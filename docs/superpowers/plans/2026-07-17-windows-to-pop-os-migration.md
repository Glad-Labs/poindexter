...
> - **✅ PHASE 5 DONE (2026-07-23 — all four tasks, same-day; PRs #2749/#2750/#2751 merged).** Orchestration is re-homed onto systemd. **5.1 ops sessions:** `python3.13` (deadsnakes PPA — Pop 24.04 ships 3.12, backend demands ≥3.13) + pipx `poetry` (symlinked into `/usr/local/bin` so systemd units find it) + full backend venv installed; the committed template + timer generator installed with per-box seds (`User=mattm`, `/home/mattm/…`); **7 timers armed** on the operator-local schedule; `alert-triage` smoke ran green end-to-end (`alerts=0 filed=0`). Gotcha fixed: docker auto-created `~/.poindexter/logs` + `backups` root-owned → chowned to mattm. **5.2 node_exporter:** `prometheus-node-exporter` installed bound `0.0.0.0:9100`; scrape job `windows`→`node`; static `WindowsExporterDown`→`NodeExporterDown`; 4 DB-rendered rules renamed (disk ×2 → `node_filesystem_*` w/ `fstype=~"ext4|xfs|btrfs"` + `mountpoint` labels, memory → `node_memory_MemAvailable_bytes`, thrashing → `rate(node_vmstat_pswpout[5m])`); 51/51 tests; **verified live end-to-end** — target `up`, both filesystems in the series, and the worker's next render wrote `node_*` into `rules/dynamic.yml` (0 `windows_*` left). On-box: `prometheus.threshold.disk_min_total_gb` lowered to **100** (the 147 GB Linux root fell under the 200 default — the filter would have excluded the exact partition that filled up). ufw lines skipped (ufw inactive, consistent with the deferred 4.6 gate). Fast-follow still open: Grafana panels querying `windows_*`/`hwinfo_*`/`aida64_*` show NoData until re-sourced. **5.3 watchdog:** `poindexter-docker-watchdog.{service,timer}` installed, 5-min cadence, healthy-path no-op verified; **`voice-agent-livekit` profile-gated (`profiles: ["voice"]`)** because its broken docker-ce build aborted every plain `up -d` — including the watchdog's own recovery. Windows scaffolding untouched (rollback path until Phase 7). **5.4 DR backup:** `restic` 0.16.4 installed; the 117 GB USB stick fstab-mounted by UUID at `/mnt/dr-usb` (`ntfs3`, `nofail`); the 4 committed units installed (per-box seds only — the `DR_*` defaults already fit) with `RequiresMountsFor=/mnt/dr-usb`; **both timers fired green and read-back-verified** — daily snapshot `513fe1a6` (bootstrap.toml restores byte-correct, **379** `~/.claude` memory files, DB dump in-snapshot; same repo as the Windows-era `Nightrider` chain) and hourly `28b2a4d6` (pop-os pg-hourly). First run exited 3 on root-owned brain-written `infrastructure/prometheus/secrets/*` → **excluded** (#2751 — derived plaintext decryptions; the snapshot's encrypted DB dump + bootstrap.toml regenerate them; that failure fired one real Telegram DR alert ~12:49 EDT 2026-07-23 — smoke artifact, disregard). Windows DR pair was already disabled at Task 4.4. **Post-prune parity note:** the operator's disk-full `docker image prune` had removed the un-running `poindexter-backup` + `wan-server` images; a full-surface `up -d` (now safe with voice profiled) restored backup-tier ×3 / wan / speaches / github-runner — **41 containers, full Windows parity, zero unhealthy.** **Phase 6 Task 6.1 is DONE — see the next entry. NEXT = Task 6.2, the ≥14-day evaluation (clock started 2026-07-23).**
> - **✅ TASK 6.1 CUTOVER VERIFICATION — ALL 7 GATES PASS (2026-07-23 evening).** **S1** GPU-in-container: both GPUs enumerate by UUID (5090 + 3090). **S2** real pipeline end-to-end: the queue's pending task ran **unprompted** on the Linux stack — `1149dfc8` "The Console Transition" (canonical_blog) reached `awaiting_approval` with **quality_score 76**, 6.8k-char content, pipeline-generated hero image, QA feedback + SEO populated; **atom_runs shows 70 executions across 42 distinct atoms = the full graph_def, QA-rescue loop included**; 3 sibling tasks QA-rejected same afternoon (rails vetoing properly); GPU placement observed live on a concurrent run — vision `qwen3-vl` resident on the **3090** (`KEEP_ALIVE=-1`), embeddings + on-demand gemma on the **5090**; writer is cloud `anthropic/claude-sonnet-5` per the current pins (the plan's "writer on the 5090" phrasing predates the cloud-writer switch — the local-model placement is what the gate meaningfully checks). **S3** `postgresql-client-16` installed → **`pg_isready` 40/40** on `:5433` under churn (wedge-class test passes with the real tool); host CLI `poindexter tasks list` does a full OAuth round-trip (1,962 tasks; needs `POINDEXTER_API_URL=http://localhost:8002` in the env — fail-loud per #198, no default). **S4** Grafana `database: ok`; brain heartbeat alive (3 decisions/15 min); 7 session timers listed. Two firing alerts, **both legitimate operations, not migration breakage**: `MonthlySpendHigh` (July spend $44.51 vs the $30 limit — real, for operator review) and `FinanceMercuryPollStale` (gauge stale from the quiesce/handoff window; `poll_mercury` is scheduled and should self-clear — eval watch-item). **S5** `liquidctl` not installed (optional per plan; the single-rail OCP setting persists in the HX1500i's onboard memory per the 2026-07-18 note — install liquidctl during the eval for a live spot-check); GPU temps 33/30 °C after an afternoon of real load (image-gen render + resident vision model + full pipeline run). **S6** gladlabs.io → HTTP 200 in 0.9 s. **S7** `OWNER.txt` reads `linux`; Windows cannot auto-start the stack (14 tasks + the Startup stack-starter disabled at Task 4.4); Windows-boots evidence = the Phase-3 verification (re-confirm on the next natural Windows boot rather than rebooting the live business host now). **The ≥14-day Task 6.2 evaluation clock starts 2026-07-23.**
> - **✅ FULL FUNCTIONAL AUDIT — eval day 1 (2026-07-23 night): every execution surface swept, environment fully green.** Scheduler: 61 jobs registered, ~33 fired in the window, all green after fixes. Prefect: 128 completed / 4 crashed (all four inside the disk-full postgres outage window — transient). Brain: **`0 issues, 28 probes (0 failed)`** — first fully-green cycle since cutover. Alert plumbing verified live (fired + resolved + routed today: memory-thrashing during the containerd rsync, stale-task reclaim ×3 after the incident — the self-healing actually healed). **Found + fixed:** (1) **GA4/GSC singer taps** — `external_taps` command paths ran through the legacy `/root/.poindexter` mount; docker-ce enforces the 0700 `/root` traversal denial Docker Desktop ignored → host-persisted `singer-venv` bind at the doc-canonical appuser path, venvs rebuilt, rows repointed; **491 records, 0 failed** live. (2) **Offsite B2 backup tier was silently DOWN since cutover** — host python lacked `asyncpg`, start-stack's secret pre-decrypt wrote an empty env, backup-offsite idled "not configured"; `python3-asyncpg` + recreate → **"offsite verify OK"**. (3) **MCP HTTP server (:8004, claude.ai connector)** — was a Windows logon-task process; new `poindexter-mcp-http.service` (uv venv; `fastapi`/`uvicorn` were never declared deps — now in pyproject); healthz 200 from host + the brain's probe path. (4) **gpu_metrics writer** — `gpu-scraper.py` was launched by Windows `background-services.ps1`; new `poindexter-gpu-scraper.service` (host `python3-httpx` added); table fresh again, `gpu_temperature` probe green. (5) **Brain watchlist fossils** — `windows_exporter` probe → `node_exporter`; `openclaw` probe removed (gateway was a Windows host process, PowerShell restart path; re-add if OpenClaw is ported). (6) `corsair_csv` tap disabled (iCUE source permanently gone). Observability sinks all verified live: Loki ingesting, Langfuse web healthy (0 callback errors), GlitchTip serving brain triage (253 issues seen), Postiz/backup/db_backup jobs green. **OPERATOR ACTIONS still open:** (a) Mercury 401 since 07-20 (pre-migration) — add WAN IP `72.82.6.67` to the token's allowlist in the Mercury dashboard (or re-issue); (b) claude.ai connector — the old Windows node still holds the `nightrider` tailnet name and there is **no serve/funnel config on this box**: decide reclaim-name vs repoint the connector to `nightrider-1`, then `tailscale funnel` `/mcp → 127.0.0.1:8004` and re-establish the tailnet-only `/voice/join` serve; (c) July spend $44.51 vs the $30 limit.
> - **⚠️ CORRECTION to the `/data` relocation (2026-07-23 evening) — the docker `data-root` move alone was INSUFFICIENT on this engine.** This docker-ce runs the **containerd image store** (`docker info` → `Storage Driver: overlayfs / driver-type: io.containerd.snapshotter.v1`), so image **layers** live under **containerd's root (`/var/lib/containerd`)**, not under docker's `data-root` — which is why the earlier data-root rsync was only 13 GB (volumes + metadata) and why `/` refilled to 89 % (17 GB free) the moment the full-surface `up` re-pulled wan/speaches/backup images: **64 GB of layers had landed back on `/`.** Fix (same stop → rsync → repoint → verify → delete ceremony): stack + docker + containerd stopped, `rsync -aHAXS --numeric-ids --delete /var/lib/containerd/ /data/containerd/` (**re-run after full quiesce** — the first pass raced container-shutdown snapshot cleanup, rc=24 "files vanished"; the `--delete` re-run gave an exact copy, rc=0), `/etc/containerd/config.toml` gains `version = 2` + `root = "/data/containerd"`, daemons restarted, old `/var/lib/containerd` deleted. **⚠️ THE FIRST ATTEMPT OF THAT REPOINT SILENTLY FAILED AND CAUSED A ~15-MIN DEGRADATION (post-mortem, be honest about it):** the config kept the v1-form `disabled_plugins = ["cri"]` next to `version = 2` — in v2 that plugin name is invalid, containerd **fell back to its default config (root still `/var/lib/containerd`)**, and the "verify" step only grepped the config FILE + checked `docker info`'s _DockerRootDir_ (docker's root, not containerd's) — so the old store was deleted **while containerd was still serving from it**. Symptoms while the corpse was warm: running mains kept serving from open handles (worker API stayed 200 the whole time) but `docker exec` failed (`executable file not found in $PATH` — overlay lowers gone), healthchecks flipped unhealthy across the stack, dockerd logged `blob … expected at /var/lib/containerd/…: not found`, and restarting containers died `exit 128` (`wandered into deleted directory`). **Recovery (~15 min, zero data loss):** pause the watchdog timer (it would have `up -d`'d mid-surgery), write the config **correctly** (`version = 2`, `root = "/data/containerd"`, `disabled_plugins = ["io.containerd.grpc.v1.cri"]` — the fully-qualified v2 name), stop docker+containerd, start containerd and **verify the EFFECTIVE root via `containerd config dump | grep ^root`** (the check the first pass lacked), start docker (35 images, 612 blobs, 64 GB visible through the copy), `up -d --force-recreate` (old containers' rootfs pointed at deleted paths; volumes untouched by recreate) → **41/41 healthy in 15 s, exec healed, posts=329.** DB volumes live under docker's data-root (`/data/docker/volumes`), already safe throughout; DR snapshots + FINAL dump existed as backstops. **Lessons: (1) on a containerd-snapshotter engine, relocating storage means BOTH roots; (2) verify the EFFECTIVE root (`containerd config dump`), never the config file; (3) never delete the old store until the new one has served a real workload; (4) v2 configs need fully-qualified plugin names — a v1 name makes containerd silently ignore your entire config.** End state: `/` 61 GB free (57 %), `/data` ~162 GB used / 941 GB free — docker data-root AND containerd root both on `/data`, verified effective. Also fixed same evening: `backup-daily`/`backup-hourly` crash-looped because their bind source is the **subdirectory** `~/.poindexter/backups/auto`, which docker auto-created root-owned (the recurring missing-`${HOME}`-mount-source class, one level deeper than the parent dir already chowned) → chown/chmod, all three backup containers healthy, first daily dump written OK (313 MB).
...
- [x] **Step 1:** `docker run --rm --gpus all nvidia/cuda:12.6.0-base-ubuntu24.04 nvidia-smi -L` → both GPUs. ✅
- [x] **Step 2:** Kick a real content-pipeline task end-to-end; confirm the graph_def path with the writer on the 5090 and vision on the 3090 (`nvidia-smi` during the run). ✅
- [x] **Step 3:** `poindexter tasks list` from the host CLI + the `postgres` MCP both reach `localhost:5433` under churn — no wedge, no workaround. ✅

```bash
for i in $(seq 40); do pg_isready -h localhost -p 5433 -U poindexter; done
```

- [x] **Step 4:** Grafana renders, alerts + brain heartbeat green, the 7 session timers listed by `systemctl list-timers`. ✅
- [x] **Step 5:** PSU still single-rail (spot-check `sudo liquidctl status` if available); dual-GPU load test holds temps sane. ✅
- [x] **Step 6:** Public site unaffected (spot-check gladlabs.io). ✅
- [x] **Step 7:** Both OSes still boot; `OWNER.txt` reads `linux`; Windows cannot auto-start the stack. ✅
