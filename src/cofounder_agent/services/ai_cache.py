...
class AIResponseCache:
    ...
    async def get(self, prompt: str, model: str, params: Dict[str, Any]) -> Optional[str]:
        ...
        if cached:
            self.metrics["hits"] += 1
            logger.debug("Cache hit", cache_key=key[:12], source="redis")
            return cached
    ...
    async def set(self, prompt: str, model: str, params: Dict[str, Any], response: str):
        """
        ...
        logger.debug(
            "Response cached", cache_key=key[:12], model=model, ttl_hours=self.ttl_seconds / 3600
        )
...
class ImageCache:
    ...
    async def cache_image(
        self, topic: str, keywords: Optional[list[str]], image_data: Dict[str, Any]
    ) -> None:
        """
...
def initialize_ai_cache(
    redis_cache: Optional[RedisCache] = None, ttl_hours: int = 24
) -> AIResponseCache:
    """
