...
class NaturalLanguageRequest(BaseModel):
    ...
    context: Optional[Dict[str, Any]] = Field(
        None, description="Optional context (topic, keywords, audience, etc.)"
    )
    auto_quality_check: bool = Field(
        True, description="Automatically evaluate quality of generated content"
    )
...
class RefineContentRequest(BaseModel):
    ...
    feedback: str = Field(
        ..., min_length=10, max_length=1000, description="Feedback on what to improve"
    )
    focus_area: Optional[str] = Field(
        None, description="Specific area to focus on (clarity, accuracy, engagement, etc.)"
    )
