...
# Prefect's ephemeral-server fallback when ``PREFECT_API_URL`` is unset —
# a uvicorn subprocess on a random port whose ``GlobalEventLoopThread``
# never drains at pytest teardown, hanging the whole ``services
# (forked)`` step in CI for the full 30-minute workflow timeout. Setting
# these env vars at the top of the unit-tier conftest (before *any*
# services import) blocks the ephemeral fallback for every unit test.
# Tests that genuinely need a Prefect API still get one via the
# per-module ``prefect_test_harness`` fixture in
# ``services/flows/conftest.py``.
