...
class PhaseMapper:
    ...
    def map_phases(
        ...
        for target_key, target_input in target_def.input_schema.items():
            ...
            source_key = self._find_best_match(
                target_key,
                target_input,
                source_def.output_schema
            )
    ...
    def validate_mapping(
        self,
        source_phase: PhaseDefinition,
        target_phase: PhaseDefinition,
...
def build_full_phase_pipeline(
    ...
    user_mappings: Optional[Dict[str, Dict[str, str]]] = None
    ...
    for i in range(1, len(phase_names)):  # Skip first phase (index 0)
        ...
        phase_mapping = mapper.map_phases(
            source_phase_name,
            target_phase_name,
            user_overrides=phase_overrides
        )
