"""Compatibility stub -- `routes` now lives at `poindexter.routes` (Glad-Labs/poindexter#1046, step 2).

Importing this installs the flat-import alias finder and replaces this module in
``sys.modules`` with the canonical package, so ``import routes.x`` yields the SAME
object as ``import poindexter.routes.x`` -- one module, two names (see
...
# `cofounder_agent.routes` (the umbrella spelling the entry points use).
