...
class UnifiedQualityService:
    ...
    async def evaluate(
        ...
        try:
            ...
            return QualityAssessment(
                dimensions=QualityDimensions(
                    clarity=5.0, accuracy=5.0, completeness=5.0,
                    relevance=5.0, seo_quality=5.0, readability=5.0, engagement=5.0
                ),
                overall_score=5.0,
                passing=False,
    ...
    async def _evaluate_llm_based(
        ...
        if not self.model_router:
            logger.warning("LLM evaluation requested but model_router not available, falling back to pattern-based")
