...
- 94 live posts on gladlabs.io (262 posts total; 1,656 pipeline_tasks across all generation runs)
- 368 Python files under `src/cofounder_agent/services/` (~298 substantive after migrations + `__init__.py` stubs; down from ~455 after the 2026-05-08+09+16 cleanup passes). The "load-bearing services" table below covers the services on a critical execution path — `flows/content_generation.py`, `worker_service.py`, and `auto_publish_gate.py` were missing from prior versions of the table.
...
- 8,748 Python unit tests across ~516 test files (some skipped in container due to host/container path-depth quirks at `Path(__file__).parents[5]` — works on host; collection picks up ~21 errors as of 2026-05-27, tracked for cleanup)
- 911 app_settings keys (66 secret) plus 4 cost_tier mappings (`cost_tier.{free,budget,standard,premium}.model`) wired 2026-05-09 — the baseline seeds the non-secret defaults; secrets get configured per-operator via `poindexter setup` + bootstrap.toml. (Cost-guard key rename 2026-05-27 closed a silent fallthrough on `daily_spend_limit_usd` / `monthly_spend_limit_usd` — see #598.)
...
- 16,361 embeddings across posts / issues / audit / memory / brain / claude_sessions
