...
- **Per-probe spans for the brain daemon.** Each health probe now runs inside its own trace span carrying the probe name, duration, and pass/fail status. Slow or failing probes are now visible as individual segments in Tempo flame graphs instead of aggregating under one parent span, so you can pinpoint which probe is dragging down a cycle without parsing logs. ([#291](https://github.com/Glad-Labs/poindexter/pull/291))
