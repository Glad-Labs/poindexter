...
### One ladder, and the other direction (2026-08-25)

The #999 lesson generalized: a rung missing from "the ladder" is invisible precisely when it matters, and by 2026-08 there were about to be **two** hand-maintained lever lists (`_attempt_vram_reclaim` plus a new LLM-side caller). The lever list therefore moved to **`GPUScheduler.reclaim_render_vram(include_ollama=...)`** — ollama → image-gen(hard) → chatterbox → wan(hard) → stable-audio(hard) → comfyui(hard) — and every caller (`_attempt_vram_reclaim`, `prepare_mode('ollama'/'idle')`, the cold-load guard below) runs that one method. New GPU sidecars get their rung there, once.

The same day supplied the reverse-direction incident the ladder had never covered: the hourly topic harvest requested `structured_extraction_model` (`gemma-4-31B-it-qat:latest`, ~18 GB) from the primary Ollama, which cold-loaded it onto the render GPU while **ComfyUI idle-squatted 7.4 GB** between renders. The fit failed — `CUDA error: out of memory`, a llama-server segfault, a CPU-projector retry — and the OOM storm took down Chrome and Claude Desktop, whose GPU processes render on the same card. Media renders had always evicted Ollama first (`ollama_unload`); nothing evicted media before a big Ollama load.

**The cold-load guard** (`services/llm_providers/coldload_guard.py`) closes that: before `LiteLLMProvider.complete()`/`stream()` dispatches to a local Ollama-prefixed model, it checks `/api/ps` (resident → no-op, the steady-state cost is one ~2 ms local GET) and `/api/tags` (below `coldload_reclaim_min_gb`, default 8 → no-op), and only for a big non-resident model runs `reclaim_render_vram(include_ollama=False)`. Every rung declines mid-render and no-ops when its sidecar is absent, so the guard is safe on every install; it never raises and never blocks the LLM call. Tunables live on the `plugin.llm_provider.litellm` config row: `coldload_reclaim_enabled` / `coldload_reclaim_min_gb`.

**Only for loads that land on the render GPU (2026-09-25).** The guard first shipped on the belief that the GPU-1-pinned judge instance (:11435, `KEEP_ALIVE=-1`) is always warm and so never fires it. It is not always warm. The judge reloads several times a day, and each cold call (qwen3-vl, 19.6 GB) ran the full ladder against GPU 0. That included the shot-QA score pass inside video renders. RIFE and chatterbox unloaded, ComfyUI got a hard `/free`, and wan / image-gen / stable-audio were queued for restarts. At 14:58Z one such pass, during an image fan-out, got ComfyUI (holding 19.8 GB) restarted by the brain's squat rule, and all of it cleared a card the judge never touches. The guard now asks where the instance the call resolves to loads its models: `gpu_scheduler.ollama_host_devices(api_base)` maps `ollama_base_url` → `llm_primary` and `ollama_vision_base_url` → `qa_judge` and reads their cards from `gpu_lock_scopes`, the same hardware claim the device-scoped GPU lock serialises on. `render_devices()` is `gpu_lock_scopes["render"]` ∪ `pipeline_gpu_index`. The ladder is skipped only when both sets are known and disjoint; the skip is logged with both card lists. Anything unproven runs the ladder as before: an unrecognised host, a role missing from the map, a malformed map, or `gpu_lock_per_device_enabled=false` (the shipped two-card map describes the operator box until an operator turns scoping on). Placement is keyed on the instance URL, never on model names, so moving a model back to :11434 re-arms the ladder with no config change. Unpinning the judge is the same one edit the lock already requires: widen `qa_judge` in `gpu_lock_scopes`.
