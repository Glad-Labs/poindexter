...
Unlike other cloud providers, **Railway's `DATABASE_URL` is already the internal connection** with **no egress fees**. ✅
...
│              │   user:pass@         │
│ FREE ✅      │   postgres:5432/db   │
│              │                      │
│ (No setup    │   (Internal IP)      │
│  needed!)    │                      │
└──────────────┴──────────────────────┘
```
...
- **DATABASE_URL on Railway:** $0 ✅ (internal network)
...
4. Go to Strapi service → Variables
...
- ✅ Configurable connection timeout (60s default)
