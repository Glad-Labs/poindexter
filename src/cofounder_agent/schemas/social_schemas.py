...
class SocialPlatformConnection(BaseModel):
    ...
    platform: SocialPlatformEnum = Field(..., description="Social media platform to connect")

    class Config:
        json_schema_extra = {"example": {"platform": "twitter"}}
...
class SocialPost(BaseModel):
    ...
    tone: ToneEnum = Field(default=ToneEnum.PROFESSIONAL, description="Content tone style")
    include_hashtags: bool = Field(default=True, description="Whether to include hashtags in post")
    include_emojis: bool = Field(default=True, description="Whether to include emojis in post")


class SocialAnalytics(BaseModel):
    """Model for social media analytics"""

    post_id: str = Field(..., min_length=1, max_length=255, description="Unique post identifier")
    platform: SocialPlatformEnum = Field(..., description="Platform the post was published to")
    ...
    engagement_rate: float = Field(
        default=0.0, ge=0.0, le=100.0, description="Engagement rate as percentage"
    )
...
class GenerateContentRequest(BaseModel):
    ...
    platform: SocialPlatformEnum = Field(..., description="Target social media platform")
    tone: ToneEnum = Field(default=ToneEnum.PROFESSIONAL, description="Content tone style")
