...
- **Voice agent for hands-free pipeline control.** A new voice service lets you run pipeline operations by talking to Poindexter — propose topics, approve/reject drafts, schedule publishes, and pull status without touching the keyboard. Built on Pipecat with Whisper for speech-to-text, a local LLM for intent, and Kokoro for natural-sounding replies. Voice activity detection means no push-to-talk; just start talking.
- **Human-in-the-loop approval gates.** Three configurable checkpoints now sit inside the pipeline — topic discovery, post review, and final publish — each with its own approve/reject/edit handlers. Drive them from the CLI (`poindexter approval list/approve/reject`) or from Claude Code via MCP. Configure which gates are required, time-out behavior, and per-gate rejection actions in `app_settings`.
- **Pipeline dry-run mode.** Flip `pipeline_dry_run_mode` to walk a task through the full pipeline without spending money on cloud LLMs, calling external APIs, or publishing anything. Useful for testing prompt changes, new RAG modes, or freshly tuned QA gates against real content. Halts in dry-run now log at `info` instead of polluting the error feed.
- **Scheduled publishing in batches.** Posts queued with `publish_at` are now released through a configurable interval queue instead of all firing at once. Set the batch size and spacing in `app_settings` to keep social adapters from rate-limiting and to spread RSS/sitemap pings cleanly across the hour.
- **Video pipeline + YouTube publishing.** Posts can now ship as long-form (16:9) and short-form (1080×1920) videos in addition to text. The pipeline scripts each post into scenes, generates per-scene visuals (Wan 2.1 motion or Ken Burns on stills), narrates with the configured TTS provider, burns Whisper-aligned captions, stitches with FFmpeg, and uploads to YouTube via a new publish adapter. Bookend intro/outro audio and visuals are handled separately so they don't clash with body content.
- **A/B experiment harness.** Declare an experiment, route tasks across variants, record outcomes, and conclude — all from the CLI. Lets you compare prompts, RAG modes, models, or QA gate settings against a real publish stream instead of synthetic benchmarks.
- **`poindexter migrate` CLI commands.** Run `poindexter migrate status`, `poindexter migrate up`, and `poindexter migrate down` to inspect and drive schema migrations from the worker container without dropping into `psql`. Pairs with the new migration drift in `/api/health`.
...
- **License change: AGPLv3 → Apache-2.0.** Poindexter is now Apache-2.0 licensed across the board, and the previous documentation paywall is gone — every operator guide, architecture doc, and API reference is now in the public repo. Forks, embeddings inside other products, and commercial use no longer require copyleft compliance. See [LICENSE](LICENSE).
- **Hybrid retrieval upgrade for the writer.** Research and citation lookups now run through a LlamaIndex layer over the existing pgvector store, combine BM25 and dense vector results via reciprocal rank fusion, and rerank the top hits with a cross-encoder before they hit the writer prompt. Drafts pull from a tighter, more relevant slice of your corpus, which translates into fewer fabricated citations and stronger grounding. The reranker is warmed up at boot so the first task of the day doesn't pay the cold-start cost.
- **More automated content reviewers.** Multi-model QA now runs Guardrails-AI, DeepEval, and Ragas (RAG-eval + self-consistency) as parallel reviewers alongside the existing critic models. Each reviewer's verdict shows up in the audit log; failing reviewers above a configurable error-rate threshold no longer block the pipeline silently — the post auto-rejects with a clear reason.
- **Better article extraction.** The external-article scraper now uses trafilatura instead of BeautifulSoup heuristics. Cleaner article text means better citations, fewer navigation/footer fragments leaking into research bundles, and noticeably higher topic-coherence scores on the resulting drafts.
- **Pipeline orchestration on Prefect 3.** The content pipeline is now driven by Prefect 3, with a UI link bar wired into the Mission Control Grafana dashboard. Operators get per-stage retry/timeout policies, a flow-run history with logs, and the ability to re-trigger a single stage without re-running the whole pipeline.
- **Self-hosted Langfuse for prompts + traces.** A Langfuse stack now ships with the docker-compose ecosystem and serves as the source of truth for prompts (with YAML/DB fallback) and end-to-end LLM traces. The Mission Control dashboard links to the Langfuse UI for one-click drill-down on any LLM call.
- **LiteLLM for unified cost lookup.** Per-token cost lookups across OpenAI, Anthropic, Gemini, and OpenAI-compatible providers now route through LiteLLM, so cost dashboards, the budget cap, and the per-task cost log all use the same authoritative pricing table. New providers don't need a hand-maintained cost map.
- **Unified cost + electricity tracking.** Local Ollama runs now record kWh alongside dollars across all four LLM providers, so the cost dashboard shows real total spend (cloud cost + electricity at your configured rate) instead of just cloud cost. New `electricity_kwh` fields are exposed in `get_budget_status` and on the per-phase, per-model, and history breakdown endpoints, and Grafana ships a comparison panel of cloud vs Ollama spend.
- **Configurable brand keywords for topic discovery.** Brand-name filtering in topic discovery is no longer hardcoded — set `topic_discovery_brand_keywords` in `app_settings` to control which terms qualify a candidate as on-brand. Existing installs see no change until they opt in.
- **OpenTelemetry traces wired to Tempo end-to-end.** The brain daemon, the worker (including Ollama dispatch), and LLM provider calls now emit OTel spans into a Tempo backend with a Grafana dashboard. Trace any task from inbound API request through model dispatch, QA, and publish in a single flame graph.
- **Embedding store retention.** New retention/orphan-cleanup jobs prune stale embedding rows on a schedule, with a rollup tier that exempts summary embeddings from TTL and lowers collapse threshold to 14 days. Keeps pgvector growth bounded on long-running installs.
- **Resilience layer for all LLM providers.** The retry + queue + circuit-breaker logic that previously wrapped only Ollama now applies to every configured LLM provider (Anthropic, Gemini, OpenAI-compatible). A flapping cloud endpoint trips the breaker and falls back instead of stalling the pipeline.
- **Cooperative GPU sidecar unloading.** Image, video, audio, and TTS sidecars now coordinate over a shared protocol so only one heavy model holds the GPU at a time. Eliminates the OOM-driven workaround from earlier releases.
- **Provider plugin matrix.** New first-class plugins for image (FLUX.1-schnell), video (Wan 2.1 T2V 1.3B), audio (Stable Audio Open 1.0), TTS (Kokoro-82M), captions (Whisper local), and LLM (Anthropic, Gemini, OpenAI-compat) ship out of the box and follow the same `Protocol`-based contract. Swap any of them by changing a single setting.
- **Mission Control dashboard consolidation.** The 14 Grafana dashboards collapse to 6 — one Mission Control overview plus five grouped dashboards (Pipeline, Cost, Models, Infra, Logs). Drilldown links across dashboards are fixed and chart variety is improved, so day-to-day ops mostly happen on a single page.
...
- **Writer no longer serves template fallback content silently.** If every model in the chain failed during draft generation, the worker used to fall back to a stub template and quietly publish it. The pipeline now fails loud — the task is rejected with a clear audit-log entry instead of producing a placeholder post.
- **Publish pipeline no longer crashes on UUID columns.** Two latent SQL bugs (a bogus `::uuid` cast in `_finalize_publish` and an `UndefinedColumnError` in the publish service) caused intermittent publish failures on fresh installs. Both are fixed; the publish step also now correctly fires a Next.js ISR revalidation so just-published posts appear immediately on the public site.
- **Stall detector measures real queue dwell time.** The worker stall detector previously tripped on wall-clock alone, which produced false positives whenever the worker was idle. It now measures pending-queue dwell time, so the alert only fires when tasks are actually stuck.
- **Validator stops flagging programming-language names as hallucinated libraries.** The hallucinated-library detector incorrectly flagged words like "Python" or "Rust" as fabricated package names, blocking legitimate technical posts. Language names are now allow-listed.
- **Citation detector catches more fabricated paper names.** Improved heuristics for detecting fake academic citations in research-heavy drafts. Fewer made-up paper titles slip through QA.
- **Topic-coherence gate on internal-link recommender.** The internal-link recommender no longer suggests unrelated posts just because they share a keyword; suggestions now have to clear a topic-coherence threshold against the source post.
- **API token rotation + dev-token bypass disabled.** The default API token now rotates on first boot, and the historical "dev token" bypass path is removed. New installs are secure by default without an extra setup step.
- **Voice agent stability fixes.** Several first-session bugs are resolved: the agent now finds the brain DB from any working directory, accepts either name or value for the Whisper model setting, defaults the Ollama URL correctly when running on the host, wires VAD via the Pipecat 1.1 aggregator API, and reads its Kokoro voice from the Settings API.
- **Video pipeline polish.** Bookend (intro/outro) visuals now de-dupe against body URLs, the Wan sidecar pre-unloads before the pipeline runs to free GPU for Ollama, the HTTP timeout to the Wan server is bumped to 900s for longer scenes, and the still-image loop + EdgeTTS duration sync issues caught during the V0 sample are fixed.
- **Author page on the public site.** The "Coming soon" placeholder author page is replaced with a real post list, real career story, education, photo, and verifiable links.
