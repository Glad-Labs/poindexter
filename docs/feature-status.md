...
| RSS Feed             | WORKING            | Blog feed.xml + JSON Feed 1.1. Feed dedup via `distributed_at` + cutoff date. Direct Bluesky + Mastodon adapters replaced dlvr.it for those networks (GH-36); dlvr.it is retained as the RSS bridge for X/Twitter because the X API is $100/mo. |
...
| X/Twitter Posting    | PARTIAL            | Generates post text (280 chars). X API is $100/mo — not paying that. dlvr.it's RSS bridge picks up feed.xml and cross-posts to X automatically.                                                                                                 |
