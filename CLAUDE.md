...
- **Numeric fidelity (the only arithmetic QA rail):** [`docs/architecture/numeric-fidelity.md`](docs/architecture/numeric-fidelity.md) — every other anti-hallucination layer is a judgement (regex shapes, or an LLM asked if the draft looks faithful); `qa.numeric_fidelity` checks that each number the draft presents as SOURCED FACT reconciles with a number in `research_context`, at the precision written or via a **recorded** pair-derivation. **Attribution is the scoring gate and it was measured, not guessed**: scoring every number flagged 33% of real prose, all false positives (hypotheticals, rhetorical targets, and self-claims that belong to `qa.self_claim`'s corpus) — digit-bearing sentences are mostly years, image URLs and figures of speech. Over 40 posts it scored 29 attributed claims, 27 reconciled, and caught a post whose HEADLINE statistic ("110,000 papers") appears nowhere in its research. Appends **no review at all** when it has nothing to judge (empty `research_context` = 42% of runs, thin corpus, no attributed claims, or its own crash) — a rail that scores 100 on absent evidence reads as coverage on the dashboard. Advisory-first via `qa_gates.numeric_fidelity`. Strict mode (`score_unattributed=True`) is built + tested for a future generated-fact-set producer but deliberately NOT wired to a `numeric_facts` channel, because an undeclared PipelineState channel is how the auto-publish gate starved for six weeks.
