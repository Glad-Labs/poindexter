...
class TestAuthUserProfile:
    ...
    def test_get_user_profile_without_token(self):
        ...
        assert "authenticated" in response.json().get("detail", "").lower() or \
               "authorization" in response.json().get("detail", "").lower()
...
class TestAuthTokenValidation:
    ...
    def test_token_validation_missing_bearer_prefix(self, valid_jwt_token):
        ...
        response = client.get(
            "/api/auth/me",
            headers={"Authorization": valid_jwt_token}  # No "Bearer " prefix
        )
        assert response.status_code == 401
    ...
    def test_token_validation_with_whitespace(self, valid_jwt_token):
        ...
        response = client.get(
            "/api/auth/me",
            headers={"Authorization": f"Bearer  {valid_jwt_token}"}  # Double space
        )
