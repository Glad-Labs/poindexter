...
| ------------------- | ----------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| `dependency-review` | deterministic           | daily 06:30 | merge green dependabot PRs after 6 h: patch bumps (any scope), dev-tooling minors and dev groups, and production minors / minor-patch groups — except a held list (`litellm`, `prefect`, `langgraph`, `torch*`, `ragas`, `deepeval`, `diffusers`, `transformers`, `sentence-transformers`, `llama-index*`, `langchain*`, `next`) whose minors wait for a live smoke because the unit suite cannot see what they change. Majors are never auto-merged, and neither is any **docker base-image** bump — held by the `docker` label dependabot.yml attaches, with the image-tag shape (`-cuda`, `-slim`, `-runtime`, …) as the fallback. That hold runs before every version rule: a pytorch tag carries a semver triple (`2.5.1-cuda12.4-cudnn9-runtime`), so the minor rule merged one twice (#3756, #3915) and each time the new base's PEP 668 Python broke every sidecar build. github-actions bumps are `owner/name` too and still auto-merge — the label is the discriminator, not the slash |
