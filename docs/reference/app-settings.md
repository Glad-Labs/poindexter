...
> **Auto-generated from live `app_settings` table on 2026-05-23.**  
> Every runtime-configurable knob in the Poindexter pipeline.
> 791 active rows across 60 categories. 59 stored encrypted via pgcrypto (`is_secret=true`); 1 additional values redacted as secret-shaped (defense-in-depth); 10 values redacted as operator-specific (Tailnet IPs, financial reality, etc.) so this file is safe to ship to the public OSS mirror.
...
- [gates](#gates) (10 keys)
- [general](#general) (284 keys)
- [gpu](#gpu) (1 key)
- [identity](#identity) (16 keys)
- [image](#image) (12 keys)
- [integration](#integration) (13 keys)
- [integrations](#integrations) (14 keys)
- [llm_routing](#llm-routing) (8 keys)
...
- [plugin_telemetry](#plugin-telemetry) (77 keys)
...
| `mcp_http_probe_enabled`                | `false`                                 |                | Master switch for brain/mcp_http_probe.py (poindexter#434).                                                              |
...
| `writing_style_reference`                  | `*(per-operator)*`                         | per-operator   |                                                                                                                           |
...
| `discord_ops_webhook_url`                                            | `*(encrypted)*`                                                                | encrypted      | Discord ops channel webhook (rotated 2026-05-15 after upstream 404)                                                      |
...
| `grafana_api_token`                                                  | `*(encrypted)*`                                                                | encrypted      |                                                                                                                          |
...
| `idle_last_run_topic_discovery`                                      | `1778380220.2115617`                                                           |                |                                                                                                                          |
...
| `langfuse_public_key`                                                | `*(encrypted)*`                                                                | encrypted      |                                                                                                                          |
| `langfuse_secret_key`                                                | `*(encrypted)*`                                                                | encrypted      |                                                                                                                          |
...
| `owner_email`                                                        | `mattg@gladlabs.io`                                                            |                | Site owner email                                                                                                         |
...
| `task_executor_first_retry_writer_model` | `ollama/qwen3-coder:30b`                   |                | Writer model that services.task_executor swaps to on the first retry. Intent-based — picks a different model from the... |
...
| `operator_url_probe_target_overrides` | `{"google_sitemap_ping_url": {"alive_c...` |                | Per-URL probe behavior overrides for the operator-url probe. JSON map keyed by app*setting key (e.g. 'google_sitemap*... |
...
| Key                                                   | Default      | Classification | Description                                                                                                              |
| ----------------------------------------------------- | ------------ | -------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `findings_alert_route_watermark`                      | `54785`      |                | Highest audit_log.id forwarded to alert_events by FindingsAlertRouterJob. Pre-seeded 2026-05-15 to current max to ski... |
| `plugin_job_last_run_analyze_topic_gaps`              | `1779478703` |                | Unix epoch of last fire for plugin job 'analyze_topic_gaps' (auto-written by PluginScheduler)                            |
| `plugin_job_last_run_audit_published_quality`         | `1779547671` |                | Unix epoch of last fire for plugin job 'audit_published_quality' (auto-written by PluginScheduler)                       |
| `plugin_job_last_run_auto_embed_posts`                | `1779547674` |                | Unix epoch of last fire for plugin job 'auto_embed_posts' (auto-written by PluginScheduler)                              |
| `plugin_job_last_run_backfill_podcasts`               | `1779540474` |                | Unix epoch of last fire for plugin job 'backfill_podcasts' (auto-written by PluginScheduler)                             |
| `plugin_job_last_run_backfill_videos`                 | `1779547671` |                | Unix epoch of last fire for plugin job 'backfill_videos' (auto-written by PluginScheduler)                               |
| `plugin_job_last_run_check_memory_staleness`          | `1779547671` |                | Unix epoch of last fire for plugin job 'check_memory_staleness' (auto-written by PluginScheduler)                        |
| `plugin_job_last_run_check_published_links`           | `1779547675` |                | Unix epoch of last fire for plugin job 'check_published_links' (auto-written by PluginScheduler)                         |
| `plugin_job_last_run_crosspost_to_devto`              | `1779540471` |                | Unix epoch of last fire for plugin job 'crosspost_to_devto' (auto-written by PluginScheduler)                            |
| `plugin_job_last_run_db_backup`                       | `1779521903` |                | Unix epoch of last fire for plugin job 'db_backup' (auto-written by PluginScheduler)                                     |
| `plugin_job_last_run_detect_anomalies`                | `1779540473` |                | Unix epoch of last fire for plugin job 'detect_anomalies' (auto-written by PluginScheduler)                              |
| `plugin_job_last_run_detect_duplicate_posts`          | `1779478703` |                | Unix epoch of last fire for plugin job 'detect_duplicate_posts' (auto-written by PluginScheduler)                        |
| `plugin_job_last_run_expire_stale_approvals`          | `1779547671` |                | Unix epoch of last fire for plugin job 'expire_stale_approvals' (auto-written by PluginScheduler)                        |
| `plugin_job_last_run_findings_alert_router`           | `1779547971` |                | Unix epoch of last fire for plugin job 'findings_alert_router' (auto-written by PluginScheduler)                         |
| `plugin_job_last_run_fix_broken_external_links`       | `1779478706` |                | Unix epoch of last fire for plugin job 'fix_broken_external_links' (auto-written by PluginScheduler)                     |
| `plugin_job_last_run_fix_broken_internal_links`       | `1779478703` |                | Unix epoch of last fire for plugin job 'fix_broken_internal_links' (auto-written by PluginScheduler)                     |
| `plugin_job_last_run_fix_uncategorized_posts`         | `1779478703` |                | Unix epoch of last fire for plugin job 'fix_uncategorized_posts' (auto-written by PluginScheduler)                       |
| `plugin_job_last_run_flag_missing_seo`                | `1779521903` |                | Unix epoch of last fire for plugin job 'flag_missing_seo' (auto-written by PluginScheduler)                              |
| `plugin_job_last_run_media_reconciliation`            | `1779547671` |                | Unix epoch of last fire for plugin job 'media_reconciliation' (auto-written by PluginScheduler)                          |
| `plugin_job_last_run_morning_brief`                   | `1779519600` |                | Unix epoch of last fire for plugin job 'morning_brief' (auto-written by PluginScheduler)                                 |
| `plugin_job_last_run_noop`                            | `1779547671` |                | Unix epoch of last fire for plugin job 'noop' (auto-written by PluginScheduler)                                          |
| `plugin_job_last_run_postgres_vacuum`                 | `1779547672` |                | Unix epoch of last fire for plugin job 'postgres_vacuum' (auto-written by PluginScheduler)                               |
| `plugin_job_last_run_prune_orphan_embeddings`         | `1779506580` |                | Unix epoch of last fire for plugin job 'prune_orphan_embeddings' (auto-written by PluginScheduler)                       |
| `plugin_job_last_run_prune_stale_embeddings`          | `1779506220` |                | Unix epoch of last fire for plugin job 'prune_stale_embeddings' (auto-written by PluginScheduler)                        |
| `plugin_job_last_run_regenerate_stock_images`         | `1779547671` |                | Unix epoch of last fire for plugin job 'regenerate_stock_images' (auto-written by PluginScheduler)                       |
| `plugin_job_last_run_reload_site_config`              | `1779547971` |                | Unix epoch of last fire for plugin job 'reload_site_config' (auto-written by PluginScheduler)                            |
| `plugin_job_last_run_render_prometheus_rules`         | `1779547971` |                | Unix epoch of last fire for plugin job 'render_prometheus_rules' (auto-written by PluginScheduler)                       |
| `plugin_job_last_run_rollup_post_performance`         | `1779478703` |                | Unix epoch of last fire for plugin job 'rollup_post_performance' (auto-written by PluginScheduler)                       |
| `plugin_job_last_run_run_dev_diary_post`              | `1779541200` |                | Unix epoch of last fire for plugin job 'run_dev_diary_post' (auto-written by PluginScheduler)                            |
| `plugin_job_last_run_run_niche_topic_sweep`           | `1779547671` |                | Unix epoch of last fire for plugin job 'run_niche_topic_sweep' (auto-written by PluginScheduler)                         |
| `plugin_job_last_run_run_retention`                   | `1779547671` |                | Unix epoch of last fire for plugin job 'run_retention' (auto-written by PluginScheduler)                                 |
| `plugin_job_last_run_run_taps`                        | `1779547713` |                | Unix epoch of last fire for plugin job 'run_taps' (auto-written by PluginScheduler)                                      |
| `plugin_job_last_run_static_export_reconciliation`    | `1779547671` |                | Unix epoch of last fire for plugin job 'static_export_reconciliation' (auto-written by PluginScheduler)                  |
| `plugin_job_last_run_sync_newsletter_subscribers`     | `1779547671` |                | Unix epoch of last fire for plugin job 'sync_newsletter_subscribers' (auto-written by PluginScheduler)                   |
| `plugin_job_last_run_sync_page_views`                 | `1779547671` |                | Unix epoch of last fire for plugin job 'sync_page_views' (auto-written by PluginScheduler)                               |
| `plugin_job_last_run_topic_auto_resolve`              | `1779544800` |                | Unix epoch of last fire for plugin job 'topic_auto_resolve' (auto-written by PluginScheduler)                            |
| `plugin_job_last_run_tune_publish_threshold`          | `1779547671` |                | Unix epoch of last fire for plugin job 'tune_publish_threshold' (auto-written by PluginScheduler)                        |
| `plugin_job_last_run_update_utility_rates`            | `1779478705` |                | Unix epoch of last fire for plugin job 'update_utility_rates' (auto-written by PluginScheduler)                          |
| `plugin_job_last_run_verify_published_posts`          | `1779547672` |                | Unix epoch of last fire for plugin job 'verify_published_posts' (auto-written by PluginScheduler)                        |
...
| Key                       | Default      | Classification | Description                                                                                                           |
| ------------------------- | ------------ | -------------- | --------------------------------------------------------------------------------------------------------------------- |
| `dev_diary_last_run_date` | `2026-05-23` |                | YYYY-MM-DD (UTC) of the last successful dev-diary job run. Idempotency marker — the job no-ops if this matches today. |
...
| `grafana_webhook_oauth_jwt`        | `*(encrypted)*` | encrypted      | PR test row                                                  |
...
| Key            | Default         | Classification | Description                                             |
| -------------- | --------------- | -------------- | ------------------------------------------------------- |
| `indexnow_key` | `*(encrypted)*` | encrypted      | IndexNow API key for instant search engine notification |
