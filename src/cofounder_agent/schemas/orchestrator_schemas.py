...
class ProcessRequestBody(BaseModel):
    ...
    context: Optional[Dict[str, Any]] = Field(
        None,
        description="Optional context (audience, keywords, tone, etc.)"
    )
    auto_approve: bool = Field(
        False,
        description="Auto-approve if quality score >= 0.85"
    )
...
class ApprovalAction(BaseModel):
    ...
    publish_to_channels: List[str] = Field(
        ["blog"],
        description="Channels to publish to: blog, linkedin, twitter, email"
    )
    modifications: Optional[Dict[str, Any]] = Field(
        None,
        description="Optional modifications before publishing"
    )
...
class TrainingModelUploadRequest(BaseModel):
    ...
    model_type: str = Field(..., description="Type: task-router, content-generator, quality-evaluator")
