...
async def research_phase(
    state: ContentPipelineState, llm_service, db_service=None
    ...
    if state.get("models_by_phase") and phase in state["models_by_phase"]:
        ...
        quality_enum = (
            QualityPreference[quality.upper()]
            if quality in ["fast", "balanced", "quality"]
            else QualityPreference.BALANCED
        )
...
"""
    ...
    try:
        ...
        research = (
            ...
            if llm_service
            else f"Research data for {state['topic']}"
        )
            ...
            await db_service.log_cost(
                {
                    ...
                    "provider": (
                        "ollama"
                        if model == "ollama"
                        else "openai" if "gpt" in model else "anthropic"
                    ),
                    "cost_usd": cost,
...
"""
    ...
    try:
        outline = (
            await llm_service.generate(prompt) if llm_service else f"Outline for {state['topic']}"
        )
        state["outline"] = outline
        state["messages"].append(
            {"role": "system", "content": f"Outline created successfully using {model}"}
        )
...
"""
    ...
    try:
        draft = (
            await llm_service.generate(prompt)
            if llm_service
            else f"Draft content for {state['topic']}"
        )
        state["draft"] = draft
        state["messages"].append({"role": "system", "content": f"Draft completed using {model}"})
...
async def assess_quality(
    state: ContentPipelineState, quality_service, db_service=None
) -> ContentPipelineState:
    ...
    try:
        ...
        if quality_service:
            ...
            assessment = {
                "score": (
                    ...
                    else assessment_result.get("score", 0)
                ),
                "passed": (
                    assessment_result.passing
                    if hasattr(assessment_result, "passing")
                    else assessment_result.get("passed", False)
                ),
                "feedback": (
                    assessment_result.feedback
                    if hasattr(assessment_result, "feedback")
                    else assessment_result.get("feedback", "")
                ),
            }
        else:
        ...
        state["messages"].append({"role": "system", "content": quality_msg})
...
async def finalize_phase(
    state: ContentPipelineState, metadata_service, db_service
) -> ContentPipelineState:
    ...
    try:
        ...
        if metadata_service:
            metadata = await metadata_service.generate(
                content=state["draft"], topic=state["topic"], keywords=state["keywords"]
            )
        else:
            metadata = {
                "title": state["topic"],
...
def create_content_pipeline_graph(
    llm_service=None, quality_service=None, metadata_service=None, db_service=None
):
    ...
    workflow.add_conditional_edges(
        "assess",
        should_refine,
        {"refine": "refine", "finalize": "finalize"},  # Loop back to refine  # Go to finalize
    )
