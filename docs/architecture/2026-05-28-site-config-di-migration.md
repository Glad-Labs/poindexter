...
- **`site_config` mutation through the in-process refresh job** — the existing scheduled `reload_site_config` job mutates the loaded SiteConfig's `_config` dict in place. Container holds the SAME instance, so mutations propagate. Verify with a test.
  - **This risk materialized (resolved 2026-06-17).** PR 2's wire-up built the container with a _fresh_ `SiteConfig` (`build_container` constructed `SiteConfig(pool=pool)` internally) instead of reusing `main.py`'s `_site_cfg`. So the worker ran TWO instances: the scheduler-seeded `_site_cfg` (what the periodic `reload_site_config` job refreshed) and `app.state.container.site_config` (what `get_site_config_dependency` returns to route handlers). A runtime `poindexter settings set` refreshed the first but never reached routes until a restart (observed: `enforce_niche_allowlist` stayed enabled for ~4h despite a DB write + "site_config refreshed" logs). Fix: `build_container(pool, *, site_config=None)` now reuses a caller-supplied instance, and `main.py` passes `site_config=_site_cfg`. Verified by `tests/unit/services/jobs/test_reload_site_config.py::TestRuntimeReloadReachesRoutes` (the "Verify with a test" this line called for) + the `build_container` reuse tests in `tests/unit/services/test_container.py`.
