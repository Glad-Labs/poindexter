...
async def dispatch_command(request: CommandRequest) -> Dict[str, Any]:
    ...
    try:
        cmd = await create_command(
            agent_type=request.agent_type, action=request.action, payload=request.payload or {}
        )
...
async def complete_command(command_id: str, request: CommandResultRequest) -> Dict[str, Any]:
    """
...
async def fail_command(command_id: str, request: CommandErrorRequest) -> Dict[str, Any]:
    """
