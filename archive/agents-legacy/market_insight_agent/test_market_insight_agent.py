...
def test_suggest_topics(market_agent):
    ...
    expected_prompt = (
        "Based on the following search results, generate three blog post titles related to 'AI in gaming'. Return them as a numbered list.\n\n---SEARCH RESULTS---\n\n---END SEARCH RESULTS---"
    )
...
def test_create_tasks_from_trends(market_agent):
    ...
    mock_response = {"ideas": [
        {"topic": "AI in Game Development", "primary_keyword": "AI gaming", "target_audience": "Developers", "category": "Technology"}
    ]}
    ...
    market_agent.firestore_client.add_content_task.assert_called_once_with(mock_response["ideas"][0])
