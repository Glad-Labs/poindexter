...
class TestExampleAPIEndpoints:
    ...
    def sample_data(self):
        """Sample test data"""
        return TestDataBuilder.api_response(data={"id": "example_1", "name": "Test Example"})
    ...
    def test_create_example_success(self, client):
        ...
        """
        # Arrange
        payload = {"name": "New Example", "description": "Test description"}
...
class TestExampleAsyncOperations:
    ...
    async def test_async_database_operation(self):
        ...
        mock_db.query.return_value = [{"id": "1", "name": "Item 1"}, {"id": "2", "name": "Item 2"}]
    ...
    async def test_concurrent_operations(self):
        ...
        results = await AsyncHelpers.run_concurrent(
            mock_operation(1), mock_operation(2), mock_operation(3)
        )
    ...
    async def test_long_operation_timeout(self):
        ...
        async def slow_operation():
            await AsyncHelpers.wait_for(lambda: False, timeout=2, check_interval=0.1)
...
class TestExamplePerformance:
    ...
    def test_bulk_create_performance(self, client):
        ...
        start = time.time()
        for i in range(100):
            response = client.post("/api/example", json={"name": f"Item {i}"})
...
class TestExampleIntegration:
    ...
    def test_complete_workflow(self, client):
        ...
        """
        # Create
        create_response = client.post("/api/example", json={"name": "Test Item"})
        ...
        # Update
        update_response = client.put(f"/api/example/{item_id}", json={"name": "Updated Item"})
