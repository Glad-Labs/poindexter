...
class WorkflowValidator:
    ...
    def __init__(self, registry: Optional[PhaseRegistry] = None, mapper: Optional[PhaseMapper] = None):
    ...
    def validate_workflow(self, workflow: CustomWorkflow) -> Tuple[bool, List[str], List[str]]:
        ...
        if indices != expected_indices:
            errors.append(
                f"Phase indices are not sequential. "
        ...
        for phase in workflow_phases:
            ...
            if phase_def:
                ...
                if phase_def.max_retries > 5:
                    warnings.append(
                        f"Phase '{phase.name}' max_retries is high: "
                        f"{phase_def.max_retries}"
                    )
