...
- **Brainstem** (`poindexter/brain/`) — standalone self-healing watchdog daemon,
  genuinely independent of the FastAPI app at runtime (only needs Python + asyncpg;
  it ships inside the `poindexter` package since 2026-09-10 but the container
  runs it alone as `python -m poindexter.brain.brain_daemon`).
...
| Brain daemon | Container (poindexter/brain), no HTTP | Self-healing watchdog — Telegram alerts on failure |
| Grafana | http://localhost:3000 | 12 dashboards (server-side PNG/PDF rendering via the `grafana-image-renderer` sidecar — `GET /render/d-solo/<uid>/<slug>?panelId=N&width=&height=&encoding=png`, which is how a panel gets verified as pixels rather than as JSON)  (Mission Control / Pipeline / Cost / Observability / System Health / Integrations / **QA Rails** / **Findings** / **Experiments & Dry-Run** / **Database** / **Hardware & Power** / **SEO Harvest** / **Revenue**) |
...
### Key Numbers (repo-derived stats auto-synced daily by CI; DB-derived counts last refreshed 2026-09-26)
...
>
> **The root `README.md` rides these same two scripts** — its live/total post
> counts, pipeline runs, `app_settings` count and tests badge are anchored the
> same way, but **floored** to a round `N+` (198 posts publishes as "190+") so
> a public claim can never overstate and only churns on a threshold crossing.
> `scripts/ci/claude_md_anchor_lint.py` fails CI on any dead anchor in either
> file, so reword README's stat sentences only alongside their patterns.

- 211 live posts on gladlabs.io (385 posts total; 2,132 pipeline_tasks across all generation runs)
- 522 Python files under `src/cofounder_agent/poindexter/services/` (~291 substantive after `__init__.py` stubs)
...
- **Migration files** — the Phase G squash (2026-07-11) folded the Phase F baseline and its 42 post-baseline migrations into a fresh `0000_baseline.py` (+ `.schema.sql` + `.seeds.sql`). 69 ordinary migrations have accumulated since (`20260712_084907_rename_opening_to_content_originality.py` … `20260923_225300_drop_the_unpopulatable_subscriber_events_subscriber_id_column.py` — spell the newest one out in full, `.py` and all: `claude_md_sync`'s `migration_drift_note` is a literal substring test for that filename, so a `_*` glob leaves the nightly Discord note firing no matter how current the count is). Baseline seeds: non-secret `app_settings` 681 (+2 empty secret placeholders), `pipeline_templates` 6, `qa_gates` 20, `retention_policies` 32. Mechanics in the Database-migrations section.
- 13 Grafana dashboards (named in Monitoring below; Revenue unparked 2026-09-22 once the invoice poll gave `revenue_events` a live producer). Alerting is two-sourced: **19 static** rules across 4 active `infrastructure/prometheus/alerts/*.yml` files (`postgres-connections.yml` is a 5th, comment-only placeholder since its rules went DB-rendered for app_settings-tunable thresholds), plus **40 DB-sourced defaults** rendered into `rules/*.yml` by `RenderPrometheusRulesJob` every 5 min (incl. the six `Ups*` NUT rules, poindexter#958, and the `Cpu Temperature*` pair) — so the live count is 19 + whatever the DB currently seeds. Verify with `len(DEFAULT_RULES)` rather than trusting this number; it read "~29" while the real count was 36. Worker-gauge rules use a long `for:` with `last_over_time`/`max_over_time` to bridge deploy-restart scrape holes; see the restart-gap policy comment atop `DEFAULT_RULES` in `services/prometheus_rule_builder.py`.
- ~17,210 test functions across 1224 test files under `tests/unit` (+ 16 integration + 40 integration_db). Last full-green run 2026-08-12 on `4ee768bb2` — 17,190 passed, 21 skipped, 1 xfailed, 0 failed, 0 collection errors; 10m46s serial on host. Re-derive with a full run rather than trusting this number; it read "~11,440 / 10,352 passed" from the 2026-06-09 nightly for two months, understating the suite by ~5,800 functions. The 21 host skips are environmental, not flake: 13 memory tests want `LOCAL_DATABASE_URL`, 5 want ffmpeg/ffprobe on PATH, 1 wants the `rerank` extra, and 2 are deliberate opt-in canaries (`REGEN_GRAPH_DEF_FP`, the release-please version-drift check). A *different* handful skips in-container due to the `Path(__file__).parents[5]` path-depth quirk — those run on host.
- 2,077 app_settings keys live on prod (71 secret; drifts as keys are added). Three sources seed the table — `settings_defaults.py` 1,024, `0000_baseline.seeds.sql` 683 (= 681 non-secret + 2 placeholders), `poindexter/brain/seed_app_settings.json` 80 free-tier — and overlapping keys must agree, enforced by `scripts/ci/settings_seed_value_drift_lint.py`.
- `plugins/registry.py` `_SAMPLES` holds 126 core-sample plugins, of which 79 are job-type (taps + retention + memory hygiene + content surfaces); the rest are 13 stages / 9 taps / 10 topic_sources (incl. `rss` — generic RSS/Atom, one plugin + N `external_taps` rows, poindexter#1017) — **a `plugin.topic_source.<name>` row only permits a source; ingestion is scheduled by a per-niche `external_taps` row (`handler_name='builtin_topic_source'`, `tap_type=<name>`). `search_autocomplete` was enabled with seeds for weeks and contributed 0 topics because no tap row existed (fixed by migration `20260907_202228`); `topic_source_rank_weights` then boosts demand-measuring sources in the batch pre-rank. **Pool rows are `batched` only while a candidate references them** (poindexter#1042, 2026-09-08): a batch refresh replaced the candidate set wholesale and an expired batch kept its rows batched, so 1,002 rows sat consumed with no candidate and no task — the best-scored fresh rows first, i.e. every autocomplete row. `_write_batch` re-pools refresh losers and `reject_batch` re-pools the whole batch; one-off migration `20260908_171846` re-pooled the 30-day backlog / 7 image / 4 llm / 1 probe / 2 video (wan21 + comfyui — hero animator picked per install via `video_generative_provider`) / 1 audio. Several jobs are dormant behind master switches, so the live scheduled count is lower. The image providers cover three `kind`s — `search` (pexels, pexels_video), `generate` (image_gen, ai_generation, flux_schnell), and `screenshot` (screenshot, poindexter#1002: renders an **allow-listed** operator surface for posts about Poindexter itself; see [`docs/architecture/screenshot-image-provider.md`](docs/architecture/screenshot-image-provider.md)).
- 5 declarative-data-plane tables (`external_taps` / `retention_policies` / `webhook_endpoints` / `publishing_adapters` / `qa_gates`) feeding the integrations handler registry's 15 handlers across 4 handler surfaces (`tap` ×5 / `retention` ×6 / `outbound` ×2 / `publishing` ×2). `webhook_endpoints` is a declarative _table_ with no handler surface of its own — its rows are consumed by `integrations/outbound_dispatcher.py` + `operator_notify.py`. A 6th `services.declarative_config_service`-managed table, `alert_rules` (`poindexter alerts`), is CRUD'd through the same generic service but synced by the bespoke `poindexter/brain/alert_sync.py` loop rather than the handler registry.
- 79,862 embeddings across posts / issues / audit / memory / brain / claude_sessions (retention prunes claude_sessions/brain vectors, so this drifts)
...
### Brain Daemon (`src/cofounder_agent/poindexter/brain/`)
...
- **Container health watch** (`poindexter/brain/container_health_watch.py`, 2026-09-24): Docker restarts a container whose process EXITS, never one that is alive but failing its healthcheck — speaches wedged at 13:19 on 2026-09-24 (an API unload deadlocked it; see `docs/architecture/video-render-vram-gate.md`), was only marked `unhealthy` at 16:14, and every render until the 18:48 restart lost its captions. **The watch sees only what a healthcheck sees**: a wedge that leaves `/health` answering is caught late or never. The probe is **detector-only**: a container unhealthy for `container_health_alert_after_minutes` (10; per-container `container_health_alert_after_overrides`, empty — image-gen-server held 30 until 2026-09-25, when its GPU work left the event loop and `/health` started answering mid-render, stack#4021) fires `container_unhealthy` every cycle until healthy. **The restart is a firefighter `restart_container` rule per safe-to-bounce container**, matched on fingerprint `^container_health_watch:<name>\|` with `--verify-after 900` (prod: speaches, chatterbox, rife, stable-audio). Rows carry `remediation=rules_only` so the LLM long-tail never bounces an un-ruled container (GPU renderer mid-job, busy worker), and the engine now ignores non-firing rows — before that, any rule would have re-run its action on its own alert's recovery row. See `docs/operations/self-healing.md`.
...
`poindexter/schemas/`, `poindexter/config/`, `poindexter/tasks/`, and since PR B
`poindexter/brain/` (plus the CLI in `poindexter/cli/`). `src/cofounder_agent` is still the
filesystem root and the `sys.path` root (`main.py`, `tests/`, `skills/`, `console/`,
`pyproject.toml` did not move). **The flat spellings are gone** (`import services.x`,
`mock.patch("services.x.y")`, the umbrella `cofounder_agent.services.x`): step 5 (2026-09-11)
deleted the one-file stubs and the `sys.meta_path` alias finder that had kept them resolving
during steps 2–4, so `import services` raises `ModuleNotFoundError` and a string path spelled
flat is refused by `services/module_paths.py` with the canonical spelling in the message.
...
| `multi_model_qa.py` | The QA **rail library** the `qa.*` atoms delegate to (the monolithic `cross_model_qa` stage is deleted). Six rails: DeepEval ×3 (`deepeval_brand_fabrication`, `deepeval_g_eval`, `deepeval_faithfulness`); guardrails ×2, reimplemented native/dep-free in `services/guardrails_rails.py` after `guardrails-ai` was dropped for CVE-2026-45758 (`guardrails_brand` runs `content_validator` patterns, `guardrails_competitor` regexes the `guardrails_competitor_list` CSV); and Ragas ×1 (`ragas_eval`, averaging faithfulness + answer-relevancy + context-precision). **Four of the six run.** `deepeval_enabled` and `ragas_enabled` are `true`. Ragas was off 2026-09-15 → 09-25 as a crash stopgap: it re-entered the flow's event loop and killed the Prefect run (`pop from an empty deque`, poindexter#1053, fixed by stack#3848). It was re-enabled once a live in-loop smoke passed. Its judge runs at `pinned_llm_endpoint_num_ctx` (16384). The dispatcher runs every call routed to the pinned :11435 judge at that one size, overriding the caller's `num_ctx` and per-phase keys like `qa_ragas_judge_num_ctx` (those size a judge only when it isn't pinned). The judge instance holds one model at one context size, so any other size forces a 10-40 s reload. The warm job and the brain's RAM-recycle re-pin use the same key, and `scripts/linux/ollama-vision.sh` exports it as `OLLAMA_CONTEXT_LENGTH` for callers that send none (it read 48 loads a day, alternating 16384 / 32768 / 8192, before 2026-09-25; see `docs/operations/model-endpoint-routing.md`). Faithfulness still overflows that window on long drafts (about 20k+ chars) and scores -1.0. A gate row must follow its rail's master switch (poindexter#1065): an enabled gate for a rail that's switched off is inert, and graduating it would reject every post, so `declarative_config_service` refuses that graduation and `poindexter qa-gates list` warns (map in `qa_gates_db.RAIL_MASTER_SWITCHES`); the two guardrails rails were **retired 2026-09-10** (`guardrails_enabled=false`, gate rows disabled) because **no atom wires them** — the #355 cutover left them behind and they produced 0 reviews in 60 days while every config surface read "on". `url_verifier` was retired the same way. None of the three was load-bearing: brand cover is duplicated by `programmatic_validator` + `deepeval_brand_fabrication`, dead links by `citation_verifier`, and competitor screening had nothing to catch across 203 posts. Rewiring means adding an atom, not flipping a flag. Advisory-vs-hard-gate is DB-driven per rail via `qa_gates.<rail>.required_to_pass` (`_mark_advisory_if_configured`). |
| `rag_engine.py` | LlamaIndex `BaseRetriever` over the `embeddings` pgvector table (`rag_engine_enabled=true` on prod). Three modes stack: vector-only → `HybridRRFRetriever` (BM25 tsvector + pgvector + RRF fusion, `rag_hybrid_enabled=true` — pure SQL, default on since it needs no extra dependency) → `CrossEncoderRerankRetriever` (sentence-transformers cross-encoder, `rag_rerank_enabled=true` on Matt's prod but code-defaults **false**, since it needs the optional `rerank` poetry extra and a fresh install should opt in rather than warn-spam). **Two text columns, two jobs** (poindexter#1033): `embeddings.chunk_text` is the RETRIEVAL payload — the whole embedded chunk, which readers reach via `_retrieval_text()`/`COALESCE(chunk_text, text_preview)` — while `text_preview` is a `varchar(500)` DISPLAY slice. Writing only the preview discarded ~87% of indexed content between the match and the consumer; backfill is `scripts/backfill_embeddings_chunk_text.py` (no re-embed). `embeddings.text_search` is `GENERATED ALWAYS AS (to_tsvector('simple', COALESCE(chunk_text, text_preview)))` — auto-populated, GIN-indexed (`idx_embeddings_text_search`); it indexed the preview alone until 2026-08-29, giving BM25 the same blind spot. **The writer's snippet query in `two_pass_writer` is a separate retrieval path** and needed the same fix — check it whenever you change the payload. Capped consumers (writer snippet block, reranker) window their budget onto the query via `services/rag_excerpt.py` rather than head-slicing. See [`docs/architecture/rag-retrieval-stack.md`](docs/architecture/rag-retrieval-stack.md). |
...
| `newsletter_service.py` | **Per-post** newsletter sender — `send_post_newsletter` fires on every publish seam (`publish_service` ×4 + `scheduled_publisher`), NOT on a cron. There is deliberately no `plugin.job.*newsletter*` row, so looking for one and concluding "nothing sends" is wrong — the send ledger is `campaign_email_logs` (70 delivered through 2026-09-22), which doubles as the per-`(campaign, subscriber)` idempotency ledger so the several go-live seams cannot double-mail. Delivery *receipts* are a separate feed: `subscriber_events`, fed by `SyncResendDeliveryEventsJob` polling Resend's `GET /emails` for `last_event` (the `POST /api/webhooks/resend` route is unreachable — 401 local, 404 public — and its silence hid behind a freshness threshold raised 7 → 180 days). |
...
**`CANONICAL_BLOG_GRAPH_DEF` is the authoritative node list — read it rather than trusting a count here** (this line has drifted from the spec before). At the v13 reseed (2026-09-15 — `v<N>` is `pipeline_templates.version` on the stored row, so check that, not this date) it was 48 nodes: 10 `stage.*` + 16 `content.*` + 17 `qa.*` + 1 `qa.rewrite` + 1 `seo.*` + 2 `atoms.approval_gate` + 1 `social.generate_drafts` — a mostly-linear chain plus **three** bounded backward cycles: one QA rescue plus two `preview_gate` regen paths (both documented below). Blocks in order:

1. **verify** — `verify_task`
2. **writer** — `content.generate_draft` → `content.generate_title` (draft-grounded: the prompt reads an article digest — a `title_content_excerpt_chars` excerpt plus the section-heading skeleton — with the topic demoted to assignment-label context, plus a `services/title_avoidance.py` variety block that names the recent corpus's *habits* rather than pasting its titles — see [`docs/architecture/title-variety.md`](docs/architecture/title-variety.md)) → `content.check_title_originality` → `content.normalize_draft` → `draft_gate` (`atoms.approval_gate`, seeded disabled) → `content.detect_contradictions` → `content.revise_contradictions` (self-review, split from one `stage.writer_self_review` node 2026-08-28: the two calls have opposite demands and can now use different models — `writer_self_review_review_model` detects, `writer_self_review_model` revises and is the one whose text ships. Seeded empty = same model for both, i.e. unchanged. Measured n=4/arm: gemma detected 1/4 but revised cleanly; glm-4.7 detected 4/4 and had every revision rejected as too long. A revision failing the minimal-edit contract is DISCARDED, keeping the original draft) → `resolve_internal_link_placeholders` → `content.reconcile_citations` (deterministic citation repair — re-links named sources the writer dropped) → `content.llm_reconcile_citations` (grounded-LLM tail; the LLM proposes, code verifies url∈corpus + text-verbatim before applying) → `content.inject_affiliate_links` (keyword-matched, dark-launched behind `affiliate_injection_enabled`) → `quality_evaluation` (pattern-based scorer) → `url_validation`
3. **image** — `content.plan_image_markers` (honours the writer's `[HERO-IMAGE:]` subject, numbers inline `[IMAGE:]` markers up to `writer_max_inline_images` and `[SCREENSHOT:]`/`[CHART:]` evidence markers up to `writer_max_evidence_per_kind` **each**, then **tops up** the remaining illustration slots via the body-fed Image Decision Agent — it used to run the agent only when the writer placed NO markers, so from 2026-09-02, when two_pass gained `[SCREENSHOT:]`, one screenshot suppressed every generated illustration; the `{screenshot_targets}` block is also gated to posts about this system via `screenshot_topic_kinds` / `screenshot_topic_keywords`, see [`docs/architecture/screenshot-image-provider.md`](docs/architecture/screenshot-image-provider.md)) → `content.generate_images` → `content.inject_images` → `source_featured_image` (grounded on the writer's hero subject via `_resolve_featured_subject`, else the decision-agent pick, else topic; art style rotates least-recently-used via `_select_style_lru`) → `caption_images` (vision re-caption)
4. **QA rails** — `qa.programmatic` → `qa.critic` → `qa.deepeval` → `qa.ragas` → `qa.vision` → `qa.topic_delivery` → `qa.citations` → `qa.numeric_fidelity` → `qa.unlinked_attribution` → `qa.consistency` → `qa.self_consistency` → `qa.content_originality` → `qa.title_coherence` → `qa.self_claim` → `qa.freshness` → `qa.web_factcheck` → `qa.aggregate`
...
QA runs as composable `qa.*` atoms in `modules/content/atoms/` that delegate to the `MultiModelQA` rail methods and append to the `qa_rail_reviews` PipelineState channel. `canonical_blog` wires **16** of them (listed in block 4 above); more rail atoms exist on disk than this graph uses — `atom_registry` discovers them all, so an ad-hoc `graph_def` can compose any of them. `qa.aggregate` combines them into the gate decision: on reject it does the DB writes via `modules/content/atoms/_qa_persist.py` and **halts the graph** (outside the rescue cycle it does not auto-rewrite); on approve it promotes `quality_score` and populates `qa_reviews`. Graduating a rail from advisory to blocking is `qa_gates.<rail>.required_to_pass=true`.
...
- `page_views` — own analytics tracking. Ingested rows carry an `is_bot` flag set by `FlagBotPageViewsJob` (windowed `(user_agent, path)` flood-cap + per-UA distinct-paths sweep for full-site crawlers, poindexter#973); reader surfaces (console `/api/analytics/views`, `posts.view_count`, `lab_outcomes_v1`) read the `page_views_human` view, while liveness/anomaly/freshness signals stay on raw `page_views`. See [`docs/architecture/page-views-bot-flag.md`](docs/architecture/page-views-bot-flag.md). **The ingest cursor deliberately trails wall-clock** by `plugin.job.sync_cloudflare_analytics.config.ingestion_lag_seconds` (default 300): CF Analytics Engine has a non-zero write→queryable delay, so a poll that advances the high-water mark to `now()` steps permanently over every row still in flight — 40 real August page views were lost this way before stack#3523 (2026-08-31). Never advance the watermark past `now() - margin` on ANY path, empty or not; the insert-time dedup plus the `view_count` recompute make the resulting re-pull overlap free. The rule lives in `services/watermark_cursor.py` and is **shared with `sync_affiliate_clicks`**, which had the identical bug — a test fails if either job grows its own copy. See [`docs/architecture/analytics-ingestion-lag.md`](docs/architecture/analytics-ingestion-lag.md).
...
- **A project module named by string goes through `services/module_paths.py`:** never `importlib.import_module("poindexter.services.x")` / `__import__("poindexter.utils.y")` / `sys.modules.get("poindexter.services.z")` directly &mdash; use `import_module_path` / `import_object_path` / `resolve_module_path` from that seam. Step 1 of the `poindexter.*` namespace migration (Glad-Labs/poindexter#1046): the ~6,400 `import` statements will be rewritten mechanically, but the ~190 places a module is named by *string* (the plugin registry's core samples, `atom_registry`'s package walk, `http_client`'s wiring list, route registration, the import audit, the CLI's lazy provider load) fail at runtime rather than import time, so they were moved behind one resolver first. Since step 5 it accepts the canonical spelling only: a flat `services.x` raises `ModuleNotFoundError` at the seam with the `poindexter.services.x` fix in the message, third-party paths and the CLI's own `poindexter.cli.*` pass through, and the seam stays as the one place a future root change would touch. `tests/unit/services/test_module_paths.py` AST-walks the wired files and imports every string path under both spellings, with a floor so a refactor can't blind it. Out of scope for the seam, by design: `emit_finding(source="modules.content…")` and other **persisted labels** &mdash; those are data, handled later in the epic.
- **A producer must have a consumer, and that is checked mechanically:** the
  recurring defect in this codebase is not a crash — it is a mechanism that runs
  correctly, writes its output, reports healthy, and is **read by nobody**. The
  producer cannot see it from its own side, so it stays green indefinitely:
  ...
  this guard too, because the name was never listed here") — it now derives from
  the seeded gate rows. Second, **the detection that works is comparing two
  independent recorders of the same event** (`atom_runs` vs `qa_gates` vs
  `qa_pass_completed`); no single table shows it. Deliberately NOT checked:
  endpoints with no recorded traffic — 83 of 131 in 30 days, almost all
  legitimately-rare operator affordances, which is the bandit-91-false-positives
  mistake waiting to happen.
- **A check that scanned nothing has not passed:** every `scripts/ci` lint must fail when its scan root is missing or it examined zero files &mdash; `scripts/ci/lib_scan_floor.py` (`require_dir` / `require_scanned`) is the shared guard, and `tests/unit/scripts/test_ci_lint_scan_floor.py` runs every lint in an empty tree and asserts a non-zero exit, so a NEW lint that forgets its floor is caught the moment it is added. Earned 2026-08-28: the poindexter#1029 follow-on audit copied every lint into an empty tree and **10 of 12 printed "clean" and exited 0**, three of them with no count in the output at all. The trigger is ordinary refactoring, not neglect &mdash; `modules/content/` arrived as a physical code move and the May 2026 wave deleted whole trees, so the next rename of `src/cofounder_agent` or `poindexter/brain/` would have turned ten gates permanently green. Same rule applies to datasource-backed checks: `grafana_panels_lint.py` fails when a *configured* datasource validated zero of its own targets, because every connection failure inside it is deliberately SKIP/WARN and would otherwise exit 0. A legitimate zero must be distinguished from a disarmed one &mdash; `uv_lock_version_lint` floors on lockfiles *discovered*, not *checked*, because the mirror strips a directory and correctly skips its lock.
- **Private-repo code scanning is self-hosted, not GitHub's:** GitHub code scanning is **disabled on every Glad-Labs private repo** by an org-level security configuration (`Private Repos`, id 246930) &mdash; a repo-level toggle returns `422`, and enabling it bills per active committer (`purchased_advanced_security_committers: null`). The public mirror sits on a different config with everything on, which is what made the gap invisible for 70 days. Since the sync filter strips the overlay (`modules/finance/`, `operator_*.py`, the sessions tap), that code is seen by neither repo's CodeQL. `scripts/ci/semgrep_lint.py` + `semgrep_baseline.json` cover it as a **ratchet** (same doctrine as bandit: block net-new, file nothing &mdash; semgrep's 47 findings here were 44 hits of one rule matching a log line reading "requires login", plus a `sql-injection` on a parameterized asyncpg call). Rules are **vendored** under `infrastructure/semgrep/`, not pulled from the registry, so no network call can redden the gate and a rule change arrives as a reviewable diff. Refresh procedure in that dir's README &mdash; read the new findings before re-baselining.
...
- **Grafana (self-hosted):** http://localhost:3000 (or http://nightrider-1.taild4f626.ts.net:3000 from the tailnet) — 13 dashboards (Revenue unparked 2026-09-22). Grafana Cloud was retired 2026-05-03; the local Docker container (poindexter-grafana) is the only Grafana now. Local Prometheus scrapes node_exporter (host, `:9100`) + the nvidia-smi gpu-exporter container directly (post-Linux-migration; windows_exporter retired with the Windows host); Alloy was the Cloud shipper and is no longer used. **The operator console (`/console/`) serves the deep-telemetry surfaces natively under a Telemetry tab** — Loki logs (worker proxy `GET /api/logs`), Langfuse LLM traces (`GET /api/traces`, waterfall deeplinks out to Langfuse), and native HISTORY / GPU & POWER / DATABASE chart sections (browser-direct Prometheus `query_range` against `:9091`, CORS-allowed) — so day-to-day operation no longer requires opening Grafana directly. The former Grafana `/d-solo` iframe embeds were removed 2026-07 ("Killing the Iframe"); `GF_SECURITY_ALLOW_EMBEDDING=true` remains in the compose `grafana` service but the console no longer depends on it.
- **Dashboards:**
  ...
  - **Cost & Analytics** — LLM spend, energy, posts published, plus the "Model Throughput — tokens & speed" row (per-model effective output tok/s + hourly token volume from `cost_logs`; console mirror = the two History "Model throughput" cards via `GET /api/metrics/llm-throughput/trend`)
  ...
  - **Database** (`/d/database`) — Postgres internals grouped by subject, not by datasource: connections + states, tables & vacuum debt, index/query activity, storage & transactions, app-side asyncpg pool + ping latency, and the data-feed dead-man's switch. Extracted from System Health 2026-06-03; the natural home for postgres_exporter metrics (#650). **Per-table panels must stay top-N** — the table-size and dead-tuple charts once selected all 116 user tables with no `LIMIT` into an 8-row panel and rendered their category labels as an unreadable overprinted block; the full per-table set belongs in the sortable table, not a bar chart. Dead tuples are single-sourced to Prometheus, and that `topk()` target is an **instant** query: as a range query it returns the union of every series that entered the top 10 anywhere in the window (18 series over 6h), several showing a stale `lastNotNull` from before they dropped out.
  ...
  - **Revenue** (`/d/revenue`) — money in, from `revenue_events`. Parked 2026-07-01 (one April row, 12 no-data panels eroding trust) and **unparked 2026-09-22** when `pro_delivery`'s invoice poll became a live producer (stack#3954). The producer is `GET /v1/subscription-invoices`, NOT the `routes/external_webhooks.py` receiver — that route is unreachable from the internet and has never fired in production. Reads near-zero until checkout goes live; that is the honest resting state, not a fault.
...
NOTE: repo-derivable CLAUDE.md stats still auto-sync daily via the `.github/workflows/sync-claude-md.yml` GitHub Action (06:17 UTC) independent of these sessions; the deterministic `claude-md-sync` refreshes the **DB-derived** counts (posts, embeddings, app_settings totals) via a prod-DB probe. The DB half **works** — verified 2026-08-28: the 02:31 EDT fire opens, arms auto-merge on, and merges a PR daily (#3419 08-28, #3372 08-27, #3346 08-26, and every day back through #3275 08-19; the 06:31 UTC PR timestamps are the same fire). It WAS broken from 2026-06-10 to 2026-07-26 — `gh pr create` ran against the shared checkout rather than the session worktree, emitting `rc=1 stderr=must be on a branch named differently than "main"` while still logging "opened CLAUDE.md sync PR" — fixed in #2809, first clean unattended run #2825. The "currently broken" claim outlived the fix by a month; check `gh pr list --search "CLAUDE.md sync"` before believing a staleness claim here.
...
**A `graph_def` reseed is a migration that declares its graph signature.**
Change an atom's `AtomMeta` contract or an active graph's nodes/edges and the
stored `pipeline_templates` row fails `assert_graph_def_current` at load — the
boot self-heal restamps only never-stamped rows, so the fix is a migration
built on `services/graph_def_reseed.apply_reseeds` with a 5-field `_RESEEDS`
(the fifth field is the graph signature the reseed brings the row to). The CI
gate `test_graph_def_reseed_gate.py` requires the newest such migration per
active graph to declare the live signature, so the change cannot merge without
it. Regenerating `graph_def_contract_fingerprints.json` alone turned the older
per-atom gate green while prod stayed broken — twice (poindexter#1876,
stack#3928, which halted every Stage-2 video render 2026-09-22). Pattern:
[`docs/operations/migrations.md`](docs/operations/migrations.md) → "Reseed a
`graph_def`".
...
and `poindexter/brain/seed_app_settings.json` seeds 81. All three use `ON CONFLICT DO
...
`0000_baseline.seeds.sql`. `poindexter/brain/seed_app_settings.json` is a deliberate
...
- **Title variety + duplicate detection:** [`docs/architecture/title-variety.md`](docs/architecture/title-variety.md) — the two independent title paths (`content.generate_title` for canonical_blog, the `TITLE:` line in `atoms.narrate_bundle` for dev_diary — dev_diary had NO avoidance mechanism at all before stack#3209), why the recent-titles dump primed the formula it was meant to suppress, and the **two originality axes** (stack#3213): `qa_title_similarity_threshold` is DuckDuckGo-only (did we restate someone else's headline?) while `title_internal_similarity_threshold` compares against our own published+queued titles (did we already write this?). Separate switches on purpose — `is_original` is false when either trips, which routes a collision into the existing regeneration loop. `originality_rank` ranks candidates on `(axes_colliding, worst_similarity)`, NOT on `max_similarity`, which is external-only and silently discarded internal fixes. **Which title SHIPS is a third axis (stack#3217):** `publish_post_from_task` preferred the draft's leading heading over the canonical title AND read it from `merged['title']` (stage_data, absent for 90% of tasks), so 71.3% of canonical_blog posts published under the writer's H1 and the two mechanisms above governed a string no reader saw. Precedence is now canonical → heading → topic with topic-echoes skipped; `publish_title_source='body_heading'` reverts it. **Fourth axis — searchability (2026-09-07):** `services/title_searchability.py` requires at least one searchable entity (digit token / proper noun / article keyword) in the canonical title; one corrective regen, then a `title_no_searchable_entity` finding. Earned by the August 2026 cohort (3.4 first-21-day impressions per post, 0 clicks, titles like "The Gap Nobody Names").
- **How a podcast is graded:** [`docs/architecture/podcast-pipeline-stage3.md` §13](docs/architecture/podcast-pipeline-stage3.md) — the article's 17 `qa.*` rails **never touch the episode** (the script is authored in block 6, after the rails in block 4, and no `qa_gates` row matches audio/podcast). Three mechanisms grade it instead: in-graph `qa.audio` (silence/volume/duration, fail-soft, findings only), Gate-2 **Layer 1** (size/duration/silence-ratio — the ONLY thing that can auto-reject, and it has never fired), and Gate-2 **Layer 2** (Whisper transcript judged against `posts.content`, advisory). §13 records the five holes closed 2026-09-18, each measured before it was fixed: the faithfulness judge was the script writer (15 of 21 episodes self-graded exactly 100); an unmeasurable episode was stamped 100 instead of NULL (12 of 39); `qa.audio` produced `audio_qa_result` that **nothing read**; every media-QA finding inherited `findings.default.delivery='log_only'` and reached nobody for three months; and auto-approve skipped the whole eval rather than just the human step. Still open there: the grade and the operator disagree (three episodes rejected at scores of 100/92/100), and the script is ungraded as text before the TTS spend.
- **Media subject/style policy (per niche):** [`docs/architecture/media-subject-policy.md`](docs/architecture/media-subject-policy.md) — whether AI media may show people and whether it may be photoreal is `media_human_subjects` / `media_style_policy`, overridable as `niche.<slug>.media.*`; one resolver (`services/media_subject_policy.py`) feeds the director, writer and image prompts, the negative prompt, the shot-list validator and the video QA frame check. People are allowed by default since 2026-09-14 (the no-faces/no-hands rule predates models that render them cleanly); the house style stays stylized unless a niche opts into `any`.
- **Custom YouTube thumbnails:** [`docs/operations/youtube-thumbnails.md`](docs/operations/youtube-thumbnails.md) — every upload before 2026-09-25 showed a frame YouTube picked. The long video's thumbnail is now **composed, not generated** (`services/video_thumbnail.py`: chromium sets real type over a text-free background — diffusion cannot set type), made by the media pipeline's `render_thumbnail` node (media_pipeline v7), stored as a `video_thumbnail` asset and reviewed with the video in the console's media drawer. `media_distribute` hands it to `thumbnails.set` after `videos.insert`; a thumbnail failure never fails the upload (`youtube_thumbnail_failed` finding). Hook text is code-checked (length, no invented number, not a title fragment; one retry, then image-only). Every look knob is a `video_thumbnail_*` setting, and scrim/shadow/glow/grid derive from the colour settings rather than literals. `poindexter integrations youtube thumbnails` backfills published videos: the dry run stores thumbnails for review and `--apply` uploads exactly the stored file, never a recomposition. Impact is read from the `youtube_reach` tap (thumbnail impressions + CTR in `external_metrics`).
...
- **Topic demand signals:** [`docs/architecture/topic-demand-signals.md`](docs/architecture/topic-demand-signals.md) — the batch pre-rank measures fit to niche goals and nothing about demand; three multipliers now act on it, each recorded in `topic_candidates.score_breakdown`: `topic_source_rank_weights` (demand-measuring sources), Wikipedia pageviews for the entity a candidate names (`services/entity_demand.py`, cached in `entity_demand_cache`, 1k→1.0 … 100k→2.0), and a dual-signal bonus when a Google-demand source AND Wikipedia agree. Unknown = factor 1.0 + `_wiki_views: None`, never a fabricated zero. Cadence on prod was cut the same day (auto-resolve daily, batch 3, SLO 0.5/day over 7 d).
- **Analytics ingestion lag:** [`docs/architecture/analytics-ingestion-lag.md`](docs/architecture/analytics-ingestion-lag.md) — why the `page_views` high-water mark stops short of `now()`. CF Analytics Engine makes a data point queryable some seconds after `writeDataPoint` returns, so an empty poll that jumps the cursor to `now()` buries every row still in flight, silently and permanently. Carries the 2026-08-31 control pair (two identical beacons, one lost, one kept) and the one rule that fixes both paths.
- **Benchmark findings (instrumentation as topics):** [`docs/architecture/benchmark-findings.md`](docs/architecture/benchmark-findings.md) — the only topic source that reads our OWN telemetry (`cost_logs`) instead of the outside world, proposing a post only when the measurements clear a sample floor, a breadth bar and an effect bar. **The measurements travel with the topic**: `DiscoveredTopic.description` → `topic_pool.summary` → `create_blog_post_task` → task `metadata.research_context`, which both grounds the writer and becomes the corpus `qa.numeric_fidelity` checks the finished draft against. That carry-forward was **broken for every source** — `claim_best_pooled_topic` always returned the summary and task creation always dropped it, discarding the context on 804 of 1,815 pool rows. Newness is a fact about the model, NOT about our instrument: the first-seen query is deliberately unfiltered on `decode_duration_ms` (which only exists from 2026-08-26, so filtering makes every model look new — `qwen3-vl:30b`, running for months, was proposed as "new" on the first run). Ships inert, and `enabled:false` MUST be a seeded row because `topic_sources/runner.py` defaults a MISSING row to `enabled=True`.
- **Numeric fidelity (the only arithmetic QA rail):** [`docs/architecture/numeric-fidelity.md`](docs/architecture/numeric-fidelity.md) — every other anti-hallucination layer is a judgement (regex shapes, or an LLM asked if the draft looks faithful); `qa.numeric_fidelity` checks that each number the draft presents as SOURCED FACT reconciles with a number in `research_context`, at the precision written. **Pair-derivation is OFF by default** (2026-09-01): against a 35-value fact block, 81 of 99 INVENTED percentages were "explained" by some pair (~3N² candidates chasing ~100 buckets), blinding the rail to the fabrication it exists to catch; disabling costs one extra false positive per 40 posts. Corpus extraction also excludes digits fused to letters — `phi4:14b` was contributing 4 and 14 as arithmetic operands. **Attribution is the scoring gate and it was measured, not guessed**: scoring every number flagged 33% of real prose, all false positives (hypotheticals, rhetorical targets, and self-claims that belong to `qa.self_claim`'s corpus) — digit-bearing sentences are mostly years, image URLs and figures of speech. Over 40 posts it scored 29 attributed claims, 27 reconciled, and caught a post whose HEADLINE statistic ("110,000 papers") appears nowhere in its research. Appends **no review at all** when it has nothing to judge (empty `research_context` = 42% of runs, thin corpus, no attributed claims, or its own crash) — a rail that scores 100 on absent evidence reads as coverage on the dashboard. Advisory-first via `qa_gates.numeric_fidelity` — and it must STAY advisory while it appends nothing, because `missing_required_gates` reads an absent required rail as a veto: graduating a silent rail hard-rejects every clean post (poindexter#1060 — it cost three rails and six days of queue). Graduating this one means emitting `_qa_rail_common.not_applicable_review` first; `tests/integration_db/test_required_qa_gates_never_go_silent.py` fails until you do. Strict mode (`score_unattributed=True`) is built + tested for a future generated-fact-set producer but deliberately NOT wired to a `numeric_facts` channel, because an undeclared PipelineState channel is how the auto-publish gate starved for six weeks.
- **Decode/prefill split capture:** [`docs/architecture/decode-split-capture.md`](docs/architecture/decode-split-capture.md) — `duration_ms` is wall-clock (queue + load + prefill + decode); `decode_duration_ms` is true decode speed, recovered from Ollama's `eval_duration` by monkey-patching LiteLLM's Ollama `transform_response`. **The gap between them is the finding**: same GPU, `glm-4.7` pays a 3% tax (pinned, warm) while `phi4:14b` pays 80% (evicted and reloaded almost every call) — read it as residency economics, never as "phi4 is slow". Pre-2026-08-26 rows are NULL and **not backfillable**; NULL means "not reported", never 0. Because the capture seam is fail-open it can go dark silently, so `ProbeDecodeSplitCoverageJob` watches it with a **self-calibrating denominator** — a model that ever reported a split is entitled to one — rather than a hardcoded local-model list that would rot on the next pin change.
- **Chart rendering:** [`docs/architecture/chart-rendering.md`](docs/architecture/chart-rendering.md) — `services/chart_render.py` draws `bar`/`line` charts from a data spec into PNG via the chromium that playwright already ships, closing the gap left by `blog-generation/SKILL.md`'s (correct) ban on asking a diffusion model for a chart. `build_chart_html` is pure, so ticks/geometry/escaping test without a browser. The palette is **validated for colorblind separation** (worst adjacent CVD ΔE 24.7), hues are assigned in fixed order and never cycled, and `nice_ticks` must return a final tick ≥ max_value — a 235 tok/s bar once drew past a 200 axis and lost its label. `ChartProvider` takes **data, never a query**: an image plugin that could fetch its own data would need a query surface reachable from a writer-emitted marker, so there is no SQL in it and a test asserts there never will be. **Both evidence markers are on the two_pass writer** — `[CHART:]` and `[SCREENSHOT:]`. That matters: `[SCREENSHOT:]` (poindexter#1002) lived only on `blog_generation.initial_draft` while `canonical_blog` runs two_pass, so it shipped **inert** — zero `image.screenshot` assets ever, 0 of 199 recent drafts carrying it, despite a `qa-rails` target being configured the whole time. A marker the live writer is never told about is indistinguishable from one it declined to use. Only those two markers are on that prompt; ordinary illustrations stay with the Image Decision Agent, pinned by a test. **The `[CHART: key]` marker path is live** — writer marker → `[IMAGE-N: chart:key]` → `plan["chart_target"]` → `services/chart_catalog.py` (a SERVICE, which may own the query) → provider, mirroring `[SCREENSHOT:]` hop for hop. The writer picks WHICH chart, never what it says; charts are code-defined (a settings-defined chart would be operator SQL reachable from an LLM-chosen key) and the per-install lever is the `chart_catalog_enabled_keys` allowlist. Every failure — unknown key, disabled key, too few models, a raised query — renders an empty slot, because a wrong chart is worse than none.
- **TTS text normalization:** [`docs/architecture/tts-text-normalization.md`](docs/architecture/tts-text-normalization.md) — the one render-boundary seam (`_normalize_for_speech`) every audio path shares, and the dash rule table (ISO date / range / negative / compound-hyphen → space, letter-digit left alone). **Names the instrument trap:** a TTS→STT round-trip answers "are the WORDS right?" and is BLIND to "is the TIMING right?" — whisper writes a pause back as nothing, which is why the 2026-08-24 probe cleared word-word compounds as verbatim while the engine was breathing at every hyphen (9,406 of them across 953 stored scripts). Measure timing with `ffmpeg -af silencedetect` over the rendered audio, words with the round-trip.
- **Speaches Kokoro device:** [`docs/operations/speaches-kokoro-gpu.md`](docs/operations/speaches-kokoro-gpu.md) — speaches v0.8.1 hands onnxruntime `list(set(providers))`. A set's order follows the per-process hash seed, so each container start decided CPU or GPU for Kokoro until the next restart. 9 of 20 speaches processes (2026-08-26 → 09-25) ran it on the CPU at ~85 chars/s instead of ~1,000, for days at a time, with no error anywhere. `PYTHONHASHSEED=0` pins CUDA first. `test_speaches_kokoro_provider_order.py` ties that seed to the image digest, so a digest bump fails until someone re-checks (upstream sorts the providers from v0.8.2). **Second trap, found while verifying:** this image's CUDA libraries predate the 5090, so the first GPU read in a fresh container spends ~75 s JIT-compiling. `CUDA_CACHE_PATH` keeps that cache on the HF mount across recreates. Diagnose from speaches' own `Generated audio for N characters in X s` log line. CI's old `"A" * 500` requests are no guide: they have almost no phonemes, so they run fast even on the CPU.
- **`cost_logs.model` identity:** [`docs/architecture/cost-logs-model-identity.md`](docs/architecture/cost-logs-model-identity.md) — why one local engine logged under up to three spellings (bare / `ollama/` / `ollama_chat/`), splitting every per-model `GROUP BY` into phantom series. 29 call sites strip the prefix off their `*_model` pin and the router silently re-adds it, so the calls were identical and only the log diverged. `dispatch_complete` now records what the provider actually dispatched, and every per-model panel normalizes with `regexp_replace(model, '^ollama(_chat)?/', '')`. `ollama_chat/` stays distinct on purpose — different endpoint, not a spelling. Also names the three non-LLM row groups on that table (`system` = brain electricity, NOT API spend).
- **Retention correctness:** [`docs/architecture/retention-backlog.md`](docs/architecture/retention-backlog.md) — why every retention signal we had (`last_error` NULL, current `last_run_at`, non-zero `total_deleted`, green panel) is a LIVENESS signal, so a misconfigured policy and an idle one are byte-identical — `checkpoint_prune` pruned nothing for months behind all of them. Handlers now declare a backlog expression stating the INVARIANT, not re-running the policy's own (possibly wrong) predicate; persistence across probes is the alarm, not magnitude; and a handler that declares none is reported `unmonitored`, never as a passing zero.
