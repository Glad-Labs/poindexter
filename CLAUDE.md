...
- **In-flight cleanup sweep (2026-05-16)**: 10 PRs ranked by ROI — Prefect Stage 4 (✅ landed), LangGraph cutover finalization (✅ landed — `content_router_service.py` legacy chunked path + `plugins/stage_runner.py` both deleted), llm_text → dispatcher consolidation (✅ landed), shared httpx client, Module v1 Phase 3.5 (ContentModule physical move + FinanceModule wiring), settings discipline. **Cutover gates are all already `true` on prod** — Prefect is the dispatcher, canonical_blog is the pipeline, LiteLLM is the LLM router, LlamaIndex+Ragas+DeepEval+Guardrails are all on.
...
- **LangGraph cutover (Lane C):** [`docs/architecture/langgraph-cutover.md`](docs/architecture/langgraph-cutover.md) — **Stage 4 complete (2026-05-16)**: legacy chunked StageRunner path in `content_router_service.py` deleted, plus `plugins/stage_runner.py` itself (no remaining production caller). `app_settings.default_template_slug='canonical_blog'`; the LangGraph `canonical_blog` template in `services/pipeline_templates/__init__.py` is now the only production pipeline. New stages go on the LangGraph template.
