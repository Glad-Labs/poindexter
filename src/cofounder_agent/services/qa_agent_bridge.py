...
class QAAgentBridge:
    ...
    async def create_hybrid_evaluation(
        ...
        try:
            ...
            logger.error(
                f"[_create_hybrid_evaluation] Error creating hybrid evaluation: {e}", exc_info=True
            )
            raise
    ...
    def _synthesize_feedback(
        ...
        if weak_criteria:
            parts.append(
                f"Pattern analysis shows room for improvement in: " f"{', '.join(weak_criteria)}"
            )

        strong_criteria = [k for k, v in pattern_scores.items() if v >= 8.5]
        if strong_criteria:
            parts.append(f"Excellent performance in: {', '.join(strong_criteria)}")

        return " ".join(parts)

    def _generate_recommendations(
        self, qa_approved: bool, qa_feedback: str, pattern_scores: Dict[str, float], passing: bool
    ) -> list:
        """
