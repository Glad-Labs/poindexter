...
class WritingStyleIntegrationService:
    ...
    async def get_sample_for_content_generation(
        ...
        try:
            ...
            if writing_style_id:
                ...
                sample_data = await self.writing_style_service.get_style_prompt_for_generation(
                    user_id
                )
    ...
    def _analyze_sample(self, sample_text: str) -> Dict[str, Any]:
        ...
        has_examples = (
            "example" in text_lower or "for instance" in text_lower or "such as" in text_lower
        )
    ...
    async def generate_creative_agent_prompt_injection(
        ...
        """
        try:
            sample_data = await self.get_sample_for_content_generation(writing_style_id, user_id)
    ...
    def _build_analysis_guidance(analysis: Dict[str, Any]) -> str:
        ...
        guidance += (
            f"- Average sentence length: {analysis.get('avg_sentence_length', 0):.1f} words\n"
        )
    ...
    def _compare_analyses(
        ...
        """
        comparison = {
            "tone_match": sample_analysis.get("detected_tone")
            == generated_analysis.get("detected_tone"),
            "tone_sample": sample_analysis.get("detected_tone"),
            "tone_generated": generated_analysis.get("detected_tone"),
            "style_match": sample_analysis.get("detected_style")
            == generated_analysis.get("detected_style"),
