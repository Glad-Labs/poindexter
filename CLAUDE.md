...
- 100 live posts on gladlabs.io (269 posts total; 1,679 pipeline_tasks across all generation runs)
- 313 Python files under `src/cofounder_agent/services/` (~291 substantive after `__init__.py` stubs; down from ~455 after the 2026-05-08+09+16 cleanup passes). The "load-bearing services" table below covers the services on a critical execution path — `flows/content_generation.py`, `worker_service.py`, and `auto_publish_gate.py` were missing from prior versions of the table.
...
- 16,932 embeddings across posts / issues / audit / memory / brain / claude_sessions (retention prunes claude_sessions/brain vectors, so this count drifts)
...
`auto_publish` / `multi_model_qa`); `services/publish_service.py:982`
...
The graph_def is **37 nodes** — 10 `stage.*` + 11 `content.*` + 12 `qa.*` + 3 `seo.*` + 1 `atoms.approval_gate` — run as a linear chain. (It was 21 at the #355 cutover; #362 decomposed the coarse `generate_content` / `replace_inline_images` / `generate_seo_metadata` / `finalize_task` stages into fine-grained `content.*` + `seo.*` atoms, #363 added `draft_gate`, and the QA block grew from 5 to 12 rails.) Authoritative order is `services/canonical_blog_spec.py::CANONICAL_BLOG_GRAPH_DEF`:

1. `verify_task` → **writer block** 2. `content.generate_draft` → 3. `content.generate_title` → 4. `content.check_title_originality` → 5. `content.normalize_draft` → 6. `draft_gate` (`atoms.approval_gate`, seeded disabled) → 7. `writer_self_review` → 8. `resolve_internal_link_placeholders` → 9. `quality_evaluation` (pattern-based scorer) → 10. `url_validation` → **image block** 11. `content.plan_image_markers` → 12. `content.generate_images` → 13. `content.inject_images` → 14. `source_featured_image` → 15. `caption_images` (vision re-caption) → **QA-rail block (16–27)** `qa.programmatic` → `qa.critic` → `qa.deepeval` → `qa.guardrails` → `qa.ragas` → `qa.vision` → `qa.topic_delivery` → `qa.citations` → `qa.consistency` → `qa.self_consistency` → `qa.web_factcheck` → `qa.aggregate` → **SEO block (28–30)** `seo.generate_title` → `seo.generate_description` → `seo.extract_keywords` → 31. `generate_media_scripts` → 32. `generate_video_shot_list` → 33. `capture_training_data` → **finalize block (34–37)** `content.compile_meta` → `content.persist_task` → `content.record_pipeline_version` → `content.evaluate_auto_publish` (awaiting_approval or auto-publish if score ≥ threshold).

**The `cross_model_qa` stage is deleted** (`services/stages/cross_model_qa.py`, 733 LOC; ~1,220 with its two test files). QA now runs as 12 composable `qa.*` atoms in `modules/content/atoms/` (`qa.programmatic` / `qa.critic` / `qa.deepeval` / `qa.guardrails` / `qa.ragas` / `qa.vision` / `qa.topic_delivery` / `qa.citations` / `qa.consistency` / `qa.self_consistency` / `qa.web_factcheck`) that delegate to the existing `MultiModelQA` rail methods and append to the `qa_rail_reviews` PipelineState channel; `qa.aggregate` combines them into the gate decision — on reject it does the DB writes the old stage did (via `modules/content/atoms/_qa_persist.py`) and halts the graph, on approve it promotes `quality_score` and populates `qa_reviews`. `multi_model_qa.py` stays — it's the rail library the atoms delegate to. Rail advisory is DB-driven: each rail reads `qa_gates.<rail>.required_to_pass` (via `MultiModelQA._mark_advisory_if_configured`), so graduating a rail (poindexter#454) is still `qa_gates.<rail>.required_to_pass=true`, now effective on the graph_def path. (The legacy `cross_model_qa` rewrite loop was **not** ported — `qa.aggregate` halts on reject rather than auto-rewriting.)
...
**STATUS 2026-06-10: all currently DISABLED.** The nine sessions below — plus the Startup and Watchdog tasks — were disabled in Windows Task Scheduler (2026-06-09) as cost-control ahead of Anthropic API billing going to full rate 2026-06-15; the definitions remain in place (re-enable via `.\claude-sessions.ps1 -Install`). NOTE: `claude-md-sync` is among the disabled, so the DB-derived counts above no longer auto-update — refresh them manually until the agents are re-enabled.
