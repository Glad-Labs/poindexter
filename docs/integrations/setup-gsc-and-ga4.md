...
| Symptom                                                                                     | Cause                                                                                     | Fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tap exited 1` with "invalid_grant" / "Token has been expired or revoked"                   | refresh_token revoked or stale — **both taps die together, they share one token**         | re-run **Step 2** (the helper requires `--client-id` and `--client-secret`; a bare `python scripts/google-oauth-setup.py` just prints an argparse error), then `poindexter settings set google_oauth_refresh_token "<new_token>" --secret` — no row edit needed. If the helper reports no refresh_token, revoke the app at <https://myaccount.google.com/permissions> first: Google issues one only on FIRST consent. The taps pick the new token up on their next fire (they read it per-run); force one with `docker exec poindexter-worker python3 -m poindexter.cli taps run gsc_main`. |
| Helper dies with `OSError: [Errno 98] Address already in use` before the browser opens      | something else holds the loopback port                                                    | Nothing to fix since 2026-09-15 — the helper now binds an OS-assigned free port by default. On an older copy, pass `--port <free port>`. The OAuth client is a **Desktop app**, and Google matches loopback redirects on the address only, ignoring the port, so any free port works and none needs registering.                                                                                                                                                                                                                                                                            |
