...
class _Platform:
    """Minimal capability handle — only ``config`` is exercised here.

    Seam 1 Wave 3e (#667): verify_task reads ``preview_base_url`` via
    ``context['platform'].config.get`` instead of ``context['site_config']``,
    so tests seed the handle rather than the raw SiteConfig.
    """
...
class TestVerifyTaskPreviewMint:
    async def test_surfaces_preview_token_and_url_in_updates(self):
        ctx = {"task_id": "t1", "database_service": _Db(), "platform": _Platform(_Cfg())}
    ...
    async def test_reuses_existing_token(self):
        ...
        ctx = {
            ...
            "platform": _Platform(_Cfg()), "preview_token": "fixedtoken123456",
        }
        result = await VerifyTaskStage().execute(ctx, {})
    ...
    async def test_uses_configured_base_url(self):
        ...
        ctx = {"task_id": "t1", "database_service": _Db(), "platform": _Platform(_RemoteCfg())}
        result = await VerifyTaskStage().execute(ctx, {})
    ...
    async def test_falls_back_to_default_base_when_no_platform(self):
        ...
        assert result.context_updates["preview_url"] == f"http://localhost:8002/preview/{token}"
    ...
    async def test_no_token_when_task_missing(self):
        ...
        ctx = {"task_id": "t1", "database_service": _Db(found=False), "platform": _Platform(_Cfg())}
        result = await VerifyTaskStage().execute(ctx, {})
