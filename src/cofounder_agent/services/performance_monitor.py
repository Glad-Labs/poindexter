...
class PerformanceMonitor:
    ...
    def __init__(self, firestore_client=None):
        ...
        logger.info(
            "Performance monitor initialized", firestore_available=firestore_client is not None
        )
    ...
    async def track_operation(self, operation_name: str, metadata: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            # Operation failed
            duration = time.time() - start_time
            await self._record_operation_error(
                operation_id, operation_name, duration, str(e), metadata
            )
            raise
        finally:
            # Clean up tracking
    ...
    async def track_command_processing(
        self,
        command: str,
        ...
        try:
            ...
            avg_processing_time = (
                self.total_processing_time / self.command_count if self.command_count > 0 else 0
            )
            error_rate = (
                (self.error_count / self.command_count) * 100 if self.command_count > 0 else 0
            )
    ...
    async def track_database_operation(
        ...
        try:
            ...
            metrics_data = {
                ...
                "timestamp": datetime.utcnow().isoformat(),
                "session_metrics": {"total_db_operations": self.database_operation_count},
            }

            logger.info(
                "Database operation tracked",
    ...
    async def track_pubsub_message(
        self,
        message_type: str,
        topic: str,
        ...
        try:
            ...
            metrics_data = {
                ...
                "success": success,
                "timestamp": datetime.utcnow().isoformat(),
                "session_metrics": {"total_pubsub_messages": self.pubsub_message_count},
            }

            logger.info(
                "Pub/Sub message tracked",
        ...
        try:
            summary = {
                ...
                "session_metrics": {
                    ...
                    "total_errors": self.error_count,
                    "error_rate_percent": (
                        (self.error_count / self.command_count) * 100
    ...
    async def _record_operation_error(
        self,
        operation_id: str,
        operation_name: str,
        duration: float,
    ...
    async def _store_metrics(self, metrics_data: Dict[str, Any]):
        ...
        try:
            if self.firestore_client:
                ...
                logger.debug(
                    "Metrics stored", doc_id=doc_ref.id, metric_type=metrics_data.get("metric_type")
                )
        except Exception as e:
    ...
    async def _get_historical_metrics(self, hours: int) -> Dict[str, Any]:
        ...
        try:
            ...
            query = (
                self.firestore_client.db.collection("metrics")
                .where("timestamp", ">=", start_time.isoformat())
                .where("timestamp", "<=", end_time.isoformat())
                .order_by("timestamp")
                .limit(1000)
            )  # Limit to prevent large queries
    ...
    def get_session_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_response_time": (
                self.total_processing_time / self.command_count if self.command_count > 0 else 0.0
            ),
