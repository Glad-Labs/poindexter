...
class CreateBlogPostRequest(BaseModel):
    ...
    enhanced: bool = Field(False, description="Use SEO enhancement")
...
class PublishDraftRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {"example": {"target_environment": "production"}}
...
class GenerateAndPublishRequest(BaseModel):
    ...
    topic: str = Field(
        ..., min_length=3, max_length=200, description="Topic for content generation (3-200 chars)"
    )
    ...
    keywords: Optional[List[str]] = Field(None, description="SEO keywords (max 15)")
    style: Optional[ContentStyle] = Field(
        ContentStyle.EDUCATIONAL, description="Content style (EDUCATIONAL/INFORMATIVE/...)"
    )
    tone: Optional[ContentTone] = Field(
        ContentTone.PROFESSIONAL, description="Content tone (PROFESSIONAL/CASUAL/...)"
    )
    ...
    category: Optional[str] = Field(
        None, min_length=1, max_length=100, description="Category ID or name (1-100 chars)"
    )
    tags: Optional[List[str]] = Field(None, description="Tag names (max 10)")
