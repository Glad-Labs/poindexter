...
# surface — the polished operator UI. OSS installs keep the backend APIs it
# consumes (/api/logs + /api/traces stay public) but not the UI itself. main.py
# mounts it PRESENCE-BASED (utils/operator_console.py::mount_operator_console),
# so stripping the directory leaves the OSS backend booting cleanly with no
# /console route rather than crashing on a missing StaticFiles directory. Its
# in-tree Jest tests live under console/js/__tests__/ and ride the directory;
# the mirror already strips the frontend CI (jest-unit.yml / playwright-e2e.yml),
# so nothing runs them there.
git rm -r --cached --quiet src/cofounder_agent/console/ 2>/dev/null || true
...
git rm --cached --quiet src/cofounder_agent/poindexter/services/operator_overrides.py 2>/dev/null || true  # private operator overlay — custom local Ollama tags (gemma-4-31B-it-qat / glm-4.7-5090 / gemma-4-E2B-Q2) + personal settings (voice persona, exact GPU). The PUBLIC apply hook (settings_defaults.apply_operator_overrides) no-ops when this module is absent, so OSS installs use the public defaults.
git rm --cached --quiet src/cofounder_agent/poindexter/services/operator_leak_patterns.py 2>/dev/null || true  # private operator overlay — operator-identity scrub regexes (real name / home paths / Tailnet IP / GitHub handle). rag_scrub imports it via a no-op-when-absent hook, so OSS installs get generic scrub only. MUST stay stripped — carries the operator-name literal.
...
git rm -r --cached --quiet src/cofounder_agent/poindexter/modules/finance/ 2>/dev/null || true
...
git rm --cached --quiet src/cofounder_agent/poindexter/services/taps/claude_code_sessions.py 2>/dev/null || true  # taps ~/.claude/projects/*.jsonl — Matt's actual conversations
...
git rm --cached --quiet .github/workflows/jest-unit.yml 2>/dev/null || true  # frontend Jest for web/public-site, same as playwright-e2e above — on poindexter `npm run test:ci` fails every sync with "No workspaces found: --workspace=web/public-site". Non-required check testing a frontend that doesn't exist on the mirror. NB: jest-unit.yml carries an `if: github.repository == 'Glad-Labs/glad-labs-stack'` guard meant to skip it on the mirror, but the cosmetic glad-labs-stack→poindexter substitution below INVERTS that guard (it becomes `== poindexter` → runs), so the file must be stripped outright, not relied on to self-skip (#1481).
git rm --cached --quiet .github/workflows/console-unit.yml 2>/dev/null || true  # operator-console JS gate — runs `npm run test:console` against src/cofounder_agent/console/, which is now a Pro-tier overlay stripped from the mirror (see the console block above). Identical self-skip-inversion trap to jest-unit.yml: its `if: github.repository == 'Glad-Labs/glad-labs-stack'` guard gets flipped to `== poindexter` by the cosmetic substitution below and would RUN on the mirror against a console that no longer exists. Strip outright (#1481 pattern).
git rm --cached --quiet .github/workflows/console-contract-drift.yml 2>/dev/null || true  # nightly live-worker drift check for the console contract net (src/cofounder_agent/console/js/__tests__/contracts/) — the console subtree it records against is a Pro-tier overlay stripped from the mirror, so this is the console-unit.yml case exactly. Third instance of the #1481 self-skip-inversion trap: its `if: github.repository == 'Glad-Labs/glad-labs-stack'` guard is flipped to `== poindexter` by the cosmetic substitution below, so the repo gate PASSES on the mirror and only `vars.CI_RUNNER != ''` keeps it dormant (poindexter has no Actions variables today — one variable away from firing against a console that isn't there). It also mints RELEASE_PLEASE_APP_* and needs CONSOLE_CONTRACT_CLIENT_ID/_SECRET, none of which exist on the mirror. Strip outright.
git rm --cached --quiet .github/workflows/runner-healthcheck.yml 2>/dev/null || true  # self-hosted CI runner control loop — guarded to glad-labs-stack (skipped/dead on the mirror), leaks the internal repo name + operator host, and needs the CI_RUNNER_ADMIN_PAT secret absent on poindexter. Same rationale as release-please / regen-app-settings-doc above. Paired with the stripped docs/operations/self-hosted-ci-runner.md runbook.
...
git rm --cached --quiet src/cofounder_agent/tests/unit/scripts/test_check_public_mirror_safety_distribution_target.py 2>/dev/null || true
