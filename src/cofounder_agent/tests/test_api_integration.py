...
from conftest import (
    TEST_CONFIG, mock_api_responses, performance_monitor, 
    test_utils, run_with_timeout, pytest_marks
)
...
class TestWebSocketFunctionality:
    ...
    async def test_websocket_real_time_updates(self, ws_client):
        ...
        assert any(t in ["metrics_update", "business_data", "chat_response"] for t in response_types)
...
class TestAPIIntegration:
    ...
    async def test_complete_workflow_via_api(self, api_client):
        ...
        for i, response in enumerate(responses):
            assert response["status"] in [200, 503], f"Step {i+1} failed with status {response['status']}"
