...
def _build_tts(site_config: Any, tts_voice_override: str | None) -> Any:
    ...
    if mode == "sidecar":
        ...
        import pipecat.services.openai.tts as _pipecat_oai_tts
        from pipecat.services.openai.tts import OpenAITTSService
        # pipecat 1.1.0 gates TTS via VALID_VOICES, a module-level dict (not a
        # class attribute). Speaches Kokoro voices (bf_emma/bf_isabella/…) aren't
        # in OpenAI's catalog, so run_tts raises KeyError on every call. Patch the
        # module-level dict to register the voice as an identity pass-through.
        _valid_voices = getattr(_pipecat_oai_tts, "VALID_VOICES", None)
        if isinstance(_valid_voices, dict):
            _valid_voices.setdefault(voice, voice)
