...
| RSS Feed             | WORKING            | Blog feed.xml + JSON Feed 1.1. Feed dedup via `distributed_at` + cutoff date. Direct Bluesky + Mastodon adapters replaced dlvr.it (GH-36). dlvr.it subscription is paused — see X/Twitter row for the intended X cross-post path when/if it resumes. |
...
| X/Twitter Posting    | SCAFFOLD           | Generates post text (280 chars) + feed.xml carries the post. No auto-cross-post today. X API is $100/mo (not worth it); dlvr.it RSS bridge is the intended path when/if the subscription resumes. Manual copy-paste in the meantime.                 |
