...
from services.phase_registry import PhaseRegistry, PhaseDefinition
from services.phase_mapper import PhaseMapper, build_full_phase_pipeline, PhaseMappingError
...
class WorkflowExecutor:
    ...
    def __init__(
        ...
        mapper: Optional[PhaseMapper] = None
    ):
        ...
        if phase_index > 0:
            for target_key, source_key in phase_mapping.items():
                if target_key not in inputs:  # Don't override user inputs
                    ...
                    prev_phase_name = list(previous_outputs.keys())[-1] if previous_outputs else None
        ...
        for i, phase in enumerate(phases):
            if isinstance(phase, WorkflowPhase):
                ...
                if hasattr(phase, 'name'):
                    normalized.append(WorkflowPhase(
                        index=i,
                        name=phase.name,
