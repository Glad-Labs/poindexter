...
def test_render_pipeline_section_matches_real_spec(mod: ModuleType) -> None:
    ...
    body = "\n".join(lines)
    assert lines[0] == "## Content pipeline (`canonical_blog` graph_def) — 47 nodes"
...
def test_build_document_has_all_sections(mod: ModuleType) -> None:
    ...
    assert "## Content pipeline (`canonical_blog` graph_def) — 47 nodes" in doc
