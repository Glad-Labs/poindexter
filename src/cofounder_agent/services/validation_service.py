...
class InputValidator:
    ...
    def validate_string(
        ...
        if not allow_sql and cls._contains_sql_injection(value):
            raise ValidationError(f"{field_name} contains invalid SQL keywords or syntax")

        # Check for XSS
        if not allow_html and cls._contains_xss(value):
            raise ValidationError(f"{field_name} contains invalid HTML or JavaScript")
    ...
    def validate_dict(
        ...
        if size > max_size:
            raise ValidationError(f"{field_name} exceeds maximum size of {max_size} bytes")

        # Check nesting depth
        if cls._get_dict_depth(value) > max_depth:
            raise ValidationError(f"{field_name} nesting is too deep (max {max_depth} levels)")

        # Check allowed keys
        if allowed_keys:
            invalid_keys = set(value.keys()) - set(allowed_keys)
            if invalid_keys:
                raise ValidationError(f"{field_name} contains invalid keys: {invalid_keys}")
