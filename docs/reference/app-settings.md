...
| `deepeval_enabled`                                                   | `true`                                                                         |                | Master switch for the DeepEval reviewer rails (brand-fabrication, g-eval, faithfulness).                                  |
| `deepeval_g_eval_criterion`                                          | `The output is well-grounded ...`                                              |                | Criterion the DeepEval g-eval judge uses to grade the post (#329 sub-issue 1).                                            |
| `deepeval_judge_model`                                               | `glm-4.7-5090`                                                                 |                | LLM used by the DeepEval g-eval + faithfulness reviewers.                                                                 |
| `deepeval_threshold_faithfulness`                                    | `0.8`                                                                          |                | Pass threshold for the DeepEval FaithfulnessMetric reviewer (0–1).                                                        |
| `deepeval_threshold_g_eval`                                          | `0.7`                                                                          |                | Pass threshold for the DeepEval g-eval LLM-judge reviewer (0–1).                                                          |
| `guardrails_competitor_list`                                         | ``                                                                             |                | CSV of competitor brand names; flag any post body that mentions one. Empty = no enforcement (#329 sub-issue 3).           |
| `guardrails_enabled`                                                 | `true`                                                                         |                | Master switch for the guardrails-ai reviewer rails (brand-fab + competitor-mention).                                      |
| `ragas_enabled`                                                      | `false`                                                                        |                | Master switch for the Ragas RAG-quality reviewer. Default off — each call costs ~6K judge tokens (#329 sub-issue 2).      |
| `rag_engine_enabled`                                                 | `false`                                                                        |                | Master switch routing MemoryClient.search through the LlamaIndex BaseRetriever (#329 sub-issue 4). Off = legacy pgvector. |
| `default_template_slug`                                              | ``                                                                             |                | Lane C cutover (#355): when set, every new task without a caller-supplied template_slug routes through this template.     |
