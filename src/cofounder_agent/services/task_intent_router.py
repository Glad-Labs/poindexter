...
class TaskIntentRouter:
    ...
    async def route_user_input(
        ...
        intent_match = await self.nlp_recognizer.recognize_intent(user_input, context=user_context)
        ...
        task_type = self.intent_to_task_type.get(intent_match.intent_type, "generic")

        # Step 3: Extract and normalize parameters
        extracted_params = self._normalize_parameters(intent_match.parameters, task_type)

        # Step 4: Determine subtasks based on task type and parameters
        suggested_subtasks = self._determine_subtasks(task_type, extracted_params)

        # Step 5: Decide if user confirmation needed
        requires_confirmation = self._should_confirm(
            intent_match.confidence, task_type, extracted_params
        )

        # Step 6: Determine execution strategy
        execution_strategy = self._determine_execution_strategy(task_type, extracted_params)
    ...
    def _normalize_parameters(self, params: Dict[str, Any], task_type: str) -> Dict[str, Any]:
    ...
    def _determine_subtasks(self, task_type: str, parameters: Dict[str, Any]) -> List[str]:
    ...
    def _should_confirm(
        self, confidence: float, task_type: str, parameters: Dict[str, Any]
    ) -> bool:
    ...
    def _determine_execution_strategy(self, task_type: str, parameters: Dict[str, Any]) -> str:
    ...
    def plan_subtasks(
        ...
        for i, subtask_stage in enumerate(subtasks):
            ...
            required_inputs = self._get_required_inputs(
                subtask_stage, i > 0  # First stage? Input is parameters, not parent output
            )
    ...
    def generate_execution_plan_summary(
        self, intent_request: TaskIntentRequest, subtask_plans: List[SubtaskPlan]
    ) -> Dict[str, Any]:
        """
        ...
        """
        total_duration_ms = sum(plan.estimated_duration_ms for plan in subtask_plans)
