...
- **`template_runner.py`** — LangGraph-backed dynamic-pipeline orchestrator. **Primary pipeline path** (`canonical_blog` template is the prod default; `default_template_slug=canonical_blog` on prod). Drives `canonical_blog` (13 nodes) + `dev_diary` (4 nodes) + future architect-composed pipelines. Postgres checkpointer enabled via `template_runner_use_postgres_checkpointer=true`. Templates registered in `services/pipeline_templates/__init__.py`.
