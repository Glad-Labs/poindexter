...
- **Minor release 1.2.0.** Migrates 5 content-agent env-vars to `app_settings` (see Updates below) on top of `v1.1.0`. Pin to [`v1.2.0`](https://github.com/Glad-Labs/poindexter/releases/tag/v1.2.0) for the latest stable upgrade target. ([#301](https://github.com/Glad-Labs/poindexter/pull/301))
...
- **5 more content-agent env-vars moved to `app_settings`.** `SERPER_API_KEY`, `LOCAL_LLM_API_URL`, `LOCAL_LLM_MODEL_NAME`, `MAX_LOG_SIZE_MB`, and `MAX_LOG_BACKUP_COUNT` are now read from the `app_settings` DB table first and only fall back to environment variables for back-compat. Operators can retune log rotation, swap the Ollama URL/model, or rotate the Serper search key with `poindexter settings set <key> <value>` instead of editing docker-compose and restarting the worker. `serper_api_key` is stored with `is_secret=true` so the schema is ready for encrypted-at-rest reads via `site_config.get_secret(...)`. Migration `0116_seed_content_agent_app_settings.py` seeds the rows from your current env-vars at apply time, so existing deployments keep their tuned values. Existing `SERPER_API_KEY` / `LOCAL_LLM_*` env-vars on the worker container continue to work unchanged. ([#300](https://github.com/Glad-Labs/poindexter/pull/300))
