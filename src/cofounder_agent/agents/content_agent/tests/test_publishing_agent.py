...
def mock_strapi_client():
    ...
    mock_client = Mock()
    mock_client.create_post.return_value = ("test-id-123", "https://strapi.example.com/posts/test-id-123")
    return mock_client
...
class TestImagePlaceholderReplacement:
    ...
    def test_handles_post_without_images(self, publishing_agent):
        ...
        post = BlogPost(
            title="No Images",
            raw_content="Content without images"
        )
...
class TestPublishing:
    ...
    def test_run_publishes_to_strapi(self, publishing_agent, sample_blog_post_with_images, mock_strapi_client):
    ...
    def test_run_converts_markdown_to_blocks(self, publishing_agent, sample_blog_post_with_images, mock_strapi_client):
        """Test that markdown is converted to Strapi blocks"""
        mock_blocks = [{"type": "paragraph", "children": [{"text": "test"}]}]
        
        with patch('agents.publishing_agent.markdown_to_strapi_blocks', return_value=mock_blocks) as mock_convert:
            result = publishing_agent.run(sample_blog_post_with_images)
        
        mock_convert.assert_called_once()
        assert result.body_content_blocks == mock_blocks
    
    def test_run_processes_images_before_publishing(self, publishing_agent, sample_blog_post_with_images, mock_strapi_client):
        """Test that images are processed before publishing"""
        with patch('agents.publishing_agent.markdown_to_strapi_blocks', return_value=[]) as mock_convert:
...
class TestErrorHandling:
    ...
    def test_handles_strapi_error(self, publishing_agent, sample_blog_post_with_images, mock_strapi_client):
    ...
    def test_handles_no_strapi_id_returned(self, publishing_agent, sample_blog_post_with_images, mock_strapi_client):
    ...
    def test_handles_markdown_conversion_error(self, publishing_agent, sample_blog_post_with_images, mock_strapi_client):
        """Test handling of markdown conversion errors"""
        with patch('agents.publishing_agent.markdown_to_strapi_blocks', side_effect=Exception("Conversion error")):
            result = publishing_agent.run(sample_blog_post_with_images)
...
class TestFullPublishingWorkflow:
    ...
    def test_complete_publishing_pipeline(self, publishing_agent, sample_blog_post_with_images, mock_strapi_client):
        """Test complete end-to-end publishing"""
        with patch('agents.publishing_agent.markdown_to_strapi_blocks', return_value=[{"type": "paragraph"}]):
            result = publishing_agent.run(sample_blog_post_with_images)
...
class TestPublishingAgentPerformance:
    ...
    def test_publishing_performance(self, publishing_agent, sample_blog_post_with_images, mock_strapi_client):
