...
1. UNIT TESTS
   ─────────────
   [ ] Test prompt_manager.py directly
       - Test get_prompt() with valid keys
       - Test get_prompt() with missing variables
       - Test list_prompts()
       - Test export_prompts_as_json()

   [ ] Test each agent/service
       - Run existing unit tests for that service
       - Verify prompts are fetched correctly
       - Check output format unchanged

2. INTEGRATION TESTS
   ──────────────────
   [ ] End-to-end blog generation
       - Creative agent uses prompts correctly
       - QA agent evaluates with new prompts
       - SEO metadata generated properly

   [ ] Social media generation
       - Research task uses new prompts
       - Creative task generates correct format

   [ ] Financial/Market analysis
       - Prompts execute without errors
