...
- 110 live posts on gladlabs.io (279 posts total; 1,798 pipeline_tasks across all generation runs)
- 371 Python files under `src/cofounder_agent/services/` (~291 substantive after `__init__.py` stubs; down from ~455 after the 2026-05-08+09+16 cleanup passes). The "load-bearing services" table below covers the services on a critical execution path — `flows/content_generation.py`, `worker_service.py`, and `auto_publish_gate.py` were missing from prior versions of the table.
...
- **Migration files** — after the **Phase G squash (2026-07-11)** the tree holds a **single `.py` file**: `0000_baseline.py` (plus `0000_baseline.schema.sql` + `0000_baseline.seeds.sql`). Phase G folded the Phase F baseline (2026-06-22) plus all 42 post-baseline migrations (`20260622_200222_*` through `20260711_202250_*`) into a fresh baseline — **true baseline-only, no surviving convergence migration**. Phase F had to keep `drop_pipeline_tasks_category` as a survivor (a baseline only `CREATE TABLE IF NOT EXISTS`, so it can add but never drop a column prod still carried); by Phase G prod is verified current through `20260711_202250_*` (every folded file is in `schema_migrations`), so the column is already dropped everywhere and the survivor folds away. Orphan `schema_migrations` rows for the 42 deleted files are harmless — the runner skips by filename and never reconciles in reverse. The baseline was regenerated fold-forward from a throwaway DB that ran the full pre-squash chain and **verified byte-for-byte against a chain `pg_dump` (schema identical + all 11 seed tables md5-identical)**. Notable folded deltas: new `clock_skew_samples`/`live_activity`/`social_post_drafts`/`remediation_rules`/`affiliate_links`/`affiliate_link_clicks` tables, `page_views` bot-flag columns + `page_views_human` view, `audit_log.source`→`text`; `pipeline_templates` 5→6 (+`image_rebuild`), baseline-seeded non-secret `app_settings` nets to 691, `qa_gates` 19 (the 2026-07-24 `title_coherence` row joined the squash-time 18), `retention_policies` 31. New schema changes still go in fresh `YYYYMMDD_HHMMSS_<slug>.py` files; the runner sorts `0000_baseline.py` first because `0` < `2` lexically.
- 12 Grafana dashboards (Mission Control / Pipeline / Cost / Observability / System Health / Integrations / QA Rails / Findings / Experiments & Dry-Run / Database / Hardware & Power / SEO Harvest — Revenue parked 2026-07-01 in `infrastructure/grafana/dashboards-parked/` until `revenue_events` has real writes; each board carries a one-line CHARTER in its description), 17 static Prometheus alert rules in `infrastructure/prometheus/alerts/*.yml` plus DB-rendered + Grafana-managed rules; Pyroscope app-profiles ship from worker/brain/voice agents under `service_name` tags (poindexter#406). The dashboard set was restructured 2026-06-03 (poindexter#654): the dev-blog-only Auto-Publish Gate board was folded into the new **Experiments & Dry-Run** board, and the 75-panel System Health "junk drawer" was split — its Postgres internals → new **Database** board, its GPU/power panels → new **Hardware & Power** board (GPU single-sourced to Prometheus `nvidia_gpu_*`, #653) — leaving a slim ~45-panel System Health. The static repo rules are 17 alerts in 4 active `infrastructure/prometheus/alerts/*.yml` files (deadmans-switch / infrastructure / observability-sidecars / prometheus-alertmanager); `postgres-connections.yml` is a 5th file kept as a comment-only placeholder — its connection rules were migrated to DB-rendered (GH-92, for app_settings-tunable thresholds), as were the disk-space and brain-db-size rules (the #735 observability sweep). On top of the static set, the DB-sourced rules (20 defaults across `poindexter-business` / `poindexter-content` / `poindexter-infra` / `poindexter-infrastructure` / `poindexter-postgres-connections`; worker-gauge rules with a long `for:` bridge deploy-restart scrape holes via `last_over_time`/`max_over_time` — see the restart-gap policy comment atop `DEFAULT_RULES` in `services/prometheus_rule_builder.py`) are rendered into `rules/*.yml` by `RenderPrometheusRulesJob` every 5 min — so the live count is the 17 static repo rules plus whatever the DB currently seeds.
- ~11,440 test functions across 918 test files under `tests/unit` (+ 16 integration + 17 integration_db) — last recorded full-green nightly 2026-06-09 (10,352 passed, 84 skipped, 1 xfailed, 0 failures, **0 collection errors**); the suite has grown ~1,200 test functions since, so re-run to refresh the pass tally. A handful still skip in-container due to the `Path(__file__).parents[5]` path-depth quirk and run on host; the prior "~21 collection errors" line described a since-fixed problem.
- 1,242 app_settings keys live on prod (was ~1,090; count drifts as keys are added) — the baseline seeds the non-secret defaults; secrets get configured per-operator via `poindexter setup` + bootstrap.toml. Three sources seed the table (`settings_defaults.py` ~734, `0000_baseline.seeds.sql` ~692, `brain/seed_app_settings.json` 81 free-tier); the overlapping keys must agree — enforced by `scripts/ci/settings_seed_value_drift_lint.py`. (The 4 `cost_tier.{free,budget,standard,premium}.model` mappings were removed in PR #1907 — model selection is now per-step `*_model` pins. Cost-guard key rename 2026-05-27 closed a silent fallthrough on `daily_spend_limit_usd` / `monthly_spend_limit_usd` — see #598.)
...
- 5 declarative-data-plane tables (`external_taps` / `retention_policies` / `webhook_endpoints` / `publishing_adapters` / `qa_gates`) feeding the integrations handler registry's 14 handlers across 5 surfaces (`tap` / `retention` / `webhook` / `outbound` / `publishing`). A 6th `services.declarative_config_service`-managed table, `alert_rules` (`poindexter alerts`), joined 2026-07-13 (poindexter#848) — CRUD'd through the same generic service but synced by the bespoke `brain/alert_sync.py` loop rather than the integrations handler registry, so it sits outside that 5-surface count.
- 22,259 embeddings across posts / issues / audit / memory / brain / claude_sessions (retention prunes claude_sessions/brain vectors, so this count drifts)
...
- **Grafana (self-hosted):** http://localhost:3000 (or http://100.81.93.12:3000 from the tailnet) — 12 dashboards (Revenue parked 2026-07-01). Grafana Cloud was retired 2026-05-03; the local Docker container (poindexter-grafana) is the only Grafana now. Local Prometheus scrapes windows_exporter + nvidia-smi-exporter directly; Alloy was the Cloud shipper and is no longer used. **The operator console (`/console/`) now embeds the deep-telemetry surfaces under a Telemetry tab** — Loki logs (worker proxy `GET /api/logs`), Langfuse LLM traces (`GET /api/traces`, full waterfall deeplinks out to Langfuse), and embedded Grafana `/d-solo` history + Database panels — so day-to-day operation no longer requires opening Grafana directly (embedding via `GF_SECURITY_ALLOW_EMBEDDING=true` + anonymous Viewer in the compose `grafana` service).
- **Dashboards:**
...
`0000_baseline.py` runs first because `0` < `2`.
