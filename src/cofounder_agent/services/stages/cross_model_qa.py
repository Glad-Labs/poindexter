...
class CrossModelQAStage:
    ...
    async def execute(
        ...
        # DETERMINISTIC_COMPOSITOR bypass branch deleted 2026-05-28: the
        # dev_diary template (the only intended consumer) skips this
        # stage entirely via its 4-node graph (verify_task → narrate_bundle
        # → source_featured_image → finalize_task). No other niche uses
        # the compositor — that writer mode was deleted in the same
