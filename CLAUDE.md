...
- 311 Python files under `src/cofounder_agent/services/` (~298 substantive after migrations + `__init__.py` stubs; down from ~455 after the 2026-05-08+09+16 cleanup passes). The "load-bearing services" table below covers the services on a critical execution path — `flows/content_generation.py`, `worker_service.py`, and `auto_publish_gate.py` were missing from prior versions of the table.
...
- 8,748 Python unit tests across ~445 test files (some skipped in container due to host/container path-depth quirks at `Path(__file__).parents[5]` — works on host; collection picks up ~21 errors as of 2026-05-27, tracked for cleanup)
...
- **Dashboards:**
  ...
  - **System Health** — worker / brain / voice container health, plus the **Scheduled-publish queue** panel set (depth / next slot / past-due / upcoming-24h table, added 2026-05-25) for observability over the `poindexter schedule batch` + `poindexter publish-at` operator surface, and the **Approved-queue** panel set (panels 78/79, added 2026-05-31): an "Approved q" stat beside the scheduled-publish stats + an "Approved — awaiting publish" detail table over `pipeline_tasks.status='approved'` — the post-approval, pre-publish queue that was previously invisible (only `awaiting_approval` had a panel). These are the tasks that, once published, pick up media via the 4h backfill jobs.
...
- **LangGraph pipeline (Lane C, post-Stage-4):** `app_settings.default_template_slug='canonical_blog'`; the LangGraph `canonical_blog` template in `services/pipeline_templates/__init__.py` is the only production pipeline. The legacy chunked StageRunner path in `content_router_service.py` plus `plugins/stage_runner.py` itself were deleted 2026-05-16 (no remaining production caller). New stages go on the LangGraph template.
