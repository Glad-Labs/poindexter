...
- **Niche-aware topic discovery + RAG-backed writer modes.** Poindexter now treats your install as one of many possible niches instead of an evergreen-tech default. Seeded with a `glad-labs` niche so existing users get the new flow without configuration; create additional niches per-install to run the same engine on different editorial voices (real estate, fashion, finance, etc.). What's new for operators:
  - **Topic batches replace ad-hoc proposals.** A sweep pulls candidates from your internal corpus (past sessions, brain knowledge, audit log, git, prior decisions, memory) plus external feeds, ranks them with a hybrid embedding + LLM scorer against per-niche weighted goals (`TRAFFIC`, `EDUCATION`, `BRAND`, `AUTHORITY`, `REVENUE`, `COMMUNITY`, `NICHE_DEPTH`), and presents the top 5 for you to rank, edit the winner, and resolve into a real content task. Leftover candidates carry forward across cycles with 0.7 decay so a near-miss this week stays in the running next week.
  - **Four writer RAG modes, configurable per niche.** Pick `TOPIC_ONLY` (single RAG query, snippets dropped into the prompt), `CITATION_BUDGET` (writer must hit N internal citations), `STORY_SPINE` (structured outline pre-pass before the draft), or `TWO_PASS` (internal-context-only first draft, then external fact-augmentation in a bounded LangGraph state machine — capped at 3 revision loops so it can't run away). The Glad Labs niche ships on `TWO_PASS`. Tasks without a `writer_rag_mode` set fall back to the legacy generator, so pre-niche pipelines are unchanged.
  - **New operator surface.** CLI: `poindexter topics show-batch / rank-batch / edit-winner / resolve-batch / reject-batch` plus a `poindexter niche` subgroup. MCP: matching `topics_show_batch`, `rank_batch`, `edit_winner`, `resolve_batch`, `reject_batch` tools so Claude Code can drive the gate end-to-end. ([#278](https://github.com/Glad-Labs/poindexter/pull/278))

- **Code-block density gate for tech-tagged posts.** A new soft quality check warns when a post tagged as technical ships without enough fenced code blocks to actually demonstrate the technique it describes. The gate runs as part of the multi-model QA review and surfaces as a warning, not a hard block — operators can still approve prose-only tech posts (architecture overviews, postmortems) at their discretion. Five DB-tunable settings let you adjust or disable the check: `code_density_check_enabled` (master kill-switch), `code_density_tag_filter` (which tags trigger the check), `code_density_min_blocks_per_700w`, `code_density_min_line_ratio_pct`, and `code_density_long_post_floor_words`. ([#274](https://github.com/Glad-Labs/poindexter/pull/274))
- **Topic discovery is now niche-configurable.** Two more hardcoded filters that assumed an evergreen-tech editorial voice are now overridable per install via app settings, so non-tech niches (real estate, fashion, finance, etc.) can keep topics that were previously dropped or mis-bucketed. Set `topic_discovery_news_patterns` (JSON array of regex strings) to control which titles get rejected as "news/junk" — for example, a real-estate site can drop the lawsuit/election filter so legal-news topics flow through. Set `topic_discovery_category_searches` (JSON object mapping a category name to a list of search queries) to define your own classification buckets instead of the default tech/startup/hardware/gaming split. Both ship with empty defaults; an empty value falls back to the built-in tech list, so existing deployments are unchanged until you opt in. Bad regex or malformed JSON falls back to the built-in list and is logged rather than crashing the pipeline. ([#276](https://github.com/Glad-Labs/poindexter/pull/276))
- **Migration drift surfaced in `/api/health`.** The health endpoint now reports a `migrations` block under `components`, listing the most recent applied migration, how many migrations are pending, and a `drift: true` flag when any are unapplied. If drift is detected, the overall health status flips to `degraded`, so Uptime Kuma, the brain daemon, and any other external monitor will trip automatically when a deploy lands code without its schema. If the database pool is unavailable the block returns `status: unknown` instead of 500-ing the whole endpoint. ([#270](https://github.com/Glad-Labs/poindexter/pull/270))
- **Operator URL drift probe.** The brain daemon now runs a periodic check (every 15 minutes) that flags operator-facing URLs and IPs that have gone stale or unreachable — Grafana dashboard links, Tailscale IPs tracked in your install, and `*_url` keys in app settings (Prefect, Grafana, Loki, etc.). When a surface fails or drifts, you get a single Telegram notification per cycle with the exact fix, so a stale dashboard link or a renumbered VPN host shows up as an alert instead of a silent UI dead-end. See [`brain/README.md`](brain/README.md) for full details. ([#214](https://github.com/Glad-Labs/poindexter/issues/214), [#256](https://github.com/Glad-Labs/poindexter/pull/256))
