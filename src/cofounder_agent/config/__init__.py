"""Compatibility stub -- `config` now lives at `poindexter.config` (Glad-Labs/poindexter#1046, step 2).

Importing this installs the flat-import alias finder and replaces this module in
``sys.modules`` with the canonical package, so ``import config.x`` yields the SAME
object as ``import poindexter.config.x`` -- one module, two names (see
...
# `cofounder_agent.config` (the umbrella spelling the entry points use).
