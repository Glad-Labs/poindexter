...
- **Local voice agent service.** New `voice_agent` service runs an end-to-end real-time voice loop on your own hardware — Silero VAD → faster-whisper STT → local Ollama LLM → Kokoro TTS — with no cloud APIs or paid keys. Six `voice_agent_*` settings (LLM model, Ollama URL, TTS voice, speed, Whisper model, system prompt) are tunable at runtime via `poindexter settings set`, with sensible defaults seeded by migration. Run with `python -m services.voice_agent`. ([62459e2](https://github.com/Glad-Labs/poindexter/commit/39c8a22288d1c2da1a54973baf88093b763f2a1e))
- **`poindexter migrate` CLI.** New `migrate status`, `migrate up`, and `migrate down` subcommands let you inspect and apply schema migrations without restarting the worker container. `migrate status` lists every migration with its applied state (text or `--json`), `migrate up` applies pending migrations (with `--to <name>` for partial runs), and `migrate down` rolls back one or more migrations (with `--to`, `--all`, and `--yes` flags). ([#226](https://github.com/Glad-Labs/poindexter/pull/226))
...
- **Relicensed to Apache-2.0.** Poindexter is now distributed under the Apache License 2.0 (previously AGPL-3.0). Apache-2.0 includes an explicit patent grant, matches the rest of the modern AI/ML ecosystem, and removes the copyleft restriction for downstream users. The old "Commercial License" support tier is no longer relevant and has been dropped from `SUPPORT.md`. ([62459e2](https://github.com/Glad-Labs/poindexter/commit/62459e2))
- **Configurable `brand_keywords` for topic discovery.** The brand-relevance filter no longer hardcodes Glad Labs's AI/ML/gaming/PC keyword set. Any niche (gardening, dentistry, indie music, etc.) can override the keyword list via the `brand_keywords` `app_settings` row; the built-in defaults still apply when the override is empty, so existing deployments are unchanged. Rejection messages now point operators at the setting key instead of the source code. ([#216](https://github.com/Glad-Labs/poindexter/pull/216))
...
- **Fresh-database installs now apply all migrations cleanly.** Ten migrations failed against a fresh Postgres database because they assumed bootstrap SQL had already run. Production deployments were unaffected (init SQL runs first), but new local and CI installs would error out partway through. Affected migrations now defensively create or skip the tables and columns they need, so a fresh `docker compose up` reaches a consistent schema on first boot. ([#87](https://github.com/Glad-Labs/poindexter/pull/87))
- **Migration number collision resolved.** Two migrations shared the prefix `0093`. The `object_stores` migration has been renumbered to `0117` to disambiguate ordering and clean up audit tooling. The `qa_gates` migration retains `0093` because a downstream seed migration depends on its slot. No action required for existing installs. ([#86](https://github.com/Glad-Labs/poindexter/pull/86))
- **CLI authentication error points at the right setting.** When `poindexter` CLI commands hit a 401, the error message used to suggest checking `app_settings.api_auth_token` — an orphan row that nothing actually validates. The message now correctly points at `app_settings.api_token`, and a migration removes the dead `api_auth_token` row. ([#231](https://github.com/Glad-Labs/poindexter/pull/231))
