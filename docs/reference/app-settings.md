...
> 693 active rows across 56 categories. 5 stored encrypted via pgcrypto (`is_secret=true`); 1 additional values redacted as secret-shaped (defense-in-depth); 10 values redacted as operator-specific (Tailnet IPs, financial reality, etc.) so this file is safe to ship to the public OSS mirror.
...
- [image](#image) (12 keys)
- [integration](#integration) (4 keys)
- [integrations](#integrations) (11 keys)
...
| `location_state`                                                     | ``                                                                             |                | Operator location (US state code or country) — used for content date/timezone hints; set via `poindexter setup`          |
...
| `r2_public_url`                                                      | ``                                                                             |                | Public R2/S3 base URL — set via poindexter set-setting                                                                   |
| `ragas_enabled`                                                      | `true`                                                                         |                |                                                                                                                          |
...
| `topic_dedup_intra_batch_threshold`          | `0.65`                                     |                | Word-overlap ratio above which two candidates from the same scrape batch are treated as duplicates (internal tracker). Range... |
...
| `plugin_job_last_run_audit_published_quality`        | `1778260286` |                | Unix epoch of last fire for plugin job 'audit_published_quality' (auto-written by PluginScheduler)     |
| `plugin_job_last_run_auto_embed_posts`               | `1778292954` |                | Unix epoch of last fire for plugin job 'auto_embed_posts' (auto-written by PluginScheduler)            |
| `plugin_job_last_run_check_published_links`          | `1778260288` |                | Unix epoch of last fire for plugin job 'check_published_links' (auto-written by PluginScheduler)       |
| `plugin_job_last_run_crosspost_to_devto`             | `1778284865` |                | Unix epoch of last fire for plugin job 'crosspost_to_devto' (auto-written by PluginScheduler)          |
...
| `plugin_job_last_run_expire_stale_approvals`         | `1778260286` |                | Unix epoch of last fire for plugin job 'expire_stale_approvals' (auto-written by PluginScheduler)      |
...
| `plugin_job_last_run_morning_brief`                  | `1778223600` |                | Unix epoch of last fire for plugin job 'morning_brief' (auto-written by PluginScheduler)               |
| `plugin_job_last_run_noop`                           | `1778292950` |                | Unix epoch of last fire for plugin job 'noop' (auto-written by PluginScheduler)                        |
| `plugin_job_last_run_postgres_vacuum`                | `1778260287` |                | Unix epoch of last fire for plugin job 'postgres_vacuum' (auto-written by PluginScheduler)             |
| `plugin_job_last_run_reload_site_config`             | `1778297547` |                | Unix epoch of last fire for plugin job 'reload_site_config' (auto-written by PluginScheduler)          |
| `plugin_job_last_run_render_prometheus_rules`        | `1778297487` |                | Unix epoch of last fire for plugin job 'render_prometheus_rules' (auto-written by PluginScheduler)     |
...
| `plugin_job_last_run_run_dev_diary_post`             | `1778245200` |                | Unix epoch of last fire for plugin job 'run_dev_diary_post' (auto-written by PluginScheduler)          |
| `plugin_job_last_run_run_niche_topic_sweep`          | `1778296287` |                | Unix epoch of last fire for plugin job 'run_niche_topic_sweep' (auto-written by PluginScheduler)       |
| `plugin_job_last_run_sync_newsletter_subscribers`    | `1778296287` |                | Unix epoch of last fire for plugin job 'sync_newsletter_subscribers' (auto-written by PluginScheduler) |
| `plugin_job_last_run_sync_page_views`                | `1778296287` |                | Unix epoch of last fire for plugin job 'sync_page_views' (auto-written by PluginScheduler)             |
| `plugin_job_last_run_tune_publish_threshold`         | `1778260286` |                | Unix epoch of last fire for plugin job 'tune_publish_threshold' (auto-written by PluginScheduler)      |
...
| `plugin_job_last_run_verify_published_posts`         | `1778296288` |                | Unix epoch of last fire for plugin job 'verify_published_posts' (auto-written by PluginScheduler)      |
