...
Now generate the task JSON for the user's request:"""
    ...
    async def compose_from_request(
        ...
        try:
            ...
            logger.info(
                f"[Composer] Task composed: {task_definition.name} with {len(task_definition.steps)} steps"
            )
    ...
    def _validate_task_definition(self, task_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if errors:
            return {"valid": False, "error": "; ".join(errors), "confidence": 0.0}
        ...
        for i, step in enumerate(task_dict["steps"]):
            ...
            if cap_name not in valid_capability_names:
                ...
                warnings.append(
                    f"Available capabilities: {', '.join(sorted(valid_capability_names))}"
                )
        ...
        return {"valid": True, "error": None, "confidence": confidence, "warnings": warnings}
