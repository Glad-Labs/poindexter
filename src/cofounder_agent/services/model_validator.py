...
class ModelValidator:
    ...
    def __init__(self, available_models: Optional[Dict[str, ModelInfo]] = None):
        ...
        logger.info(f"🔧 ModelValidator initialized with {len(self.available_models)} available models")
    
    def set_available_models(self, models: List[str]) -> None:
        """Update the list of available models"""
        self.available_models = {model: ModelInfo(name=model, provider="unknown", available=True) 
                                 for model in models}
    ...
    def validate_model_selection(self, model_name: str) -> tuple[bool, Optional[str]]:
        ...
        if not self.is_model_available(model_name):
            ...
            return False, f"Model '{model_name}' is not available. Available models: {', '.join(available[:5])}..."
        
        return True, None
    
    def validate_models_by_phase(self, models_by_phase: Dict[str, str]) -> tuple[bool, Dict[str, str]]:
        """
        ...
        for phase, model in models_by_phase.items():
            ...
            if phase not in self.PIPELINE_PHASES:
                errors[phase] = f"Unknown phase '{phase}'. Valid phases: {', '.join(self.PIPELINE_PHASES)}"
                all_valid = False
                continue
    ...
    def estimate_cost_by_phase(self, models_by_phase: Dict[str, str], 
                                tokens_per_phase: Optional[Dict[str, int]] = None) -> float:
        """
