...
class CapabilityTasksService:
    ...
    async def list_tasks(
        ...
        """
        # Base query
        query = select("capability_tasks").where("capability_tasks.owner_id" == owner_id)
        ...
        count_query = (
            select("COUNT(*)")
            .select_from("capability_tasks")
            .where("capability_tasks.owner_id" == owner_id)
        )
        if active_only:
    ...
    async def update_task(
        ...
        update_query = (
            ...
            .where("capability_tasks.id" == task_id)
    ...
    async def persist_execution(
        ...
        self.db.execute(
            update("capability_tasks")
            .where("capability_tasks.id" == result.task_id)
            .values(
    ...
    async def list_executions(
        ...
        count_query = (
            select("COUNT(*)")
            .select_from("capability_executions")
            .where(
                and_(
                    "capability_executions.task_id" == task_id,
                    "capability_executions.owner_id" == owner_id,
