...
- 8,748 Python unit tests across ~400 test files (some skipped in container due to host/container path-depth quirks at `Path(__file__).parents[5]` — works on host; collection picks up ~21 errors as of 2026-05-27, tracked for cleanup)
...
The 13-stage flow in `services/pipeline_templates/__init__.py::_CANONICAL_BLOG_ORDER`:
