...
<CardGroup cols={2}>
  ...
  <Card title="Title variety" icon="heading" href="/docs/architecture/title-variety">
    How both title paths are steered off the house formula — corpus habit
    profiling instead of a recent-titles dump.
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card title="Publish scheduling" icon="calendar-days" href="/docs/architecture/publish-scheduling">
    Where a publish slot lives (`posts.status='scheduled'` + `published_at`),
    how approve stages into it, and why the task row never holds one.
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card title="Image OCR text gate" icon="font" href="/docs/architecture/image-ocr-text-gate">
    Generated images carry no legible text. OCR scan, bounded seed re-roll,
    and a 422 that blocks the render rather than annotating it.
  </Card>
