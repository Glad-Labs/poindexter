...
- **Migration files** — `0000_baseline.py` (the squashed history) plus the post-baseline migrations under `services/migrations/`. The baseline is the **Phase F squash (2026-06-22)** — it supersedes the Phase E baseline (2026-06-06, #1194, which absorbed the Phase D baseline + 55 migrations through `20260606_233518_*`), folding in all 73 timestamped migrations from `20260607_*` through `20260622_*` **and retiring the `pipeline_tasks.category` column** (the add→drop→re-add churn collapses to nothing — the new schema simply omits it; `niche_slug` superseded it per #796). So the files in tree are just **2 `.py` files**: `0000_baseline.py` plus the single surviving post-baseline migration `20260622_200222_drop_pipeline_tasks_category.py` — a `DROP COLUMN IF EXISTS category` convergence step that no-ops on fresh installs (the baseline omits the column) but performs the real drop on prod (which still carries it from the pre-squash `20260622_055500` re-add). Phase G can fold that survivor away once every install has dropped the column. Key fold-forward seed deltas captured in the new baseline vs Phase E: `pipeline_templates` 2→5 (canonical_blog v6 39-node + `media_pipeline`/`podcast_pipeline`/`seo_refresh`), `retention_policies` 5→23, `external_taps` 6→1 (the dead global topic taps retired by `promote_niche_sources_to_niche_bound_topic_taps`), `qa_gates` 14→16, `app_settings` ~741→761. New schema changes still go in fresh `YYYYMMDD_HHMMSS_<slug>.py` files; the runner sorts `0000_baseline.py` first because `0` < `2` lexically.
- 12 Grafana dashboards (Mission Control / Pipeline / Cost / Observability / System Health / Integrations / QA Rails / Findings / Experiments & Dry-Run / Database / Hardware & Power / SEO Harvest — Revenue parked 2026-07-01 in `infrastructure/grafana/dashboards-parked/` until `revenue_events` has real writes; each board carries a one-line CHARTER in its description), 16 static Prometheus alert rules in `infrastructure/prometheus/alerts/*.yml` plus DB-rendered + Grafana-managed rules; Pyroscope app-profiles ship from worker/brain/voice agents under `service_name` tags (poindexter#406). The dashboard set was restructured 2026-06-03 (poindexter#654): the dev-blog-only Auto-Publish Gate board was folded into the new **Experiments & Dry-Run** board, and the 75-panel System Health "junk drawer" was split — its Postgres internals → new **Database** board, its GPU/power panels → new **Hardware & Power** board (GPU single-sourced to Prometheus `nvidia_gpu_*`, #653) — leaving a slim ~45-panel System Health. The static repo rules are 16 alerts in 4 active `infrastructure/prometheus/alerts/*.yml` files (deadmans-switch / infrastructure / observability-sidecars / prometheus-alertmanager); `postgres-connections.yml` is a 5th file kept as a comment-only placeholder — its connection rules were migrated to DB-rendered (GH-92, for app_settings-tunable thresholds), as were the disk-space and brain-db-size rules (the #735 observability sweep). On top of the static set, the DB-sourced rules (17 defaults across `poindexter-business` / `poindexter-content` / `poindexter-infra` / `poindexter-infrastructure` / `poindexter-postgres-connections`) are rendered into `rules/*.yml` by `RenderPrometheusRulesJob` every 5 min — so the live count is the 16 static repo rules plus whatever the DB currently seeds.
- ~11,440 test functions across 812 test files under `tests/unit` (+ 16 integration + 17 integration_db) — last recorded full-green nightly 2026-06-09 (10,352 passed, 84 skipped, 1 xfailed, 0 failures, **0 collection errors**); the suite has grown ~1,200 test functions since, so re-run to refresh the pass tally. A handful still skip in-container due to the `Path(__file__).parents[5]` path-depth quirk and run on host; the prior "~21 collection errors" line described a since-fixed problem.
- 1,090 app_settings keys (68 secret) — the baseline seeds the non-secret defaults; secrets get configured per-operator via `poindexter setup` + bootstrap.toml. (The 4 `cost_tier.{free,budget,standard,premium}.model` mappings were removed in PR #1907 — model selection is now per-step `*_model` pins. Cost-guard key rename 2026-05-27 closed a silent fallthrough on `daily_spend_limit_usd` / `monthly_spend_limit_usd` — see #598.)
- `plugins/registry.py` `_SAMPLES` registers 43 job-type plugins (taps + retention + memory hygiene + content surfaces); several are dormant behind master switches, so the live scheduled count is lower
...
- **Atoms are independent graph nodes:** an atom (`modules/content/atoms/`) may import utilities/libraries downward (a `_`-prefixed `atoms/_helper` — the registry skips `_` modules so they're libraries not nodes, e.g. `atoms/_image_helpers.py` — or `services.*`) but must **NOT** import a **stage** (`modules.content.stages.*`, the retiring pre-atom layer) or a **sibling atom**. An architect LLM composes atoms into ad-hoc `graph_def`s, so the graph must be the whole truth of what runs; if atom A imports atom B, placing A on a graph silently runs B with B's contract invisible to the graph. Enforced by `scripts/ci/atom_independence_lint.py` (ratchet). The **image** fossil burned down 2026-07 (`replace_inline_images`'s helper web moved into `atoms/_image_helpers.py`; the zombie `ReplaceInlineImagesStage` class stays as a thin stub pending its own deletion), leaving one baselined fossil: the **writer** — `content.generate_draft` still delegates to the 1,129-LOC `generate_content` stage. Precedent for the target shape: the atom-cutover deleted the `cross_model_qa` **stage** and replaced it with `qa.*` atoms delegating to the `multi_model_qa` **library**.
...
re-rolled most recently by the **Phase F squash (2026-06-22)** — which
folded the Phase E baseline (2026-06-06, Glad-Labs/poindexter#1194) plus
its 73 post-baseline migrations into a fresh baseline and retired the
vestigial `pipeline_tasks.category` column. The file's docstring explains
what each generation captured. The baseline was regenerated fold-forward
from a throwaway DB that ran the full pre-squash chain (correct-by-
construction, not a prod re-dump) and verified byte-for-byte against a
chain `pg_dump`. Exactly **one** post-baseline migration survives —
`20260622_200222_drop_pipeline_tasks_category.py` — because a baseline
only `CREATE TABLE IF NOT EXISTS` (a no-op on prod, which still carries
the column), so it can't drop a column; that migration is the
prod/fresh convergence step. The runner still sorts lexically;
