...
class CloudinaryCMSService:
    ...
    def __init__(self):
        ...
        # Base URLs
        self.api_base_url = f"https://api.cloudinary.com/v1_1/{self.cloud_name}" if self.cloud_name else None
        self.cdn_base_url = f"https://res.cloudinary.com/{self.cloud_name}" if self.cloud_name else None
    ...
    async def optimize_featured_image(
        ...
        try:
            asset = await self.upload_image(
                ...
                alt_text=f"Featured image for {content_title}" if content_title else "Featured image"
            )
