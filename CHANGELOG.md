...
- **Fresh-database installs now apply every migration cleanly.** Ten migrations previously failed against a brand-new database because they assumed schema state from an internal init script that only existed on long-running deployments. New installs would crash partway through bootstrap and leave the database in an inconsistent state. All ten migrations now apply end-to-end on a fresh database, so first-time installs and CI smoke tests no longer need a pre-seeded schema. Existing deployments are unaffected. ([#87](https://github.com/Glad-Labs/poindexter/pull/87))
- **Migration number collision resolved.** Two migrations shared the same `0093` number, making upgrade ordering ambiguous and harder to audit. The object-stores migration has been renumbered to `0117` (after the current max), with no impact on already-applied installs. The QA-gates migration retains `0093` because a later seed migration depends on its ordering. ([#86](https://github.com/Glad-Labs/poindexter/pull/86))
