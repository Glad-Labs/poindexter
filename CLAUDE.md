...
### Key Numbers (as of May 16, 2026)

- 56 live posts on gladlabs.io (222 posts total; 1,519 pipeline_tasks across all generation runs)
- ~327 Python files under `src/cofounder_agent/services/` (~285 substantive after migrations + `__init__.py` stubs; down from ~455 after the 2026-05-08+09+16 cleanup passes). The "load-bearing services" table below covers the services on a critical execution path — `flows/content_generation.py`, `worker_service.py`, and `auto_publish_gate.py` were missing from prior versions of the table.
...
- **In-flight cleanup sweep (2026-05-16)**: 10 PRs ranked by ROI — Prefect Stage 4 (✅ landed), LangGraph cutover finalization (delete `content_router_service.py` legacy chunked path), llm_text → dispatcher consolidation (✅ landed), shared httpx client, Module v1 Phase 3.5 (ContentModule physical move + FinanceModule wiring), settings discipline. **Cutover gates are all already `true` on prod** — Prefect is the dispatcher, canonical_blog is the pipeline, LiteLLM is the LLM router, LlamaIndex+Ragas+DeepEval+Guardrails are all on. Remaining legacy is `content_router_service.py` (dead-but-loaded; sweep removes the dead weight).
...
| Service                                   | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `content_router_service.py`               | Legacy 6-stage content pipeline with cross-model QA. **Scheduled for deletion** in the current sweep — `canonical_blog` LangGraph template at `services/pipeline_templates/__init__.py` is the prod default (`default_template_slug=canonical_blog`). Don't add new stage logic here; new stages go on the LangGraph template.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `flows/content_generation.py`             | Prefect-orchestrated content pipeline (Glad-Labs/poindexter#410). **Sole task dispatch path as of 2026-05-16** (Stage 4 deleted `task_executor.py`). Claims a pending `pipeline_tasks` row, runs it through `content_router_service.process_content_generation_task`, then fires post-pipeline side-effects via `post_pipeline_actions.run_post_pipeline_actions`. Cron / retry / heartbeat / stale-run sweep are all Prefect-native. Deployment registered by `scripts/deploy_content_flow.py`; operator UI at http://localhost:4200. Lane C cutover (#355) routes every task through `TemplateRunner` + the LangGraph `canonical_blog` template when `app_settings.default_template_slug='canonical_blog'`. See [`docs/architecture/prefect-cutover.md`](docs/architecture/prefect-cutover.md) and [`docs/architecture/langgraph-cutover.md`](docs/architecture/langgraph-cutover.md). |
| `template_runner.py`                      | LangGraph-backed dynamic-pipeline orchestrator (TemplateRunner). **PRIMARY PIPELINE PATH as of 2026-05-16** (`canonical_blog` template is the prod default; `default_template_slug=canonical_blog` on prod). Drives `canonical_blog` + `dev_diary` + future architect-composed pipelines. Postgres checkpointer enabled via `template_runner_use_postgres_checkpointer=true`. Templates registered in `services/pipeline_templates/__init__.py`.                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `auto_publish_gate.py`                    | Auto-publish decision logic — imported by `stages/finalize_task.py` to route generated content to `awaiting_approval` or auto-publish based on `quality_score` vs `auto_publish_threshold` (currently 80). On the happy-path of every task. Helpers live in `services/auto_publish.py` (ported from the deleted `task_executor.py` in the 2026-05-16 Stage 4 deletion).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
**Content pipeline stages (canonical_blog LangGraph template — live as of 2026-05-10):**

The 12-stage flow in `services/pipeline_templates/__init__.py::_CANONICAL_BLOG_ORDER`:

1. `verify_task` → 2. `generate_content` (writer, may use RAG mode) → 3. `writer_self_review` → 4. `resolve_internal_link_placeholders` (closes leaked `[posts/<slug>]` markers — added 2026-05-15) → 5. `quality_evaluation` → 6. `url_validation` → 7. `replace_inline_images` → 8. `source_featured_image` → 9. `cross_model_qa` (DeepEval + guardrails + Ragas rails advisory) → 10. `generate_seo_metadata` → 11. `generate_media_scripts` → 12. `capture_training_data` → 13. `finalize_task` (awaiting_approval or auto-publish if score ≥ threshold).

`dev_diary` template uses a 4-node subset: `verify_task` → `narrate_bundle` (atom) → `source_featured_image` → `finalize_task`.

The legacy 6-stage chunked StageRunner flow (`content_router_service.process_content_generation_task`) is **scheduled for deletion in the current sweep** — do not add new logic to it. New stages go on the LangGraph template (`services/pipeline_templates/__init__.py`).
...
- **LangGraph cutover (Lane C):** [`docs/architecture/langgraph-cutover.md`](docs/architecture/langgraph-cutover.md) — **LIVE on prod** as of 2026-05-16 sweep: `app_settings.default_template_slug='canonical_blog'`. The `canonical_blog` LangGraph template in `services/pipeline_templates/__init__.py` is the production pipeline. The legacy chunked StageRunner path in `content_router_service.py` is scheduled for deletion. Do not add new stage logic to the legacy path.
- **Prefect cutover (#410):** [`docs/architecture/prefect-cutover.md`](docs/architecture/prefect-cutover.md) — **Stage 4 complete (2026-05-16)**: `services/task_executor.py` deleted entirely (~1500 LOC). Phase 0 shipped 2026-05-10 behind `app_settings.use_prefect_orchestration`; Stage 3 flipped the fresh-install default to `true` on 2026-05-13; Stage 4 removed the legacy polling daemon now that prod has been running on Prefect dispatch for ~7 clean days. Prefect's deployment owns dispatch entirely; retry / heartbeat / stale-task sweep are native Prefect features. Operator UI at port 4200. The `_notify_discord` / `_notify_alert` helpers moved to `services/integrations/operator_notify.py`; `_auto_publish_task` / `_get_auto_publish_threshold` moved to `services/auto_publish.py`.
