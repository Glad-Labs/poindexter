...
class TestRunner:
    ...
    def __init__(self):
        ...
        self.results = {"start_time": None, "end_time": None, "test_runs": [], "summary": {}}
    ...
    def run_test_suite(self, test_type="all", verbose=True, coverage=True):
        ...
        test_configs = {
            ...
            "smoke": {
                "files": [
                    "test_unit_comprehensive.py::TestIntelligentCoFounder::test_cofounder_initialization"
                ],
    ...
    def generate_report(self):
        ...
        self.results["summary"] = {
            ...
            "average_duration": (
                sum(run["duration"] for run in self.results["test_runs"]) / total_runs
                if total_runs > 0
                else 0
            ),
        }
...
def main():
    ...
    parser.add_argument(
        "--report-only", action="store_true", help="Generate report from existing results only"
    )
    ...
    try:
        if args.test_type == "quick":
            ...
            success = runner.run_test_suite(
                test_type=args.test_type, verbose=not args.quiet, coverage=not args.no_coverage
            )
