...
- **Resource caps:** `CI_RUNNER_CPUS` (default **4**) and `CI_RUNNER_MEM`
  (default **8g**), applied to **each** runner. Memory: the services step runs xdist
...
- **CPU — the cap is PER RUNNER, and the profile ships two.** So the default
  already commits **8 threads** whenever both runners are busy, on a box that is
  also running the production stack, Ollama, and the image/video servers. Budget
  from that number, not from 4.

  The default was 8 until 2026-08-09, i.e. 16 of the operator rig's 32 threads.
  That pinned sustained all-core boost and pushed peak CPU package temperature to
  ~96°C — not dangerous (a modern CPU clamps itself there), but it left nothing
  in reserve and made the machine audibly unpleasant. Dropping to 4 cost about a
  quarter of the median `unit-tests` run — **3.3 → 4.1 min**, max **8.8 → 7.8**,
  excluding the path-filtered sub-minute skips, against a `timeout-minutes: 25`
  budget (post-change n=6, so treat the median as indicative). The step asks for
  4 cores at `-n 4` and never used the other 4, which is why the cut was nearly
  free. 4 also keeps the consumer-stack target hardware — 8-core boxes — usable
  while CI runs, instead of fully saturated.

  Raise it only if you genuinely have threads to spare, and check your own
  headroom against that 25-minute timeout first. `CpuTemperatureHigh` /
  `CpuTemperatureBaselineDrift` (DB-rendered Prometheus rules) alert if cooling
  ever stops keeping up — the first is acute, the second watches the 24h median
