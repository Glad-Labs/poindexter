...
class TaskPlanningService:
    ...
    async def generate_plan(
        ...
        stages = self._generate_stages(
            task_intent_request.suggested_subtasks,
            quality_preference
        )
        ...
        if parallelization_strategy == "parallel":
            ...
            total_duration = max(
                stage.estimated_duration_ms for stage in stages
            ) * 2  # Rough estimate
    ...
    def _generate_stages(
        ...
        for subtask in subtasks:
            ...
            stage = ExecutionPlanStage(
                ...
                quality_metrics=self._determine_quality_metrics(
                    subtask_lower,
                    quality_preference
                ),
            )
    ...
    def _determine_quality_metrics(
        self,
        stage: str,
        quality_preference: str
    ) -> Dict[str, Any]:
    ...
    def _determine_resource_requirements(
        ...
        return {
            ...
            "max_concurrent_cost": sum(
                s.estimated_cost for s in stages if not s.depends_on
            ),
        }
    ...
    async def get_alternative_strategies(
        ...
        if len(task_intent_request.suggested_subtasks) > 2:
            minimal_subtasks = [s for s in task_intent_request.suggested_subtasks 
