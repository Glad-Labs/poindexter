...
- **The CPU row is white, deliberately — not the rig amber.** It is the middle
  readout, so it always lands on the brightest, busiest part of the logo, and
  amber simulates to a yellow-green `(228,234,91)` under deuteranomaly, which
  collapses into the teal circuitry behind it. Measured against the actual
  pixels beneath the glyphs: white beats amber on worst-case contrast **1.86 vs
  1.07** and median **8.07 vs 4.67**. Magenta is the intuitive "opposite of
  cyan" and measures **worse than amber** (median 2.83) — luminance beats hue
  here. Pick these by measuring against the real background and checking a
  colourblind simulation, not by eye.
