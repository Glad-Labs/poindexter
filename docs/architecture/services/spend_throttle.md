...
**File:** `src/cofounder_agent/poindexter/services/spend_throttle.py`
...
(§6) and a fail-**open** sibling of [`pipeline_throttle`](../../../src/cofounder_agent/poindexter/services/pipeline_throttle.py):
instead of gating on approval-queue depth it gates on money — the
[`cost_ledger`](../../../src/cofounder_agent/poindexter/services/cost_ledger.py) `total_usd` (paid API + measured electricity)
...
- [`services/cost_ledger.py`](../../../src/cofounder_agent/poindexter/services/cost_ledger.py) — the spend read seam.
- [`cost_guard.md`](./cost_guard.md) — the per-call hard cap (fails closed).
- [`services/pipeline_throttle.py`](../../../src/cofounder_agent/poindexter/services/pipeline_throttle.py) — the approval-queue sibling.
- `feedback_self_heal_not_suppress` — throttle before page.
