---
title: "Poindexter"
description: "The open-source AI content factory that runs on your PC — writes, reviews, and publishes on its own. Local-first, Ollama-powered, $0 API costs."
---

<Note>
Poindexter is an open-source, self-hosted AI content factory. It researches,
writes, reviews, and publishes long-form content on your own hardware — with a
human-in-the-loop approval queue and 13 QA rails that catch hallucinations
before anything ships.
</Note>

<img
  src="/docs/assets/readme/grafana-pipeline.png"
  alt="The Pipeline dashboard that ships with Poindexter — drafts arrive scored, weak ones auto-rejected, survivors queued for one-click approval"
/>
...
different LLM, runs 13 QA rails over the result, and hands you a queue of
...
<CardGroup cols={3}>
  <Card title="13 QA rails" icon="shield-check">
    Cross-model critics, DeepEval (×3), native guardrails (×2), Ragas,
    citation verification, vision QA — every post survives the gauntlet or
    gets rejected.
  </Card>
  <Card title="60+ registered jobs" icon="clock">
  ...
  <Card title="18 plugin entry points" icon="plug">
  ...
  <Card title="12 Grafana dashboards" icon="chart-line">
    Mission Control, Pipeline, Cost, Observability, System Health,
    Integrations, QA Rails, Findings, Experiments & Dry-Run, Database,
    Hardware & Power, and SEO Harvest — pre-provisioned.
  </Card>
  <Card title="44-node pipeline" icon="list-check">
    `canonical_blog` graph_def: write → self-review → citation repair →
