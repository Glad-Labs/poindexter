...
class OllamaGenerationTester:
    ...
    async def test_model_generation(
        self, model: str, prompt: str, timeout: int = 60
    ) -> Dict[str, Any]:
        """
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    quality_score = self._score_generation_quality(generated_text, prompt)
                    ...
                    logger.info(
                        f"✅ {model} - Quality: {quality_score}/100, Time: {generation_time:.2f}s"
                    )
                    ...
                    self.results["errors"].append({"model": model, "error": error_msg})
                    return {"model": model, "success": False, "error": error_msg}

        except asyncio.TimeoutError:
            logger.error(f"❌ {model} - Request timeout after {timeout}s")
            self.results["errors"].append({"model": model, "error": f"Timeout after {timeout}s"})
            return {"model": model, "success": False, "error": f"Timeout after {timeout}s"}
        except Exception as e:
            logger.error(f"❌ {model} - Error: {e}")
            self.results["errors"].append({"model": model, "error": str(e)})
...
async def test_mistral_generation():
    ...
    result = await tester.test_model_generation(model="mistral", prompt=prompt, timeout=60)
...
async def test_llama2_generation():
    ...
    result = await tester.test_model_generation(model="llama2", prompt=prompt, timeout=60)
...
async def test_model_quality_comparison():
    ...
    for prompt in prompts:
        for model in ["mistral", "llama2"]:
            result = await tester.test_model_generation(model=model, prompt=prompt, timeout=60)
...
async def test_generation_performance():
    ...
    for model in ["mistral", "llama2"]:
        result = await tester.test_model_generation(model=model, prompt=prompt, timeout=120)
...
async def test_content_variety():
    ...
    tester = OllamaGenerationTester()
    test_cases = [
        {"type": "technical", "prompt": "Write a technical guide for implementing REST APIs"},
        {"type": "creative", "prompt": "Write a creative story about a robot discovering emotions"},
        {
            "type": "educational",
            "prompt": "Explain the theory of relativity for a high school student",
        },
        {"type": "business", "prompt": "Write a business case for adopting cloud computing"},
    ]

    for test_case in test_cases:
        result = await tester.test_model_generation(
            model="mistral", prompt=test_case["prompt"], timeout=60
        )
