"""Compatibility stub -- `utils` now lives at `poindexter.utils` (Glad-Labs/poindexter#1046, step 2).

Importing this installs the flat-import alias finder and replaces this module in
``sys.modules`` with the canonical package, so ``import utils.x`` yields the SAME
object as ``import poindexter.utils.x`` -- one module, two names (see
...
# `cofounder_agent.utils` (the umbrella spelling the entry points use).
