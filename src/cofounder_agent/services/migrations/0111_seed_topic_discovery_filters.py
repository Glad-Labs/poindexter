...
- ``_CATEGORY_SEARCHES`` — category → keyword-query map used by
...
_SEEDS: list[tuple[str, str, str]] = [
    (
        ...
        "regex-based junk filtering entirely. The trailing-fragment "
        "and leading-emoji filters in services.topic_sources._filters "
    ...
    (
        ...
        "scraped title into a category, and by WebSearchSource to issue "
        "DuckDuckGo queries per category. Empty object falls back to the "
        "hardcoded Glad Labs categories (technology / startup / security "
        "/ engineering / insights / business / hardware / gaming) for "
        "backwards compatibility. Customers in a different niche should "
        "set this to their own category map so classification doesn't "
        "default everything to 'technology'.",
    ),
]
