...
class MigrationService:
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                already_applied = await conn.fetchval(
                    "SELECT id FROM migrations_applied WHERE name = $1", migration_name
                )
            ...
            async with self.pool.acquire() as conn:
                ...
                await conn.execute(
                    "INSERT INTO migrations_applied (name) VALUES ($1)", migration_name
                )
    ...
    async def rollback_last_migration(self) -> bool:
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                await conn.execute("DELETE FROM migrations_applied WHERE name = $1", last_migration)
